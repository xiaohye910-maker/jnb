  ...
  */
  ...
  $(document).on('change', 'input[name="autoship_scheduled_orders_display_version"]', function(e) {
    ...
    if( 'template' == val ) {
      ...
      $('.autoship_v2_portal_extra_options').hide();
    } else if ( 'v2_portal' == val ) {
      $('.autoship_native_ui_extra_options').hide();
      $('.autoship_v2_portal_extra_options').show();
    } else {
      $('.autoship_native_ui_extra_options').hide();
      $('.autoship_v2_portal_extra_options').hide();
    }
  ...
  $( document ).on( 'autoship_autoship_bulk_update_scheduled_orders_wholesale_pricing_BulkActionComplete', dynamicResult );
    ...
    // Go to the lead step.
    $(document).on('click', '#autoship-go-to-quicklaunch-button', function() {
      const quicklaunchUrl = $(this).data('quicklaunch-url');
      window.location.href = quicklaunchUrl;
    });
