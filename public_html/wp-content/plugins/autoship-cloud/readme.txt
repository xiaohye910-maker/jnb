...
= 2.12.3 - 2026-03-13 =
  ...
  - Fixed: QPilot API URL construction that caused "Could not resolve host" errors by ensuring a proper path separator between the base URL and endpoint paths.

= 2.12.2 - 2026-03-12 =

  - New! Migrated the health check system to the modern module architecture with dedicated REST and AJAX controllers, a centralized health check service, and full legacy compatibility.

  - New! Added a Labels service for centralized management of UI labels and display strings.

  - Improved: Reorganized legacy code files (logger, deprecated functions, languages, health checks) into a dedicated `src/legacy/` directory with proper deprecation notices.

  - Improved: Removed the chatbot (WebChat) feature and its dependencies.

  - Improved: Cleaned up unused legacy code including WC-Autoship dead code, export/import utilities, and upgrade routines.

  - Improved: Updated positioning of event logs and product dropdown on the scheduled order deletion modal.

  - Improved: Added `is_tracing_enabled` support on logs for better debugging control.

  - Improved: Updated translations and composer configuration.

  - Fixed: Stripe gateway metadata fees compatibility for Stripe plugin versions earlier than 8.8.0.

= 2.12.1 - 2026-03-04 =

  - New! Extended the centralized payment gateway module with support for Braintree, Square, CyberSource V2, NMI, Paya V1, TrustCommerce, Sage, Checkout.com, and Airwallex. All payment gateways are now managed through the modern architecture with full backward compatibility.

  - Improved: Refactored the bulk frequency updates utility for better reliability.

= 2.12.0 - 2026-02-16 =

  - New! Payment gateway integrations are now managed through a centralized module with support for Stripe, PayPal and Authorize.NET. This provides a unified, modern architecture for handling payment method synchronization with QPilot.

  - New! Added a payment compatibility layer that bridges the new modular payment system with existing WooCommerce payment hooks, ensuring a seamless transition with no disruption to existing store configurations.

  - New! Payment gateways can now be remotely enabled or disabled via QPilot feature flags, giving merchants and support teams centralized control over which payment methods are active without requiring code changes.

= 2.11.0 - 2026-02-03 =

  - New! The new Admin Portal is now integrated into the WooCommerce plugin, making it easier for teams to update and manage Scheduled Orders directly within the plugin pages.

  - New! Built-in Quick Actions that allow customers to easily and quickly update their Scheduled Orders. Quick Actions also empower upsells and retention campaigns. Check it out under Retain and Grow and keep an eye out for more actions coming soon.

= 2.10.5.3 - 2026-01-15 =

  - Fixed: Resolved a critical error on product pages when viewing variable products with variations that have missing or empty regular prices.

= 2.10.5.2 - 2025-12-03 =

  - Fixed: Resolved a critical error when the v2 Subscriber Portal was enabled on WordPress 6.9.0.

= 2.10.5.1 - 2025-10-31 =

  - Fixed: Corrected an issue where multiple notices were shown on the checkout page for wholesale customers.

= 2.10.5 - 2025-10-31 =

  - Fixed: Corrected an issue where updating cart quantities did not trigger notifications when wholesale minimum quantity requirements were not met.

  - Improved: Integration with FunnelKit Payment Gateway now supports ACH payments.

= 2.10.4 - 2025-09-24 =

  - Fixed: Resolved an issue where updating the order status on the _WooCommerce_ > _Edit Order_ page would cause a critical error.

  - Fixed: Corrected a problem where Airwallex payment methods were not being deleted from QPilot customer profiles  or Scheduled Orders as expected.

  - Fixed: Fixed an issue where an Airwallex payment method was not being assigned to a customer’s Scheduled Order during checkout if they were a first time customer.

= 2.10.3 - 2025-09-16 =

  - New! New integration with [Nextime.Ai](https://nextime.ai), which is a shipping and last-mile delivery solution used by Subscription Apps to increase Subscriber Value and Retention by communicating delivery dates to Subscribers and optimizing upcoming order dates, delivery methods, and shipping rates for Merchants whenever an upcoming Subscription Order is changed.

  - New! Admins can now sync customer updates from _WP-Admin_ > _Users_ > _Edit_ directly to all existing Scheduled Orders for that customer.

  - Improved: Performance enhancements on _My Account_ and _WP-Admin_ pages which increase loading speeds for admins and customers.

= 2.10.2 - 2025-09-05 =

  - Fixed: Resolved a site wide critical error when updating a user from the User Management page.

= 2.10.1 - 2025-09-03 =

  - Fixed: Resolved an issue in the quick launch registration page that prevented new merchants to start the installation.

  - Improved: Schedule Orders page enhanced for faster loading times.

= 2.10.0 - 2025-08-28 =

  - Improved: Increased WordPress compliance for improved platform compatibility.

  - Improved: Applied key security fixes to strengthen protection and reliability.

  - Improved: Payment Methods page enhanced for faster loading times.

  - New: Recurring product sync is enabled by default for new WooCommerce sites.

= 2.9.4 - 2025-07-31 =

  - Improved! The plugin onboarding "Quicklaunch" now requires a Merchant's phone number, enabling merchants to receive faster support and timely updates.

= 2.9.3 - 2025-07-29 =

  - Fixed: Resolved an issue with the Autoship discounted price displaying twice on some product pages.

  - Fixed: Resolved a site wide critical error when upgrading to WooCommerce version 10 through a file upload.

  - Fixed: Resolved an issue with prices reflecting incorrectly when switching between one-time purchases and Autoship on product pages.

= 2.9.2 - 2025-07-08 =

 -  New! [FunnelKit Payment Gateway for Stripe WooCommerce](https://wordpress.org/plugins/funnelkit-stripe-woo-payment-gateway/) is now a supported payment integration for Autoship Cloud. Merchants using the FunnelKit Payment Gateway for WooCommerce can now use Autoship with the FunnelKit integration to securely save payment methods during an Autoship Checkout and for processing Autoship Scheduled Orders.

= 2.9.1 - 2025-06-30 =

  - Fixed: Resolved an issue with Stripe pay by link payment methods not saving to Scheduled Orders after checkout.

= 2.9.0 - 2025-06-25 =

  - New! The plugin [Airwallex Online Payments Gateway](https://wordpress.org/plugins/airwallex-online-payments-gateway/) by Airwallex is now supported with Autoship! Merchants using the Airwallex Online Payments Gateway for WooCommerce can now use Autoship with the Airwallex integration to securely save payment methods during an Autoship Checkout and for processing Autoship Scheduled Orders.
  - Fixed: Resolved an issue where [Authorize.net](http://authorize.net/) payment methods were not being saved after checkout to Scheduled Orders when using WooCommerce block checkout options.

= 2.8.10.1 - 2025-06-16 =

 - Fixed: Fixed versioning issue with the plugin.
 
= 2.8.10 - 2025-06-12 =

 - Improved! The Settings page link in the Autoship Cloud plugin menu now directs to `/wp-admin/admin.php?page=autoship` instead of `/page=settings`, preventing conflicts with other plugin settings pages.
 - Improved! Merchants using the plugin onboarding flow will now see an alert if they attempt to register a QPilot account with an email address that already exists.
 - Fixed: Shipping address updates made via _My Account > Addresses_ in WooCommerce now correctly sync with Scheduled Orders.

= 2.8.9.2 - 2025-06-02 =

 - Fixed: Resolved an issue that prevented sites using legacy storage file systems from loading the plugin for the first time.

= 2.8.9.1 - 2025-05-29 =

 - New! Streamlined and improved onboarding experience for new plugin installs to make setup faster, clearer, and easier than ever.

= 2.8.9 - 2025-05-23 =

 - Fixed: Unable to check "Save Card" when using block checkout style and a new payment method is added.
 - Fixed: Incorrect "Autoship and Save" Discount Displayed in Cart for Wholesale Product.

= 2.8.8 - 2025-05-14 =

 - Fixed: Autoship checkouts completed with Stripe Payment Links are now successfully saving to Scheduled Orders for future processing.

= 2.8.7 - 2025-04-30 =

- Improved! Merchants can now control product syncing directly from _Autoship Cloud > Settings > Utilities_, ensuring their product data syncs on a regular, smart cadence that prioritizes unsynced products.

- New: Merchants can easily monitor and troubleshoot issues with product syncing using detailed error logging in _Autoship Cloud > Settings >> Logs_.

- New: Added the ability to customize the "No Scheduled Orders" message shown to customers in _My Account > Scheduled Orders_ when no Scheduled Order exists. This can be managed directly from the QPilot Merchant Center when the v2 portal display option is enabled.

= 2.8.6 - 2025-04-15 =

- Fixed: Resolved an issue where wholesale prices were not showing on product pages when a wholesale customer selected Autoship options.

- Fixed: Resolved an issue where the Save Card for payment methods option was not enabled by default when using WooCommerce block-style checkout and Autoship items are in the cart.

= 2.8.5 - 2025-03-27 =

- New! Change your site wide frequency options in bulk from _WP-Admin > Autoship Cloud > Settings > Utilities_
  - Updating the site-wide settings will automatically update the frequency options displayed on product pages and offered to customers at checkout.
  - If you have overridden the frequency options on a specific product in _WP-Admin > Products > Edit_ then those settings will remain active as well.

= 2.8.4 - 2025-03-24 =

- Fixed: Improved the customer experience when adding and updating Items in My Account > Scheduled Orders when using the Native Ui. This is due to corrected HTML properties verification when loading products in the Native Ui over JS.
 
- New! Added a new bulk utility in Autoship Cloud > Settings >> Utilities that enable wholesale suite customers to sync wholesale pricing rules to existing Scheduled Orders
 
- Fixed: Resolved an issue where the wholesale customer product price was missing after making changes to a Scheduled Order in My Account > Scheduled Orders

= 2.8.3 - 2025-03-06 =

- Fixed: Resolved an issue where several terms (including Scheduled Order status, the Next Occurrence Date month, and the "Months" for the frequency) could not be translated within the Native UI when using Loco Translate.
