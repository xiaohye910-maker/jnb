<?php
/**
 * The functional code that handles the customers integration.
 *
 * @package Autoship
 * @since 1.0.0
 */
...
function autoship_generate_customer_shipping_data( $wc_customer, $customer_data = array() ) {
	...
	return apply_filters(
		'autoship_generated_customer_shipping_data',
		array_merge(
			array(
				...
				'shippingCompany'   => $wc_customer->get_shipping_company(),
			),
			$customer_data
		),
		$wc_customer,
...
function autoship_generate_customer_billing_data( $wc_customer, $customer_data = array() ) {
	...
	// Get the Customer Data.
	return apply_filters(
		'autoship_generated_customer_billing_data',
		array_merge(
			array(
				...
				'billingCompany'   => $wc_customer->get_billing_company(),
			),
			$customer_data
		),
		$wc_customer,
...
function autoship_generate_customer_data( $wc_customer, $customer_data = array() ) {
	...
	$customer_data['applyToScheduledOrders'] = autoship_get_apply_to_scheduled_orders_setting();

	// Get the Customer Data.
	return apply_filters(
		'autoship_generated_customer_data',
		array_merge(
			array(
	...
	if ( isset( $wp->query_vars['update-scheduled-orders-customer-info'] ) ) {
		...
		if ( ( get_current_user_id() === $wc_customer_id ) || autoship_rights_checker( 'autoship_update_customer_info_on_all_scheduled_orders', array( 'administrator' ) ) ) {
			...
			if ( is_wp_error( $autoship_customer ) ) {
				...
				$message = esc_html( sprintf( __( 'There was a problem updating the information on your %1$s. Additional details: %2$s', 'autoship' ), $schedule_label, $autoship_customer->get_error_message() ) );
				$notice  = apply_filters(
					'autoship_update_customer_info_on_all_scheduled_orders_failure_notice',
					array(
						'message' => $message,
						'code'    => 'error',
					),
				...
				// translators: %1$s is the label for Scheduled Orders.
				$message = esc_html( sprintf( __( 'Your %1$s have been successfully updated.', 'autoship' ), $schedule_label ) );
				$notice  = apply_filters(
					'autoship_update_customer_info_on_all_scheduled_orders_updated_notice',
					array(
						'message' => $message,
						'code'    => 'success',
					),
		...
		wp_redirect( wc_get_account_endpoint_url( 'edit-address' ) ); // phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
		exit();
	}
}
...
function autoship_after_update_customer_autoship_actions( $wc_customer_id, $load_address ) {
	...
	if ( $trigger_notice && isset( $_POST['autoship_apply_to_scheduled_orders_by_default'] ) && absint( $_POST['autoship_apply_to_scheduled_orders_by_default'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		if ( is_wp_error( $autoship_customer ) ) {
			// translators: %1$s is the label for Scheduled Orders, %2$s is the error message.
			$message = esc_html( sprintf( __( 'There was a problem updating the shipping information on your %1$s. Additional details: %2$s', 'autoship' ), $schedule_label, $autoship_customer->get_error_message() ) );
			$notice  = apply_filters(
				'autoship_update_customer_info_on_all_scheduled_orders_failure_notice',
				array(
					'message' => $message,
	...
	// Only display the notice if the update was not applied by default to all scheduled orders.
	if ( $trigger_notice && apply_filters( 'autoship_display_customer_update_actions', true, get_current_user_id() ) ) {
		...
		// translators: %1$s is the label for Scheduled Orders, %2$s is the link to update the shipping address, %3$s is the label for Update.
		$message = sprintf( __( 'Update Shipping Address information for %1$s? <a href="%2$s" tabindex="1" class="button wc-forward">%3$s</a>', 'autoship' ), $schedule_label, esc_url( $url ), esc_html__( 'Update', 'autoship' ) );
		$notice  = apply_filters(
			'autoship_apply_customer_info_to_all_scheduled_order_default_action_notice',
			array(
				'message' => $message,
				'code'    => 'success',
			),
...
function autoship_display_apply_customer_info_to_all_scheduled_orders_checkbox() {
	...
	woocommerce_form_field(
		'autoship_apply_to_scheduled_orders_by_default',
		array(
			'type'        => 'checkbox',
		...
		apply_filters(
			'autoship_display_apply_customer_info_to_all_scheduled_orders_checkbox_default',
			0,
			get_current_user_id()
		)
	);

	echo apply_filters( 'autoship_display_apply_customer_info_to_all_scheduled_orders_checkbox', ob_get_clean(), apply_filters( 'autoship_display_apply_customer_info_to_all_scheduled_orders_checkbox_default', 0, get_current_user_id() ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
 ...
 *
 * @param string $notice The notice to display before the link.
 */
function autoship_display_apply_customer_info_to_all_scheduled_orders_btn_link( $notice ) {
	...
	if ( apply_filters( 'autoship_display_customer_update_actions', true, get_current_user_id() ) ) {
		...
		echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			'autoship_display_apply_customer_info_to_all_scheduled_orders_btn_link_label',
			wp_kses_post(
				sprintf(
					// translators: %1$s is the link to update the shipping address, %2$s is the label for Scheduled Orders.
					__( 'Click <a href="%1$s">here</a> to update your shipping address on all %2$s', 'autoship' ),
					$url,
	...
	echo apply_filters( 'autoship_display_apply_customer_info_to_all_scheduled_orders_btn_label', ob_get_clean(), $url, $notice ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
...
function autoship_update_autoship_customer( $wc_customer, $customer_data = array() ) {
	...
	try {
		...
		$result = new WP_Error( 'Updating Customer Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_create_update_autoship_customer( $wc_customer, $customer_data = array() ) {
	...
	return autoship_update_autoship_customer( $wc_customer, apply_filters( 'autoship_create_update_autoship_customer', $customer_data, $wc_customer ) );
}
...
function autoship_maybe_create_update_autoship_customer( $wc_customer, $customer_data = array() ) {
	...
	return autoship_customer_type_validation( $wc_customer ) ? autoship_create_update_autoship_customer( $wc_customer, $customer_data ) : false;
}
...
function autoship_customer_type_validation( $wc_customer ) {
	...
	foreach ( $valid_types as $type ) {
		...
		if ( in_array( $type, (array) $user->roles, true ) ) {
			$valid = true;
		}
	}
 ...
 * @param int   $index The page index to return. Default 1.
 * @param bool  $iterative True iterate through and return all results. If False only the page and params requested returned.
...
function autoship_search_available_customers( $params = array(), $index = 1, $iterative = true ) {
	...
	try {
		...
		$notice    = autoship_expand_http_code( $e->getCode() );
		$customers = new WP_Error( 'Customer Search Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	if ( $customers->totalPages > $index ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		++$index;
 ...
 *
 * @param int|WC_Customer $wc_customer WooCommerce customer ID or WC Customer Object.
 * @param string          $action The scenario for when this is being called. Used for the new filter so auto-create on 404 can be controlled.
 *
...
function autoship_customer_has_scheduled_order( $wc_customer_id ) {
	...
	$filter = wp_parse_args(
		...
		apply_filters(
			'autoship_customer_has_scheduled_order_filter',
			array(
				'frequency'      => null,
				'frequency_type' => null,
			),
...
function autoship_available_customer_summaries( $params = array() ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = new WP_Error( 'QPilot Customer Summaries Not Found', __( 'Customer Summaries Data matching the supplied values can not be found in QPilot', 'autoship' ) );
...
function autoship_get_customer_metrics_fields( $field = '' ) {
	...
	$fields = apply_filters(
		'autoship_customer_metrics_fields_mapping',
		array(
...
if ( ! function_exists( 'autoship_get_customer_metrics_data' ) ) {
	 ...
	 *
	 * @return mixed|array The value(s) for the requested fields. If string supplied then return value maybe single.
	 */
...
if ( ! function_exists( 'autoship_save_customer_metrics_data' ) ) {
	 ...
	 * @param bool  $translate Optional. If true will translate the raw field names to the WP Meta field names.
	 */
...
/**
 * Includes the Customer Metrics Refresh & TimeStamp on the User Profile.
 *
...
function autoship_include_customer_metrics_on_profile( $user ) {
	...
	if ( ! empty( $date ) ) {
		...
		// translators: %s is the date of the last download.
	...
	<h3><?php echo esc_html( __( 'Autoship Customer Metrics', 'autoship' ) ); ?></h3>
		...
		<tr>
			<td>
				<div id="autoship_refresh_customer_metrics_field">
					<?php
					woocommerce_form_field(
						'autoship_refresh_customer_metrics_data',
						array(
							'type'     => 'checkbox',
							'required' => false,
...
/**
 * Retrieves the Apply to Scheduled Orders setting for a customer.
 *
 * @return bool True if enabled else false.
 */
function autoship_get_apply_to_scheduled_orders_setting() {
	$value = isset( $_POST['autoship_apply_to_scheduled_orders_by_default'] ) ? (bool) $_POST['autoship_apply_to_scheduled_orders_by_default'] : false; // phpcs:ignore WordPress.Security.NonceVerification.Missing

	return empty( $value ) ? false : (bool) $value;
}

/**
 * Includes the Sync Autoship Orders on the User Profile.
 */
function autoship_include_sync_autoship_orders_on_profile() {
	?>

	<table class="form-table">
		<tr>
			<td>
				<div id="autoship_apply_to_scheduled_orders_field">
					<?php
					woocommerce_form_field(
						'autoship_apply_to_scheduled_orders_by_default',
						array(
							...
							'required' => false,
							'class'    => array( 'autoship-option' ),
							'label'    => __( 'When enabled, any changes made to this customer’s account will also update their Autoship Scheduled Orders. Changes to the customer profile are always synced, even if this setting is disabled', 'autoship' ),
						)
					);
					?>
				</div>
...
function autoship_process_customer_metrics_refresh_from_profile( $user_id ) {
	...
	$refresh = isset( $_POST['autoship_refresh_customer_metrics_data'] ) ? (bool) $_POST['autoship_refresh_customer_metrics_data'] : false; // phpcs:ignore WordPress.Security.NonceVerification.Missing
...
add_action( 'edit_user_profile', 'autoship_include_sync_autoship_orders_on_profile' );
add_action( 'show_user_profile', 'autoship_include_sync_autoship_orders_on_profile' );

/**
 * Get total customers (cached to reduce API calls on admin render).
 */
function autoship_get_total_customers_cached() {
	$ttl       = apply_filters( 'autoship_total_customers_ttl', 15 * 60 );
	$cache_key = '_autoship_total_customers';
	$exp_key   = '_autoship_total_customers_expiration';

	$now = time();
	$exp = (int) get_option( $exp_key, 0 );
	$val = get_option( $cache_key, null );

	if ( is_numeric( $val ) && $exp > $now ) {
		return (int) $val;
	}
	...
	try {
		...
		$total     = isset( $customers->totalCount ) ? (int) $customers->totalCount : 0; // phpcs:ignore
	} catch ( Exception $e ) {
		$total = 0;
	}

	update_option( $cache_key, $total, false );
