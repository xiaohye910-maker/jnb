<?php
/**
 * The payment functions for Autoship.
 *
 * @package Autoship
 * @since 1.0.0
 */
	...
	$types = apply_filters(
		...
		array_merge(
			array(
				...
				'nmi'                                 => 'standard',
				'airwallex_card'                      => 'standard',
				'fkwcs_stripe'                        => 'standard',
				'fkwcs_stripe_ach'                    => 'standard',
			),
...
function autoship_get_valid_payment_methods() {
	...
	return apply_filters(
		...
		array(
			...
			'nmi'                                 => 'nmi_gateway',
			'airwallex_card'                      => 'airwallex_card',
			'fkwcs_stripe'                        => 'fkwcs_stripe',
			'fkwcs_stripe_ach'                    => 'fkwcs_stripe_ach',
		)
	);
}
 ...
 *
 * @param string $method_gateway_id The method gateway id.
 *
 * @return string The valid payment method type.
 */
...
function autoship_enable_payment_method_types_for_optional_gateways( $types ) {
	if ( class_exists( '\WooCommerce\PayPalCommerce\PPCP' ) && \WooCommerce\PayPalCommerce\PPCP::container()->get( 'vaulting.vault-v3-enabled' ) ) {
...
function autoship_get_related_tokenized_id( $token, $tokenize = true ) {
	global $wpdb;
	$token_id = $wpdb->get_var( $wpdb->prepare( "SELECT token_id FROM {$wpdb->prefix}woocommerce_payment_tokens WHERE token = %s", $token ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
...
function autoship_get_general_payment_method_customer_data( $wc_customer, $data = array() ) {
	...
	$data = array_merge( array( 'customerId' => $wc_customer->get_id() ), $data );
 ...
 *
 * @param WC_Payment_Token|int $token_id The WC Payment Token or Token ID.
...
function autoship_get_general_payment_method_data( $token_id, $gateway ) {
	...
	return ! is_null( $token ) ? autoship_add_general_payment_method( $token, true ) : array();
}
...
function autoship_get_order_payment_data( $order_id ) {
	...
	if ( isset( $valid_methods[ $payment_method_id ] ) ) {
		...
		$valid_name = $valid_methods[ $payment_method_id ];

		Logger::trace( 'Autoship Payment Methods Tracer', 'Using valid payment method for ' . $payment_method_id . ' the gateway is ' . $valid_name );
		...
		if ( function_exists( $function ) ) {
			...
			Logger::trace( 'Autoship Payment Methods Tracer', 'There is no processor with name ' . $function );
		}
	} else {
		Logger::trace( 'Autoship Payment Methods Tracer', 'There is no processor for ' . $payment_method_id );
	}
 ...
 */
function autoship_get_payment_method_cod_order_payment_data( $order_id, $order ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
...
/**
 * Gets customer payment tokens from PayPal API
 *
 * @param int $gateway_customer_id Customer ID.
 *
 * @return array Array of customer tokens
 */
function autoship_get_paypal_payments_tokens_for_customer( $gateway_customer_id ) {
	$payment_tokens_endpoint = \WooCommerce\PayPalCommerce\PPCP::container()->get( 'api.endpoint.payment-tokens' );

	try {
		$customer_tokens = $payment_tokens_endpoint->payment_tokens_for_customer( $gateway_customer_id );
 ...
 *
 * @return QPilotPaymentData
 * @throws Exception If the PayPal order cannot be retrieved.
 */
function autoship_get_ppcp_order_payment_data( $order_id, $order ) {

	if ( $order->get_payment_method() != 'ppcp-gateway' ) { //phpcs:ignore
		return null;
	}
...
/**
 * Retrieves the Payment information from the order for the Paypal Payments Gateway Advanced Credit Card
 *
 * @param int      $order_id The WC Order number.
 ...
 * @throws Exception If the PayPal order cannot be retrieved.
 */
function autoship_get_ppcp_cc_order_payment_data( $order_id, $wc_order ) {

	if ( $wc_order->get_payment_method() !== 'ppcp-credit-card-gateway' ) {
		return null;
	}
	...
	$card_payment_token_id = wc_clean( wp_unslash( $_POST['wc-ppcp-credit-card-gateway-payment-token'] ?? '' ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	$user_id               = $wc_order->get_user_id();
	$wc_tokens             = WC_Payment_Tokens::get_customer_tokens( $user_id, 'ppcp-credit-card-gateway' );
	if ( ( $card_payment_token_id && 'new' != $card_payment_token_id ) && $wc_tokens ) { //phpcs:ignore
		...
		foreach ( $wc_tokens as $token ) {
			if ( $token->get_id() === (int) $card_payment_token_id ) {
				...
				$gateway_payment_token = $token->get_token();
				$token_description     = $token->get_display_name();
				$last_four             = $token->get_last4() ?? '';
				if ( $token->get_expiry_month() && $token->get_expiry_year() ) {
					$expiration = $token->get_expiry_year() . '-' . $token->get_expiry_month();
				}
				break;
			}
		}
		...
		// Get payment data from PayPal if not found in WP.
		$order_meta     = $wc_order->get_meta( \WooCommerce\PayPalCommerce\WcGateway\Gateway\PayPalGateway::ORDER_ID_META_KEY );
		$order_id       = $order_meta ? $order_meta : wc_clean( wp_unslash( $_POST['paypal_order_id'] ?? '' ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$order_endpoint = \WooCommerce\PayPalCommerce\PPCP::container()->get( 'api.endpoint.order' );
		...
		if ( is_string( $order_id ) && $order_id ) {
			...
			if ( $payment_vault_attributes ) {
				$gateway_customer_id   = $payment_vault_attributes->customer->id ?? '';
				$gateway_payment_token = $payment_vault_attributes->id ?? '';
				$last_four             = $payment_source->properties()->last_digits ?? '';
				$expiration            = $payment_source->properties()->expiry ?? '';
				$brand                 = $payment_source->properties()->brand;
			}
		}
	}
	...
	if ( ! empty( $gateway_payment_token ) ) {
		if ( ! $token_description ) {
			$token_description = $expiration ? sprintf( '%s ending in %s (expires %s)', ucfirst( strtolower( $brand ) ), $last_four, $expiration ) : sprintf( '%s ending in %s', ucfirst( strtolower( $brand ) ), $last_four );
		}
		$payment_data                       = new QPilotPaymentData();
		$payment_data->description          = $token_description;
		$payment_data->type                 = 'PayPalV3';
		$payment_data->gateway_payment_id   = $gateway_payment_token;
...
function autoship_get_authorize_net_order_payment_data( $order_id, $order ) {
	...
	Logger::trace( 'Autoship Payment Methods Tracer', 'Entering autoship_get_authorize_net_order_payment_data. Order ID: ' . $order_id );
	...
	$export_token_string = var_export( $token_string, true ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export
	$export_customer_id  = var_export( $customer_id, true ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export

	Logger::trace( 'Autoship Payment Methods Tracer', 'The token string is ' . $export_token_string );
	Logger::trace( 'Autoship Payment Methods Tracer', 'The token string is ' . $export_customer_id );
	...
	if ( ! empty( $token_string ) && ! empty( $customer_id ) ) {
		...
		$export_payment_data = var_export( $payment_data, true ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export
		Logger::trace( 'Autoship Payment Methods Tracer', 'The payment method is: ' . $export_payment_data );

		return $payment_data;
	} else {
		Logger::trace( 'Autoship Payment Methods Tracer', 'Unable to retrieve authorize net order data. ' );
	}
 ...
 */
function autoship_stripe_intent_request_filter( $request, $order ) {
 ...
 *
 * @param bool $val The current value of the force save.
 */
 ...
 * @return bool True.
 */
function autoship_force_nmi_payment_tokenization( $force ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
	return true;
}
...
function autoship_sagepaydirect_order_payment_data_saved_payment_patch( $order_id, $posted_data, $order ) {
	...
	$sage_card_save_token = isset( $_POST['wc-sagepaydirect-new-payment-method'] ) && ! empty( $_POST['wc-sagepaydirect-new-payment-method'] ) ? wc_clean( $_POST['wc-sagepaydirect-new-payment-method'] ) : false; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
/**
 * Removes the Cybersource payment gateway from the available gateways on the account page.
 * This is to prevent issues with the Cybersource gateway not being compatible with the
 * Autoship payment method flow.
 *
...
/**
 * Retrieves the Payment information from the order for the Airwallex Card Gateway
 *
 * @param int      $order_id The WC Order number.
 * @param WC_Order $order The WC Order object.
 *
 * @return QPilotPaymentData
 */
function autoship_get_airwallex_card_order_payment_data( $order_id, $order ) {
...
/**
 * Retrieves the Payment information from the order for the Funnelkit Stripe Gateway
 *
 * @param object | null $payment_data The QPilotPaymentData data, or null.
 * @param int           $order_id The WC Order number.
 * @return QPilotPaymentData
 */
function autoship_get_fkwcs_stripe_order_payment_data( $payment_data, $order_id ) {
	$order             = wc_get_order( $order_id );
	$payment_method_id = $order->get_payment_method();

	if ( 'fkwcs_stripe' !== $payment_method_id ) {
		return $payment_data;
	}
...
/**
 * Retrieves the Payment information from the order for the Funnelkit Stripe Gateway ACH
 *
 * @param object | null $payment_data The QPilotPaymentData data, or null.
 * @param int           $order_id The WC Order number.
 * @return QPilotPaymentData
 */
function autoship_get_fkwcs_stripe_ach_order_payment_data( $payment_data, $order_id ) {
	$order             = wc_get_order( $order_id );
	$payment_method_id = $order->get_payment_method();

	if ( 'fkwcs_stripe_ach' !== $payment_method_id ) {
		return $payment_data;
	}
 ...
 *
 * @param array                                                    $result Result of the add payment method transaction.
 * @param \SV_WC_Payment_Gateway_API_Create_Payment_Token_Response $response API response object.
 * @param \WC_Order                                                $order order instance.
 * @param \SV_WC_Payment_Gateway_Direct                            $client client instance.
 *
 * @return mixed
 ...
 * }
 */
function autoship_add_authorize_net_payment_method( $result, $response, $order, $client ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 */
function autoship_add_my_account_skyverge_payment_method( $token, $user_id, $response ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param WC_Payment_Token_CC $token The payment token object.
 *
...
function autoship_delete_authorize_net_payment_method( $token_id, $user_id ) {
	...
	return ! isset( $types['authorize_net_cim_credit_card'] ) ? autoship_delete_non_wc_token_payment_method( $token_id, null, 'AuthorizeNet', $user_id ) : false;
}
 ...
 * }
 */
function autoship_add_braintree_credit_card_payment_method( $result, $response, $order, $client ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param array               $payment_method_data Current payment Method Data.
 * @param string              $autoship_method_type The QPilot Method Type.
 * @param WC_Payment_Token_CC $token                The WC Payment Token object.
 *
 * @return array The modified Payment Method Data to Send to QPilot
 */
function autoship_add_braintree_payment_method_data( $payment_method_data, $autoship_method_type, $token ) {
	...
	$ext_type = 'Paypal' == $type ? 'braintree_paypal' : 'braintree_credit_card'; //phpcs:ignore
	...
	if ( ! isset( $types[ $ext_type ] ) && 'Paypal' == $type ) { //phpcs:ignore
	...
	} elseif ( 'Paypal' == $type ) { //phpcs:ignore
 ...
 * }
 */
function autoship_add_braintree_paypal_payment_method( $result, $response, $order, $client ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * }
 */
function autoship_add_square_payment_method( $result, $response, $order, $client ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	// Check if the transaction has been approved.
 ...
 * @return array The modified Payment Method Data to Send to QPilot
 */
function autoship_filter_deleted_payment_method_gateway_ids( $gateway_id, $token ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
...
function autoship_add_stripe_payment_method( $payment_method_data, $type, $token ) {
	...
	if ( 'fkwcs_stripe' === $token->get_gateway_id() || 'fkwcs_stripe_ach' === $token->get_gateway_id() ) {
...
/**
 * Handle Stripe Link payment method addition
 *
 * @param mixed            $payment_method The payment method data.
 * @param WC_Payment_Token $token The payment token.
 * @param array            $data Additional data.
 *
 * @return mixed Modified payment method data
 */
function autoship_add_stripe_link_payment_method( $payment_method, $token, $data ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	if ( $token instanceof WC_Payment_Token_CC ) {
		$payment_method = new QPilotPaymentData();

		$payment_method->description          = $token->get_display_name();
		$payment_method->type                 = 'Stripe';
		$payment_method->gateway_payment_id   = $token->get_token();
		$payment_method->gateway_payment_type = 32;
		$payment_method->gateway_customer_id  = $token->get_customer_id();

		if ( method_exists( $token, 'get_last4' ) && method_exists( $token, 'get_expiry_month' ) && method_exists( $token, 'get_expiry_year' ) ) {
			$payment_method->last_four  = $token->get_last4();
			$payment_method->expiration = $token->get_expiry_month() . substr( $token->get_expiry_year(), -2 );
		}
	}
...
/**
 * Deletes the Airwallex payment method from QPilot for a customer
 * Hooked into the new {@see autoship_delete_{$type}_payment_method_qpilot_match} filter.
 *
 * @param bool                $valid Current match boolean.
 * @param string              $type The QPilot Method Type.
 ...
 * @return bool True it's a match otherwise not.
 */
function autoship_delete_airwallex_payment_method( $valid, $type, $token, $method ) {
	if ( 'Airwallex' === $type ) {
 ...
 * Hooked into the new {@see autoship_add_{$type}_payment_method} filter.
 */
function autoship_add_payav1_payment_method( $payment_method_data, $type, $token ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * Hooked into the new {@see autoship_add_{$type}_payment_method} filter.
 */
function autoship_add_nmi_payment_method( $payment_method_data, $type, $token ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @return array The modified Payment Method Data to Send to QPilot
 */
function autoship_add_cybersource_payment_method( $payment_method_data, $type, $token ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @return array The modified Payment Method Data to Send to QPilot
 */
function autoship_add_trustcommerce_payment_method( $payment_method_data, $type, $token ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @return array The modified Payment Method Data to Send to QPilot
 */
function autoship_add_sagepaydirect_payment_method( $payment_method_data, $type, $token ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @return array The modified Payment Method Data to Send to QPilot
 */
function autoship_add_checkout_payment_method( $payment_method_data, $type, $token ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
...
/**
 * Modifies the Payment Info for a Customer before
 * For Airwallex payment methods added to QPilot.
 * Hooked into the new {@see autoship_add_{$type}_payment_method} filter.
 *
 * @param array               $payment_method_data Current payment Method Data.
 * @param string              $type The QPilot Method Type.
 ...
 * @return array The modified Payment Method Data to Send to QPilot
 */
function autoship_add_airwallex_payment_method( $payment_method_data, $type, $token ) {
	$customer_id = autoship_get_airwallex_customer_id( $token->get_user_id() );
...
function autoship_delete_payment_method( $method_id ) {
	try {
		...
		$service   = $container->get( PaymentMethodService::class );

		return $service->delete( $method_id );
	} catch ( Exception $e ) {
		// Fallback: service container unavailable, use legacy logic.
		$client = autoship_get_default_client();

		try {
			$client->delete_payment_method( $method_id );
		} catch ( Exception $ex ) {
			$notice = autoship_expand_http_code( $ex->getCode() );
			autoship_log_entry( __( 'Autoship Payment Methods', 'autoship' ), sprintf( 'Error deleting QPilot Payment Method. Additional Details: %s - %s', $ex->getCode(), $ex->getMessage() ) );
 ...
 *
 * @param array $payment_method_data Array of Payment Method Information.
 *
...
function autoship_add_payment_method( $payment_method_data ) {
	try {
		...
		return $service->upsert( $payment_method_data );
	} catch ( Exception $e ) {
		// Fallback: service container unavailable, use legacy logic.
		$client = autoship_get_default_client();
		...
		try {
			...
			$notice = autoship_expand_http_code( $ex->getCode() );
			autoship_log_entry( __( 'Autoship Payment Methods', 'autoship' ), sprintf( 'Error creating QPilot Payment Method. Additional Details: %s - %s', $ex->getCode(), $ex->getMessage() ) );
...
function autoship_update_payment_method( $payment_method_data ) {
	...
	try {
		...
		return new WP_Error( 'Updating QPilot Payment Method Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	}
...
function autoship_standard_gateway_id_types() {
	...
	return apply_filters(
		'autoship_extend_gateway_id_types',
		array(
			'trustcommerce'                       => 'TrustCommerce',
 ...
 * @return array The filtered types
 */
function autoship_add_paypal_v3_token_support( $types ) {
...
/**
 * Deletes the PayPal V3 payment method from QPilot for a customer
 * Hooked into the new {@see autoship_delete_{$type}_payment_method_qpilot_match} filter.
 *
 * @param bool                $valid Current match boolean.
 * @param string              $type The QPilot Method Type.
 ...
 * @return bool True it's a match otherwise not.
 */
function autoship_delete_paypal_v3_payment_method( $valid, $type, $token, $method ) {
	if ( 'PayPalV3' == $type ) { //phpcs:ignore
		...
		return ( $method->gatewayCustomerId == $gateway_customer_id && $method->gatewayPaymentId == $token->get_token() ); // phpcs:ignore
	}
...
function autoship_add_tokenized_payment_method( $token_id ) {
	try {
		...
		return $service->add_tokenized( $token_id );
	} catch ( Exception $e ) {
		// Fallback: service container unavailable, use legacy logic.
		$token = WC_Payment_Tokens::get( $token_id );
...
function autoship_delete_tokenized_payment_method( $token_id, $token ) {
	try {
		...
		return $service->delete_tokenized( $token_id, $token );
	} catch ( Exception $e ) {
		// Fallback: service container unavailable, use legacy logic.
...
function autoship_add_non_wc_token_payment_method( $response, $order, $autoship_method_type ) {
	try {
		...
		return $service->add_from_gateway_response( $response, $order, $autoship_method_type );
	} catch ( Exception $e ) {
		// Fallback: service container unavailable, use legacy logic.
...
function autoship_delete_non_wc_token_payment_method( $gateway_payment_method_id, $gateway_customer_id, $type, $wc_customer_id = null ) {
	try {
		...
		return $service->delete_non_wc_token( $gateway_payment_method_id, $gateway_customer_id, $type, $wc_customer_id );
	} catch ( Exception $e ) {
		// Fallback: service container unavailable, use legacy logic.
		...
		foreach ( $payment_methods as $method ) {
			$valid = ! empty( $gateway_customer_id ) ? ( $method->type === $type && $method->gatewayPaymentId === $gateway_customer_id && $method->gatewayPaymentId === $gateway_payment_method_id ) : ( $method->type === $type && $method->gatewayPaymentId === $gateway_payment_method_id ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
 ...
 *
 * @param WC_Payment_Token_CC $token The payment token object.
 * @param bool                $return_or_added Should the payment method data be returned or added. True to return, false to add to QPilot.
 *
 * @return stdClass|WP_Error|array The resulting payment method object, array of payment data,
 *                                 or WP_Error on failure.
 */
function autoship_add_general_payment_method( $token, $return_or_added = false ) {
	...
	try {
		...
		return $service->add_from_token( $token, $return_or_added );
	} catch ( Exception $e ) {
		// Fallback: service container unavailable, use legacy logic.
		...
		if ( method_exists( $token, 'get_last4' ) && is_callable( array( $token, 'get_last4' ) ) && 'ppcp-gateway' !== $gateway ) {
			$last_four_digits = $token->get_last4();
		}
		...
		if ( 'PayPalV3' == $type ) { //phpcs:ignore
			if ( 'ppcp-credit-card-gateway' == $gateway ) { //phpcs:ignore
				$data['gatewayPaymentType'] = 26;
			}

			if ( 'ppcp-gateway' == $gateway ) { //phpcs:ignore
		...
		if ( is_a( $token, 'WC_Payment_Token_Link' ) && 'Stripe' == $type && 'stripe' == $gateway ) { //phpcs:ignore
		...
		if ( Logger::is_tracing_enabled() ) {
			$export_payment_data = var_export( $payment_method_data, true ); // phpcs:ignore
		...
		return $return_or_added ? $payment_method_data : autoship_add_payment_method( $payment_method_data );
	}
}
 ...
 * @param WC_Payment_Token_CC $token The token to delete.
...
function autoship_delete_general_payment_method( $token_id, $token, $type = '' ) {
	try {
		...
		return $service->delete_for_token( $token_id, $token, $type );
	} catch ( Exception $e ) {
		// Fallback: service container unavailable, use legacy logic.
		$wc_customer_id = $token->get_user_id();
		...
		$payment_method_id = $token->get_token();
		$payment_methods   = $customer->paymentMethods; // phpcs:ignore
...
function autoship_tokenize_non_standard_methods( $token, $autoship_method_id, $autoship_method_type ) {
	...
	if ( 'braintree_paypal' != $autoship_method_type ) { //phpcs:ignore
...
function autoship_tokenize_non_fully_implemented_token_classes( $token ) {
	...
	if ( empty( $token ) || 'braintree_paypal' != $token->get_gateway_id() ) { //phpcs:ignore
		return $token;
	}
...
function autoship_update_payment_method_on_all_scheduled_orders() {
	...
	if ( ! empty( $autoship_method_id ) && ! empty( $autoship_method_type ) ) {
		...
		if ( empty( $type ) || is_null( $token ) || get_current_user_id() !== $token->get_user_id() || false === wp_verify_nonce( $_REQUEST['_wpnonce'], 'autoship-apply-payment-method-to-orders-' . $autoship_method_id ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

			$message = __( 'Invalid payment method.', 'autoship' );
			$notice  = apply_filters(
				'autoship_update_payment_method_on_all_scheduled_orders_failure_invalid_notice',
				array(
					'message' => $message,
					'code'    => 'error',
				),
		...
		} elseif ( ! autoship_rights_checker( 'autoship_update_payment_info_on_all_scheduled_orders', array() ) ) {

			$message = __( 'You have insufficient rights to make this change.', 'autoship' );
			$notice  = apply_filters(
				'autoship_update_payment_method_on_all_scheduled_orders_failure_rights_notice',
				array(
					'message' => $message,
					'code'    => 'error',
				),
			...
			if ( is_wp_error( $result ) ) {
				// translators: %1$s is the label for Scheduled Orders, %2$s is the error message.
				$message = sprintf( __( 'There was a problem updating the information on your %1$s. Additional details: %2$s', 'autoship' ), $schedule_label, $result->get_error_message() );
				$notice  = apply_filters(
					'autoship_update_payment_method_on_all_scheduled_orders_failure_notice',
					array(
						'message' => $message,
						'code'    => 'error',
					),
				...
				// translators: %s is the label for Scheduled Orders.
				$message = sprintf( __( 'Your %s Have been successfully updated.', 'autoship' ), $schedule_label );
				$notice  = apply_filters(
					'autoship_update_payment_method_on_all_scheduled_orders_updated_notice',
					array(
						'message' => $message,
						'code'    => 'success',
					),
		...
		wp_redirect( wc_get_account_endpoint_url( 'payment-methods' ) ); // phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
		exit();
	}
}
...
function autoship_after_save_payment_method_autoship_action_notice( $token_id, $gateway_id = 'standard', $name = '' ) {
	...
	// translators: %1$s is the label for Scheduled Orders, %2$s is the URL to update the payment method, %3$s is the Update button text.
	$message = empty( $name ) ? sprintf( __( 'Use the newly Added Payment Method for your <strong>%1$s</strong> as well? <a href="%2$s" tabindex="1" class="button">%3$s</a>', 'autoship' ), $schedule_label, esc_url( $url ), esc_html__( 'Update', 'autoship' ) ) : sprintf( __( 'Use the new Payment Method ( %1$s ) for your <strong>%2$s</strong> as well? <a href="%3$s" tabindex="1" class="button">%4$s</a>', 'autoship' ), $name, $schedule_label, esc_url( $url ), esc_html__( 'Update', 'autoship' ) );
	$notice  = apply_filters(
		'autoship_apply_payment_method_to_all_scheduled_orders_default_action_notice',
		array(
			'message' => $message,
			'code'    => 'success',
		),
 ...
 * @param \SV_WC_Payment_Gateway_API_Response $response API response object.
 */
function autoship_after_save_authorize_net_payment_method_notice( $token_id, $user_id, $response ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param \SV_WC_Payment_Gateway_API_Response $response API response object.
 */
function autoship_after_save_braintree_credit_card_payment_method_notice( $token_id, $user_id, $response ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param \SV_WC_Payment_Gateway_API_Response $response API response object.
 */
function autoship_after_save_square_credit_card_payment_method_notice( $token_id, $user_id, $response ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 */
function autoship_display_apply_payment_method_to_all_scheduled_orders_note( $has_methods ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
	...
	echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		'autoship_display_apply_method_to_all_scheduled_orders_note',
		sprintf(
			// translators: %s is the label for Scheduled Orders.
			__( '<p><strong><i class="autoship_update_all_orders"></i></strong> Select to apply the payment method to your %s.</p>', 'autoship' ),
			$schedule_label
		)
	);
}
...
/**
 * Non-Standard skyverge framework Gateways actions. Outputs the apply action button after each payment method.
 *
 ...
 * @param string                                    $gateway The gateway id for the payment method.
 *
 * @type string $url action URL
 * @type string $class action button class
 * @type string $name action button name
 */
function autoship_display_apply_payment_method_to_all_scheduled_orders_skyverge_btn( $list_item, $payment_token, $instance, $gateway ) {
	// Check if user has any SO's and skip if not.
	// Only display the notice if the user has scheduled orders.
	if ( ! apply_filters( 'autoship_display_customer_update_actions', true, get_current_user_id() ) ) {
		return $list_item;
	...
	$list_item['autoship_update_all_orders'] = apply_filters(
		'autoship_apply_payment_method_to_all_scheduled_orders_row_action',
		array(
			'url'   => $apply_to_all_orders_url,
			'name'  => apply_filters( 'autoship_update_orders_icon_action', '' ),
			'class' => 'autoship_update_all_orders',
		),
		$apply_to_all_orders_url,
		$payment_token,
		$list_item
	);

	return apply_filters( 'autoship_apply_payment_method_to_all_scheduled_orders_non_standard_row', $list_item, $payment_token, $instance );
}
 ...
 * @param array                                    $list_item The list item array.
 * @param \SV_WC_Payment_Gateway_Payment_Token     $payment_token The payment token object.
 * @param \SV_WC_Payment_Gateway_My_Payment_Method $instance The instance of the My Payment Methods class.
 */
function autoship_display_apply_payment_method_to_all_scheduled_orders_braintree_btn( $list_item, $payment_token, $instance ) {
	$gateway = $payment_token->is_paypal_account() ? 'braintree_paypal' : 'braintree_credit_card';

	return autoship_display_apply_payment_method_to_all_scheduled_orders_skyverge_btn( $list_item, $payment_token, $instance, $gateway );
}
 ...
 * @param \SV_WC_Payment_Gateway_My_Payment_Methods $instance The instance of the My Payment Methods class.
 */
function autoship_display_apply_payment_method_to_all_scheduled_orders_authorize_btn( $list_item, $payment_token, $instance ) {
	return autoship_display_apply_payment_method_to_all_scheduled_orders_skyverge_btn( $list_item, $payment_token, $instance, 'authorize_net_cim_credit_card' );
}
 ...
 * @param \SV_WC_Payment_Gateway_My_Payment_Methods $instance The instance of the My Payment Methods class.
 */
function autoship_display_apply_payment_method_to_all_scheduled_orders_cybersource_cc_btn( $list_item, $payment_token, $instance ) {
	return autoship_display_apply_payment_method_to_all_scheduled_orders_skyverge_btn( $list_item, $payment_token, $instance, 'cybersource_credit_card' );
}
 ...
 * @param \SV_WC_Payment_Gateway_My_Payment_Methods $instance The instance of the My Payment Methods class.
 */
function autoship_display_apply_payment_method_to_all_scheduled_orders_square_btn( $list_item, $payment_token, $instance ) {
...
function autoship_display_apply_payment_method_to_all_scheduled_orders_btn( $list_item, $payment_token ) {
	...
	$list_item['actions']['autoship_update_all_orders'] = apply_filters(
		'autoship_apply_payment_method_to_all_scheduled_orders_row_action',
		array(
			'url'  => $apply_to_all_orders_url,
			'name' => apply_filters( 'autoship_update_orders_icon_action', '' ),
 ...
 */
function autoship_add_adjusted_gateway_metadata_ppec_paypal( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed

	// Set the status for the transaction.
	$status = 'Completed' == $payment_meta['PAYMENTSTATUS'] ? $payment_meta['PAYMENTSTATUS'] : $payment_meta['PAYMENTSTATUS'] . '_' . $payment_meta['PENDINGREASON']; //phpcs:ignore
...
function autoship_add_adjusted_gateway_metadata_ppep_paypal( $payment_meta, $order, $payment_method ) {
	 ...
	 */
	if ( isset( $payment_meta['links'] ) && ! empty( $payment_meta['links'] ) && ( strpos( $payment_meta['links'][0]['href'], 'https://api.sandbox.paypal.com' ) != false ) ) { //phpcs:ignore
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_stripe( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_fees_stripe( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_square( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	// Set the status for the transaction.
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_nmi_gateway_woocommerce_credit_card( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_braintree_credit_card( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_braintree_paypal( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
...
function autoship_add_adjusted_gateway_metadata_braintree( $payment_meta, $order, $payment_method ) {
	...
	$is_paypal = apply_filters( 'autoship_is_paypal_payment_method_scheduled_order', false != strpos( $payment_method['description'], $paypal_flag ), $payment_meta, $order, $payment_method ); //phpcs:ignore
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_sagepaymentsusaapi( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_trustcommerce( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_sage( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param array    $payment_method The associated payment method data.
 */
function autoship_add_adjusted_gateway_metadata_checkout( $payment_meta, $order, $payment_method ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
...
function autoship_adjust_payment_gateway_option_description( $settings ) {
	...
	// translators: %1$s is the URL to the Autoship Plugin, %2$s is the URL to the Autoship Support Article.
	$settings[0]['desc'] .= sprintf( __( '<br><hr><img class="autoship-payment-icon" width="24" height="24" src="%1$s"/><span class="autoship-payment-description">Autoship Cloud Supported Payment Gateways. <a href="%2$s">Learn More</a>' ), Autoship_Plugin_Url . '/images/scheduled_order.svg', 'https://support.autoship.cloud/article/1002-payment-integrations' );
...
/**
 * Adds Support for the PayPal Payments Version
 *
 * @see autoship_add_paypal_v3_token_support()
 */
add_filter( 'autoship_extend_gateway_id_types', 'autoship_add_paypal_v3_token_support', 12, 1 );
...
add_filter( 'autoship_delete_PayPalV3_payment_method_qpilot_match', 'autoship_delete_paypal_v3_payment_method', 10, 4 );
add_filter( 'autoship_add_Airwallex_payment_method', 'autoship_add_airwallex_payment_method', 10, 3 );
add_filter( 'autoship_delete_Airwallex_payment_method_qpilot_match', 'autoship_delete_airwallex_payment_method', 10, 4 );
 ...
 */
add_filter( 'wc_stripe_generate_create_intent_request', 'autoship_stripe_intent_request_filter', 30, 2 );
...
/**
 * Appends Mandate data to order metadata when Link by Stripe payment method used
 *
 * @param array $order_data The array of order data to use for creating the scheduled order.
 * @param int   $order_id The WC Order ID.
 *
 * @return $order_data The array of order data to use for creating the scheduled order.
 */
function autoship_stripe_link_order_mandate_data( $order_data, $order_id ) {
...
add_filter( 'autoship_create_scheduled_order_data', 'autoship_stripe_link_order_mandate_data', 10, 2 );

/**
 * Retrieve the first Airwallex customer ID associated with a user.
 *
 * @param int $user_id The ID of the user whose metadata to search.
 *
 * @return mixed|null The value of the first matching Airwallex customer ID, or null if none found.
 */
function autoship_get_airwallex_customer_id( $user_id ) {
	global $wpdb;

	$meta_key_prefix = 'airwallex_customer_id_';

	// Find one matching key.
	...
	return $result ? maybe_unserialize( $result['meta_value'] ) : null;
}
