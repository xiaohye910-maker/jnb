 ...
 * @param WC_Product_Data_Store_CPT $data_store_cpt The current Data Store Object.
 *
 * @return array The filtered query arguments
 */
function autoship_handling_meta_query_keys( $wp_query_args, $query_vars, $data_store_cpt ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	$meta_key = '_autoship_schedule_order_enabled';
...
function autoship_get_product_display_name( $product ) {
	...
	if ( $product->is_type( 'variation' ) ) {
		...
		if ( version_compare( WC()->version, '3.0', '<' ) ) {
			// WC 2.6.
			$display_name .= ' (' . trim( wp_strip_all_tags( str_replace( '><', ' > <', str_replace( '</dd><dt>', ', ', $product->get_formatted_variation_attributes() ) ) ) ) . ')';
		} else {
			// WC 3.0.
 ...
 */
function autoship_handle_custom_sync_active_meta_query( $wp_query_args, $query_vars, $data_store_cpt ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @return array modified $query
 */
function autoship_handle_dissable_schedule_order_options_meta_query( $wp_query_args, $query_vars, $data_store_cpt ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
...
function autoship_get_mapped_stocklevel( $stock_status ) {
	...
	$translation = apply_filters(
		'autoship_get_mapped_stocklevel',
		array(
...
function autoship_valid_product_sync_statuses() {
	return apply_filters(
		'autoship_valid_product_statuses',
		array(
			'publish',
		)
	);
...
function autoship_is_valid_sync_type_product( $product ) {
	...
	return in_array( $product_type, $valid_sync_types, true );
}
...
function autoship_is_valid_sync_status_product( $product ) {
	...
	return in_array( $product_status, $valid_sync_statuses, true );
}
...
function autoship_is_valid_sync_post_type( $product ) {
	...
	return in_array( get_post_type( $product->get_id() ), $valid_sync_post_types, true );
}
...
function autoship_is_valid_sync_product( $product ) {
	...
	if ( ! in_array( get_post_type( $product->get_id() ), $valid_sync_post_types, true ) ) {
		return false;
	}
	...
	return ( in_array( $product_type, $valid_sync_types, true ) && in_array( $product_status, $valid_sync_statuses, true ) );
}

/**
 * Checks if a Variation is Available.
 *
 * @param WC_Product_Variation|int $variation The WC Product Variation to check.
 *
 * @return bool True if valid else false.
 ...
 * @param bool                    $include_invalids When true an array is returned with both valid and invalid ids.
 *
 ...
 *
 * @param int $product The product or variation id.
 *
...
function autoship_get_product_custom_data_values( $product ) {
	...
	return apply_filters( 'autoship-product-custom-data-values', $data, $product->get_id() ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
...
function autoship_is_valid_product_frequency_option( $product_id, $frequency, $frequency_type ) {
	...
	if ( ! empty( $options ) ) {
		...
		foreach ( $options as $key => $values ) {
			...
			if ( $frequency_type == $values['frequency_type'] && $frequency == $values['frequency'] ) { //phpcs:ignore
				return true;
			}
		}
	}
...
function autoship_get_product_custom_frequency_options( $product ) {
	...
	return apply_filters( 'autoship-product-frequency-options', $options, $product->get_id() ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
...
function autoship_product_frequency_options( $product_id ) {
	...
	return ! empty( $options ) ? $options : autoship_default_frequency_options();
}
...
function autoship_default_frequency_options() {
	...
	return apply_filters( 'autoship-default-frequency-options', $options ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 *
 * @param int $wc_product_id The product or variation id.
 *
...
function autoship_ajax_product_discount() {
	$product_id = $_POST['product_id']; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	$product    = wc_get_product( $product_id );
...
function autoship_checkout_price( $product, $prices = array() ) {
	...
	$autoship_price = empty( $prices['discount'] ) ? $prices['price'] : $prices['discount'];
...
function autoship_get_formatted_price( $price, $args = array() ) {
	...
	$formatted_price = ! empty( $args['original'] ) && ( $args['original'] !== $price ) ? wc_format_sale_price( $args['original'], $price ) . $args['suffix'] : wc_price( $price, $args ) . $args['suffix'];
...
function autoship_percent_recurring_discount( $product, $prices = array() ) {
    ...
    // Added guard condition to prevent division by zero (PHP 8+ throws DivisionByZeroError).
    if ( empty( $prices['price'] ) || floatval( $prices['price'] ) == 0 ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
        return 0;
    }
 ...
 *
 * @param int   $variation_id To retrieve the values for.
 * @param array $fields Optional.  A specific field or fields to retrieve. Defaults to all fields.  Takes a comma separated string or array.
 *
...
function autoship_get_variable_product_custom_field_values( $variation_id, $fields = array() ) {
	...
	foreach ( $autoship_field_names as $name => $settings ) {
		...
		$variation_data[ $name ] = $settings['inherit'] ? get_post_meta( $variation->get_parent_id(), $name, true ) : get_post_meta( $variation_id, $name, true );
	}
...
function autoship_get_custom_metafield_names( $set = '' ) {
	...
	$core_fields = apply_filters(
		'autoship_custom_core_metafields',
		array(
			'_autoship_checkout_price'                   => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_recurring_price'                  => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_sync_active_enabled'              => array(
				'default' => '',
				'inherit' => true,
			),
			'_autoship_schedule_order_enabled'           => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_schedule_process_enabled'         => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_group_ids'                        => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_override_product_data'            => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_title_override'                   => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_weightunit_override'              => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_lengthunit_override'              => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_schedule_options_enabled'         => array(
				'default' => '',
				'inherit' => true,
			),
			'_autoship_override_frequency_options'       => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_dissable_schedule_order_options'  => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_relative_next_occurrence_enabled' => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_relative_next_occurrence_type'    => array(
				'default' => '',
				'inherit' => false,
			),
			'_autoship_relative_next_occurrence'         => array(
				'default' => '',
				'inherit' => false,
			),
		)
	...
	for ( $i = 0; $i < $autoship_frequency_options; ++$i ) {
		...
		$frequency_overrides[ "_autoship_frequency_type_{$i}" ]         = array(
			'default' => '',
			'inherit' => false,
		);
		$frequency_overrides[ "_autoship_frequency_{$i}" ]              = array(
			'default' => '',
			'inherit' => false,
		);
		$frequency_overrides[ "_autoship_frequency_display_name_{$i}" ] = array(
			'default' => '',
			'inherit' => false,
		);
 ...
 * @param bool           $is_cart Optional. If true, the data is for the cart page.
 *
 * @return array         The product pricing and label info.
 */
...
function autoship_schedule_order_enabled( $product ) {
	...
	$val = apply_filters( 'autoship_override_schedule_order_enabled_default', get_post_meta( $id, '_autoship_schedule_order_enabled', true ), $id );
...
function autoship_schedule_process_enabled( $product ) {
	...
	$val = apply_filters( 'autoship_override_schedule_process_enabled_default', get_post_meta( $id, '_autoship_schedule_process_enabled', true ), $id );
 ...
 *
 * @return string no or yes value.
 * @see autoship_override_schedule_options_enabled_default
 */
function autoship_schedule_options_enabled( $product ) {

	$id = is_numeric( $product ) ? $product : $product->get_id();

	$val = apply_filters( 'autoship_override_schedule_options_enabled_default', get_post_meta( $id, '_autoship_schedule_options_enabled', true ), $id );
...
function autoship_override_frequency_options_enabled( $product ) {
	...
	$val = apply_filters( 'autoship_override_simple_frequency_options_default', get_post_meta( $id, '_autoship_override_frequency_options', true ), $id );
...
function autoship_disable_schedule_order_options( $product ) {
	...
	// Show or hide Frequency Overrides based on checkbox value.
	$val = apply_filters( 'autoship_override_dissable_schedule_order_options_default', get_post_meta( $id, '_autoship_dissable_schedule_order_options', true ), $id );
...
function autoship_is_visible_active_shop_product( $product ) {
	...
	return apply_filters( 'autoship_is_visible_active_shop_product', 'yes' === autoship_schedule_options_enabled( $product ) && 'yes' === autoship_sync_active_enabled( $product ), $product );
}
...
function autoship_product_message_string( $product_id ) {
	return apply_filters( 'autoship_product_message_string', ' ' . get_option( 'autoship_product_message' ), $product_id );
}
...
function autoship_get_product_default_relative_next_occurrence( $product_id, $frequency_type = '', $frequency = 0 ) {
	...
	if ( 'yes' === autoship_relative_next_occurrence_enabled( $product_id ) ) {
		...
		$date = autoship_calculate_relative_next_occurrence_date( $relative_nod, $relative_nod_type, apply_filters( 'autoship_calculate_relative_next_occurrence_date_from_basedate', new DateTime(), $relative_nod, $relative_nod_type, $product_id, $frequency_type, $frequency ) );
	}
 ...
 *
 * @param array $args Additional Query Params to narrow down search.
 *
 * @return array variation products.
 */
function autoship_get_available_schedulable_variations( $args = array() ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found

	$products_args = apply_filters(
		'autoship_filter_schedulable_variation_products_query_args',
		array(
			'status'                                    => 'publish',
			'limit'                                     => '-1',
	...
	// Fix to bug when no purchasable products are found and null returned.
	$all_products = array();
 ...
 * @param int   $index Optional. The page index to return.
 *
...
function autoship_search_available_products( $params = array(), $index = 1 ) {
	...
	try {
		...
		$notice = new WP_Error( 'Product Search Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	if ( $products->totalPages > $index ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		++$index;
		$params['page'] = $index;
...
function autoship_get_products_by_ids( $product_ids, $batch_size = null, $args = array() ) {
	...
	$batch_size = ! isset( $batch_size ) || ! $batch_size ? apply_filters( 'autoship_get_products_by_ids_default_batch_size', 5 ) : $batch_size;
	...
	if ( count( $product_ids ) > $batch_size ) {
		...
		try {
			...
			$notice = autoship_expand_http_code( $e->getCode() );
			$notice = new WP_Error( 'Product Search by ID Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_get_available_product( $product_id ) {
	...
	try {
		...
		$notice = autoship_expand_http_code( $e->getCode() );
		$notice = new WP_Error( 'Product Retrieval Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_get_remote_products( $product_ids = array() ) {
	$search_params = apply_filters(
		'autoship_get_remote_products_search_params',
		array(
			'availability' => 1,
			'active'       => 'true',
		)
...
function autoship_get_remote_product( $product_id ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			// translators: %d is the QPilot Site ID.
			...
			return new WP_Error( 'Product Retrieval Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
		}
	}
...
function autoship_productize_and_validate( $product ) {
	...
	$valid_variation = 'variation' === $product->get_type() ? autoship_is_available_variation( $product ) : true;
...
function autoship_init_variation_metafields( $the_variation ) {
	...
	$variation = autoship_productize_and_validate( $the_variation );
...
function autoship_generate_product_upsert_data( $product, $product_data = array() ) {
	...
	$wc_product = autoship_productize_and_validate( $product );
	...
	$group_ids     = autoship_get_assigned_group_ids( $wc_product );
...
function autoship_save_product_custom_fields( $id, $index = null ) {
	...
	foreach ( $autoship_field_names as $name => $settings ) {
		...
		if ( null === $index ) {
			$value = isset( $_POST[ $name ] ) ? sanitize_text_field( $_POST[ $name ] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
		} else {
			$value = isset( $_POST[ $name ] ) && isset( $_POST[ $name ][ $index ] ) ? sanitize_text_field( $_POST[ $name ][ $index ] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
		}
		...
		if ( in_array( $name, array( '_autoship_sync_active_enabled' ), true ) ) {
 ...
 */
function autoship_format_autoship_group_ids_on_save( $value, $product_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 */
function autoship_format_autoship_prices_on_save( $value, $product_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 *
 * @param array $autoship_field_names The array of meta fields.
 *
...
function autoship_attach_product_shipperhq_metadata( $metadata, $wc_product ) {
	...
	$shipperhq_fields = apply_filters(
		'autoship_shipperhq_product_fields',
		array(
 ...
 * @param WC_Data          $item Object data.
 * @param WP_REST_Request  $request Request object.
 */
function autoship_adjust_rest_product_data( $response, $item, $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	// Retrieve the Custom Data.
	$data                   = autoship_get_product_custom_data_values( $item );
...
function autoship_get_all_products_sync_summary( $ids, $wc_products = array() ) {
	...
	$client = autoship_get_default_client();
	if ( empty( $client ) || empty( $client->get_token_auth() ) ) {
	...
	foreach ( $summaries as $key => $summary ) {
		...
		$wc_product = empty( $wc_products ) || ! isset( $wc_products[ $summary->id ] ) ? wc_get_product( $summary->id ) : $wc_products[ $summary->id ];
		...
		$availability = 'None';
		if ( $summary->addToScheduledOrder && $summary->processScheduledOrder ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			$availability = 'AddToScheduledOrder,ProcessScheduledOrder';
		} elseif ( $summary->addToScheduledOrder ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			$availability = 'AddToScheduledOrder';
		} elseif ( $summary->processScheduledOrder ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			$availability = 'ProcessScheduledOrder';
		}

		$products[ $summary->id ] = apply_filters(
			'autoship_all_products_summary_data_row',
			array(
				'Id'                           => $summary->id,
				'ProductName'                  => $summary->productName, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'AddToScheduledOrder'          => false !== $summary->addToScheduledOrder,  // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'ProcessScheduledOrder'        => false !== $summary->processScheduledOrder, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'Availability'                 => $availability,
				'ProductType'                  => $summary->productType, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'StockLevel'                   => $summary->availability,
				'TotalStock'                   => $summary->stock,
				'QuantityScheduled'            => $summary->totalQuantityScheduledActive + $summary->totalQuantityScheduledPaused + $summary->totalQuantityFailed + $summary->totalQuantityProcessing + $summary->totalQuantityQueued, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'TotalQuantityScheduledActive' => $summary->totalQuantityScheduledActive, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'TotalQuantityScheduledPaused' => $summary->totalQuantityScheduledPaused, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'TotalQuantityFailed'          => $summary->totalQuantityFailed, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'TotalQuantityProcessing'      => $summary->totalQuantityProcessing, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'TotalQuantityQueued'          => $summary->totalQuantityQueued, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'LifetimeValue'                => $summary->lifetimeValue, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'ShippingClass'                => $summary->shippingClass, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'Active'                       => $summary->active,
				'Valid'                        => $summary->valid,
				'ValidationErrorCode'          => $summary->validationErrorCode, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'UpdatedUtc'                   => $summary->updatedUtc, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'SyncError'                    => '',
			),
			$summary
		);
...
function autoship_get_product_sync_summary( $id ) {
	...
	$product = autoship_productize_and_validate( $id );
	if ( false === $product ) {
	...
	if ( 'variation' === $product->get_type() || 'simple' === $product->get_type() ) {
		...
		if ( $summaries[0]->addToScheduledOrder && $summaries[0]->processScheduledOrder ) {
			$availability = 'AddToScheduledOrder,ProcessScheduledOrder';
		} elseif ( $summaries[0]->addToScheduledOrder ) {
			$availability = 'AddToScheduledOrder';
		} elseif ( $summaries[0]->processScheduledOrder ) {
			$availability = 'ProcessScheduledOrder';
		}

		$totals = apply_filters(
			'autoship_product_summary_data',
			array(
				'Id'                           => $summaries[0]->id,
				'ProductName'                  => $summaries[0]->productName,
				'AddToScheduledOrder'          => false !== $summaries[0]->addToScheduledOrder,
				'ProcessScheduledOrder'        => false !== $summaries[0]->processScheduledOrder,
				'Availability'                 => $availability,
				'ProductType'                  => $summaries[0]->productType,
				'StockLevel'                   => $summaries[0]->availability,
				'TotalStock'                   => $summaries[0]->stock,
				'QuantityScheduled'            => $summaries[0]->totalQuantityScheduledActive + $summaries[0]->totalQuantityScheduledPaused + $summaries[0]->totalQuantityFailed + $summaries[0]->totalQuantityProcessing + $summaries[0]->totalQuantityQueued,
		...
		foreach ( $summaries as $key => $summary ) {
			...
			$variations[ $summary->id ] = apply_filters(
				'autoship_variable_product_summary_data',
				array(
					'Id'                           => $summary->id,
					'ProductName'                  => $summary->productName, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					'AddToScheduledOrder'          => false !== $summary->addToScheduledOrder, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			...
			if ( 'variation' === $summary->productType ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				...
				$totals['TotalVariations']              += (int) ( 'variation' === $summary->productType ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
 ...
 */
function autoship_adjust_sync_error_to_warning( $is_warning, $error, $product ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 */
function autoship_product_summary_updatedutc_sync_check( $sync_error, $data, $record, $product ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
...
function autoship_product_summary_stocklevel_sync_check( $sync_error, $data, $record, $product ) {
	...
	return $stock_status === $data['StockLevel'] ?
		$sync_error :
		new WP_Error(
			'stock-level-out-of-sync',
			sprintf(
				'variation' === $product->get_type() ?
					// translators: %1$s is the API Health URL, %2$s is the Products Report URL.
					__( 'Product not synchronized: Unable to synchronize Stock Status for one or more variations.<br/>Please confirm your <a href="%1$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%2$s">Autoship Cloud > Products</a> report.', 'autoship' ) :
					// translators: %1$s is the API Health URL, %2$s is the Products Report URL.
					__( 'Product not synchronized: Unable to synchronize Stock Level.<br/>Please confirm your <a href="%1$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%2$s">Autoship Cloud > Products</a> report.', 'autoship' ),
				$api_health_url,
				$products_report_url
			)
		);
 ...
 * @return WP_Error|string The filtered Sync Error.
 */
function autoship_product_summary_active_sync_check( $sync_error, $data, $record, $product ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	if ( ! $data['Active'] && $data['AutoshipActive'] ) {
		// translators: %1$s is the API Health URL, %2$s is the Products Report URL.
		$sync_error = new WP_Error( 'active-flag-out-of-sync', sprintf( __( 'Product not synchronized: Unable to activate this product in QPilot. <br/>Please confirm your <a href="%1$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%2$s">Autoship Cloud > Products</a> report.', 'autoship' ), $api_health_url, $products_report_url ) );
	} elseif ( $data['Active'] && ! $data['AutoshipActive'] ) {
		// translators: %1$s is the API Health URL, %2$s is the Products Report URL.
		$sync_error = new WP_Error( 'active-flag-out-of-sync', sprintf( __( 'Product not synchronized: Invalid Product Data returned by QPilot. <br/>Please confirm your <a href="%1$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%2$s">Autoship Cloud > Products</a> report.', 'autoship' ), $api_health_url, $products_report_url ) );
	}
...
function autoship_product_summary_orphaned_invalid_variations_sync_check( $sync_error, $data, $record, $product, $ids ) {
	...
	if ( 'variable' === $product->get_type() && is_array( $ids ) && isset( $ids['invalids'] ) ) {
		...
		$sync_error = $data['Active'] && $data['Valid'] && in_array( $data['Id'], $ids['invalids'], true ) ?
			new WP_Error(
				'valid-out-of-sync',
				sprintf(
					// translators: %1$s is the API Health URL, %2$s is the Products Report URL.
					__( 'Product not synchronized: Product data not found.<br/>One or more variations are Active in QPilot but not found in WooCommerce. Please confirm your <a href="%1$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%2$s">Autoship Cloud > Products</a> report.', 'autoship' ),
					$api_health_url,
					$products_report_url
				)
			) :
		...
		$sync_error = $data['Active'] && $data['Valid'] && ! autoship_is_available_variation( $data['Id'] ) ?
			new WP_Error(
				'valid-out-of-sync',
				sprintf(
					// translators: %1$d is the variation id, %2$s is the API Health URL, %3$s is the Products Report URL.
					__( 'Product not synchronized: Product data not found.<br/>This variation #%1$d is Active in QPilot but not found in WooCommerce. Please confirm your <a href="%2$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%3$s">Autoship Cloud > Products</a> report.', 'autoship' ),
					$data['Id'],
					$api_health_url,
					$products_report_url
				)
...
function autoship_product_summary_active_invalid_sync_check( $sync_error, $data, $record, $product ) {
	...
	return ( $data['Active'] && $data['Valid'] ) || $data['Valid'] ?
		$sync_error :
		new WP_Error(
			'active-invalid-out-of-sync',
			'variable' === $product->get_type() ?
				sprintf(
					// translators: %1$s is the error message, %2$s is the API Health URL, %3$s is the Products Report URL.
					__( 'Product not synchronized: Invalid Product Data returned by QPilot for one or more variations.<br/> %1$s Please confirm your <a href="%2$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%3$s">Autoship Cloud > Products</a> report.', 'autoship' ),
					$error,
					$api_health_url,
					$products_report_url
				) :
				sprintf(
					// translators: %1$s is the error message, %2$s is the API Health URL, %3$s is the Products Report URL.
					__( 'Product not synchronized: Invalid Product Data returned by QPilot.<br/> %1$s Please confirm your <a href="%2$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%3$s">Autoship Cloud > Products</a> report.', 'autoship' ),
					$error,
					$api_health_url,
					$products_report_url
				)
...
function autoship_product_summary_orphaned_variations_sync_check( $totals, $product, $ids ) {
	...
	if ( $totals['TotalVariations'] < count( $ids['valids'] ) ) {
		...
		if ( ! empty( $missing_variations ) ) {
			// translators: %1$s is the API Health URL, %2$s is the Products Report URL.
			$totals['SyncError']      = new WP_Error( 'variations-out-of-sync', sprintf( __( 'Product not synchronized: Invalid Product Data returned by QPilot for one or more variations.<br/>Please confirm your <a href="%1$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%2$s">Autoship Cloud > Products</a> report.', 'autoship' ), $api_health_url, $products_report_url ) );
	...
	} elseif ( $totals['TotalAutoshipVariations'] > count( $ids['valids'] ) ) {
		...
		if ( ! empty( $missing_variations ) ) {
			// translators: %1$d is the number of missing variations, %2$s is the list of missing variations, %3$s is the API Health URL, %4$s is the Products Report URL.
			$totals['SyncError']      = new WP_Error( 'variations-out-of-sync', sprintf( __( 'Product not synchronized: %1$d Active variation(s) not found in QPilot: %2$s.<br/>Please confirm your <a href="%3$s">API connection is healthy</a> and ensure there are no issues with this product in the <a href="%4$s">Autoship Cloud > Products</a> report.', 'autoship' ), count( $missing_variations ), implode( ', ', $missing_variations ), $api_health_url, $products_report_url ) );
			$totals['AllSyncError'][] = $totals['SyncError'];
		}
	}
...
function autoship_sync_product( $id, $product = false ) {
	...
	if ( apply_filters( 'autoship_sync_product', ( 'yes' === autoship_sync_active_enabled( $id ) ) && autoship_is_valid_sync_product( $id ), $id ) && ! ( did_action( 'wp_ajax_woocommerce_save_variations' ) && doing_action( 'woocommerce_update_product_variation' ) ) ) {
		...
		$overrides = ( 'yes' === autoship_schedule_order_enabled( $id ) && 'yes' === autoship_schedule_process_enabled( $id ) ) ? array(
		...
		if ( is_wp_error( $result ) && ( doing_action( 'woocommerce_update_product' ) || doing_action( 'woocommerce_update_product_variation' ) || doing_action( 'woocommerce_save_product_variation' ) ) ) {

			// translators: %1$s is the error message, %2$s is the link to the Autoship Cloud Log File.
			$error = sprintf( __( 'Your last product update did not synchronize with your connected QPilot Site successfully. Please review the error before attempting to "Update & Sync" again. <br/><strong>Error Details:</strong> %1$s <br/><strong>Download the full Error Details by exporting your site\'s Autoship Cloud Log File <a href="%2$s">here</a>.</strong>' ), substr( $result->get_error_message(), 0, 280 ), admin_url( 'admin.php?page=autoship&tab=autoship-logs' ) );
 ...
 * @param string $new_instance The new value of the metadata key.
 * @param string $original The original value of the metadata key.
 *
 * @see autoship_push_product ()
 */
function autoship_sync_product_on_active_change( $id, $name, $new_instance, $original ) {

	// If the active flag is switched update it in metadata.
	if ( apply_filters( 'autoship_sync_product_on_active_change', $original !== $new_instance, $id, $new_instance, $original ) ) {
...
function autoship_push_sync_product_on_type_change( $product, $old_type, $new_type ) {
	...
	if ( ( $old_type !== $new_type ) && in_array( $old_type, autoship_valid_product_sync_types(), true ) && ! in_array( $new_type, autoship_valid_product_sync_types(), true ) ) {
		...
		if ( apply_filters( 'autoship_push_sync_product_on_type_change', 'yes' === autoship_sync_active_enabled( $product->get_id() ), $product->get_id(), $old_type, $new_type ) ) {
			...
			$overrides = ( 'yes' === autoship_schedule_order_enabled( $product->get_id() ) && 'yes' === autoship_schedule_process_enabled( $product->get_id() ) ) ? array(
				'addToScheduledOrder'   => true,
				'processScheduledOrder' => true,
			) : array();
...
function autoship_push_sync_product_on_status_change( $new_status, $old_status, $post ) {
	...
	if ( apply_filters( 'autoship_push_sync_product_on_status_change', autoship_is_valid_sync_product( $post->ID ) && ( $new_status !== $old_status ) && ( 'yes' === autoship_sync_active_enabled( $post->ID ) ), $post->ID, $new_status, $old_status ) ) {

		$overrides = ( 'yes' === autoship_schedule_order_enabled( $post->ID ) && 'yes' === autoship_schedule_process_enabled( $post->ID ) ) ? array(
			'addToScheduledOrder'   => true,
			'processScheduledOrder' => true,
		) : array();
...
function autoship_push_product( $id, $overrides = array(), $product = false ) {
	...
	try {
		...
		if ( 'variation' === $product->get_type() ) {
			...
			try {
				$q_product = $client->get_product( $product->get_parent_id() );
			} catch ( Exception $e ) {
			...
			if ( ! empty( $overrides ) ) {
				$overrides = array_merge(
					$overrides,
					array(
						'addToScheduledOrder'   => false,
						'processScheduledOrder' => false,
					)
				);
			}
		}
		...
		$upsert_data = autoship_generate_product_upsert_data( $product );
		...
		if ( ! doing_action( 'wp_ajax_woocommerce_save_variations' ) ) {
			...
			foreach ( $variations as $variation ) {
				...
				$result = autoship_push_product( $variation, $original_overrides );
...
function autoship_delete_product( $product ) {
	...
	// Grab the client and only continue if connection exists.
	$client = autoship_get_default_client();
	if ( empty( $client ) || empty( $client->get_token_auth() ) || ! $product ) {
		return false;
	}
	...
	try {
		...
		return new WP_Error( 'Product Delete Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	}
...
function autoship_update_product_availability( $id, $availability = '' ) {
	...
	try {
		...
		$addToScheduledOrder   = strpos( $availability, 'AddToScheduledOrder' ) !== false; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
		$processScheduledOrder = strpos( $availability, 'ProcessScheduledOrder' ) !== false; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase

		// update product availability.
		$client->update_product_availability( $product->get_id(), $addToScheduledOrder, $processScheduledOrder ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
	} catch ( Exception $e ) {
		autoship_log_entry( __( 'Autoship Products', 'autoship' ), $e->getMessage() );
	}
...
function autoship_trigger_js_event_on_default( $product ) {
	...
	if ( ( 'no' === apply_filters( 'autoship_default_product_schedule_options_choice_value', 'no', $product ) ) && apply_filters( 'autoship_product_not_loaded_via_ajax', ! wp_doing_ajax(), $product ) ) {
		return;
	}
...
function autoship_get_product_group_by_id( $group_id ) {
	...
	try {
		...
		$notice = autoship_expand_http_code( $e->getCode() );
		$notice = new WP_Error( 'Product Group Retrieval Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
