<?php
/**
 * The scheduled orders functions.
 *
 * @package Autoship
 * @since 1.0.0
 */
 ...
 * @param string $type The type of address to format, either 'shipping' or 'billing'.
 *
...
function autoship_order_address_values( $autoship_order, $type = 'shipping' ) {
	$address = 'shipping' === $type ? autoship_order_get_woocommerce_shipping_address_values( $autoship_order ) : autoship_order_get_woocommerce_billing_address_values( $autoship_order );
 ...
 * @param array          $settings Optional. The site settings to use for the lock duration.
 *
...
function autoship_check_lock_status_info( $autoship_order, $customer_id = null, $settings = array() ) {
	...
	if ( $lockable_status ) {
		...
		// translators: %s is the processing start date.
 ...
 *
 * @param bool   $locked True if locked else false.
 ...
 */
function autoship_set_locked_status( $locked, $status, $autoship_order, $customer_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
...
function autoship_customer_has_scheduled_orders( $customer_id ) {
	...
	// Create a cache key specific to this function and customer.
	$cache_key = "autoship_customers_data:{$customer_id}:has_scheduled_orders";

	// Check if we already have a cached result for this customer.
	$cached_result = wp_cache_get( $cache_key, 'autoship' );

	// If we have a cached result, return it.
	if ( false !== $cached_result ) {

		return 'yes' == $cached_result; // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
	}

	$params['page']        = 1;
	$params['pageSize']    = 25;
	$params['orderBy']     = 'Id';
	$params['statusNames'] = array( 'Active', 'Processing', 'Pending', 'Paused', 'Failed' );
	...
	try {
		...
		autoship_log_entry( 'Autoship Orders', "Unable to retrieve order check for customer {$customer_id}. The error is: {$e->getMessage()}" );

		// Cache the negative result to avoid repeated API calls.
		wp_cache_set( $cache_key, 'no', 'autoship', 60 );

		return false;
	}

	if ( is_wp_error( $orders ) ) {
		autoship_log_entry( 'Autoship Orders', "Unable to retrieve order check for customer {$customer_id}. The error is: {$orders->get_error_message()}" );
	...
	$result = ! empty( $orders->items );

	// Cache the result for future calls in this request.
	wp_cache_set( $cache_key, $result ? 'yes' : 'no', 'autoship', 60 );
 ...
 */
function autoship_get_customer_scheduled_orders( $customer_id, $index = 0, $pagesize = -1 ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 */
function autoship_filter_customer_scheduled_orders_exclude_deleted( $orders, $customer_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 *
 * @param string $autoship_status The status to change name.
 *
 * @return string
 */
function autoship_get_scheduled_order_status_nicename( $autoship_status ) {
	...
	$nicenames = autoship_rights_checker( 'autoship_admin_account_scheduled_orders_nicenames', array( 'administrator' ) ) ? array(
		'Active'  => 'Active',
		'Locked'  => 'Processing',
		'Pending' => 'Pending',
		'Failed'  => 'Failed',
...
/**
 * Get the translated name for a given autoship status nice name.
 *
 * @param string $autoship_status_nicename The nice name to change.
 *
 * @return string
 */
function autoship_get_scheduled_order_status_translatedname( $autoship_status_nicename ) {
	$statuses = array(
		'Active'     => __( 'Active', 'autoship' ),
		'Processing' => __( 'Processing', 'autoship' ),
		'Pending'    => __( 'Pending', 'autoship' ),
		'Failed'     => __( 'Failed', 'autoship' ),
		'Paused'     => __( 'Paused', 'autoship' ),
		'Deleted'    => __( 'Deleted', 'autoship' ),
	);

	return isset( $statuses[ $autoship_status_nicename ] ) ? $statuses[ $autoship_status_nicename ] : $autoship_status_nicename;
}
...
function autoship_get_next_available_nextoccurrence( $autoship_order, $format = '' ) {
	...
	if ( isset( $autoship_order['nextOccurrenceOffset'] ) && null !== $autoship_order['nextOccurrenceOffset'] ) {
...
function autoship_get_item_count( $autoship_order, $return_ids = false ) {
	...
	$items       = array();
	if ( $autoship_order instanceof stdClass ) {
		foreach ( $autoship_order->scheduledOrderItems as $item ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase

			$cycle_item      = isset( $item->minCycles ) || isset( $item->maxCycles ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			$valid_for_cycle = $cycle_item ? autoship_valid_cycle_for_item( $item->cycles, $item->minCycles, $item->maxCycles ) : true; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
 ...
 *
 * @param bool $exclude True to exclude total in Scheduled Orders screen calcs.
 *
 * @return bool false so $0 orders totals show in the Scheduled Orders screen
 */
function autoship_include_scheduled_order_total_zero_total( $exclude ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
	return false;
}
 ...
 * @return bool false so $0 orders totals show in the Scheduled Orders screen
 */
function autoship_include_scheduled_order_shipping_total_zero_total( $exclude ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
	return false;
}
 ...
 * @param string $currency The currency to format the totals in. Default is 'USD'.
 *
...
function autoship_get_formatted_order_display_totals( $totals = array(), $currency = 'USD' ) {
	...
	$defaults = apply_filters(
		'autoship_formatted_order_display_totals_default_values',
		array(
			'subtotal'       => array(
				'label' => __( 'Subtotal', 'autoship' ),
				'value' => 0,
			),
			'shipping_total' => array(
				'label' => __( 'Shipping Total', 'autoship' ),
				'value' => 0,
			),
			'tax_total'      => array(
				'label' => __( 'Tax', 'autoship' ),
				'value' => 0,
			),
			'discount_total' => array(
				'label' => __( 'Discount', 'autoship' ),
				'value' => 0,
			),
			'total'          => array(
				'label' => __( 'Total', 'autoship' ),
				'value' => 0,
			),
		)
	);
	...
	foreach ( $defaults as $key => $values ) {
		...
		if ( isset( $totals[ $key ] ) ) {
			$new_totals[ $key ]['value'] = 'shipping_total' === $key && 0 === $totals[ $key ] ? __( 'Free', 'autoship' ) : autoship_get_formatted_price( $totals[ $key ], array( 'currency' => $currency ) );
 ...
 *
 * @param array $totals An array of the current formatted totals with labels.
 *
...
function autoship_format_free_shipping_order_display_total( $totals ) {
	if ( ! array_key_exists( 'shipping_total', $totals ) && ! empty( $totals ) ) {
		...
		$totals = autoship_array_insert_before(
			'total',
			$totals,
			'shipping_total',
			array(
				'label' => __( 'Shipping Total', 'autoship' ),
				'value' => __( 'No Shipping Options Found', 'autoship' ),
			)
		);

	} elseif ( array_key_exists( 'shipping_total', $totals ) && ! $totals['shipping_total']['value'] ) {
...
function autoship_add_tax_tbd_formatted_order_display_totals( $totals ) {
	if ( ! isset( $totals['tax_total'] ) && ! empty( $totals ) ) {
		$totals = isset( $totals['discount_total'] ) ?
			autoship_array_insert_before(
				'discount_total',
				$totals,
				'tax_total',
				array(
...
function autoship_output_shipping_rate_total_row( $totals, $autoship_order, $customer_id ) {
	...
	$rate = autoship_is_valid_shipping_rate( $rates['preferred_rate'], $autoship_order ) ? $rates['available_rates'][ $rates['preferred_rate'] ] : $rates['available_rates'][ $rates['default_rate'] ];
	...
	if ( apply_filters( 'autoship_disable_shipping_rate_selection_action', ! isset( $autoship_order['lock_data'] ) || empty( $autoship_order['lock_data'] ) || $autoship_order['lock_data']['locked'], $rate, $autoship_order, $customer_id ) ) {
		...
		<tr class="cart-shipping-rate">
			<th class="selected-shipping-rate" scope="row"><?php echo esc_html( __( 'Shipping', 'autoship' ) ); ?></th>
			<td><?php echo wp_kses_post( $rate['label'] ); ?></td>
		</tr>

		<?php
	} elseif ( ( count( $rates['available_rates'] ) <= 1 ) && apply_filters( 'autoship_include_shipping_rate_refresh_option', true, $rates, $autoship_order, $customer_id ) ) {

		$refresh_url = autoship_refresh_scheduled_order_url( $autoship_order['id'] );
		?>
		<tr class="cart-shipping-rate">
			<th class="selected-shipping-rate" scope="row"><?php echo esc_html( __( 'Shipping', 'autoship' ) ); ?>
				<a href="<?php echo esc_url( $refresh_url ); ?>" class="small-autoship-action dismissible-action" data-feature-id="autoship-refresh-shipping-rates-<?php echo esc_attr( $autoship_order['id'] ); ?>"><?php esc_html_e( 'Refresh Rates', 'autoship' ); ?></a>
			</th>
			<td><?php echo wp_kses_post( $rate['label'] ); ?></td>
		</tr>
		<?php
		...
		<tr class="cart-shipping-rate">
			<th class="selected-shipping-rate" scope="row"><?php echo esc_html( __( 'Shipping', 'autoship' ) ); ?> 
			<a href="#" class="activate-action small-autoship-action autoship-modal-trigger" data-modal-toggle="#autoship_shipping_rate_option_modal"><?php esc_html_e( 'Edit', 'autoship' ); ?></a>
			</th>
			<td><a href="#" class="shipping-rate-name autoship-modal-trigger" data-modal-toggle="#autoship_shipping_rate_option_modal"><?php echo wp_kses_post( $rate['label'] ); ?></a></td>
		</tr>
		<?php
	}
...
/**
 * Get account schedule orders actions. Values can be modified through {@see autoship_get_account_scheduled_orders_actions} filter.
 *
 * @param int    $order_number The Autoship Schedule Order Number.
 * @param string $status Scheduled Order instance Status.
 * @param string $context Context in which the actions are being displayed. Default 'orders'.
 * @param int    $customer_id Optional. The wc_customer id to retrieve the orders for.
 *
 * @return array
 */
function autoship_get_account_scheduled_orders_actions( $order_number, $status, $context = 'orders', $customer_id = null ) {
	...
	$actions = array(
		'Paused'  => array(
			...
			// translators: %s is the Scheduled Order label.
 ...
 * @return array The resulting actions.
 */
function autoship_adjust_no_item_actions( $actions, $autoship_order, $customer_id, $autoship_customer_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
...
function autoship_get_my_account_scheduled_orders_columns() {
	...
	$columns = apply_filters(
		'autoship_my_account_my_scheduled_orders_columns',
		array(
			'order-number'  => __( $schedule_label, 'autoship' ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_get_scheduled_order_variation_item_name_parts( $name ) {
	...
	return apply_filters(
		'autoship_get_scheduled_order_variation_item_name_parts',
		array(
			'name' => $new_name,
			'meta' => $variation,
		),
...
function autoship_get_available_scheduled_order_shipping_rates( $autoship_order ) {
	...
	$default = array();
	$rates   = array();
	if ( isset( $autoship_order['shippingRateOptions'] ) ) {
	...
	return apply_filters(
		'autoship_get_available_scheduled_order_shipping_rates',
		array(
			'available_rates' => $rates,
			'default_rate'    => $default,
			'preferred_rate'  => $autoship_order['preferredShippingRateOption'],
		),
		$autoship_order
	);
}
 ...
 * @param boolean         $show Whether to return or echo|string.
 *
 * @return string
 */
function autoship_scheduled_order_quantity_input( $args, $product, $wc_product, $show = true ) {
	...
	if ( $args['max_value'] && $args['min_value'] === $args['max_value'] ) {
		...
		<div class="quantity hidden">
			<input type="hidden" id="<?php echo esc_attr( $args['input_id'] ); ?>" class="qty"
		...
		$labelledby = ! empty( $args['product_name'] ) ? sprintf( __( '%s quantity', 'autoship' ), wp_strip_all_tags( $args['product_name'] ) ) : '';
		?>
		<div class="quantity">
			<label class="screen-reader-text"
					for="<?php echo esc_attr( $args['input_id'] ); ?>"><?php esc_html_e( 'Quantity', 'autoship' ); ?></label>
			<input
					type="number"
	...
	$input_display = apply_filters( 'autoship_scheduled_order_quantity_input_display', ob_get_clean(), $args, $product, $wc_product, $show );

	if ( $show ) {
		echo $input_display; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	} else {
 ...
 * @param WC_Product|int $product The current wc product object or id associated with the Price.
 *
...
function autoship_get_calculated_scheduled_order_object_totals( $scheduled_order, $scheduled_items = array(), $filter_ids = array() ) {
	...
	// Convert the items.
	$scheduled_items = empty( $scheduled_items ) ? $scheduled_order['scheduledOrderItems'] : autoship_convert_object_to_array( $scheduled_items );
...
function autoship_get_calculated_scheduled_order_item_sale_price( $scheduled_item, $scheduled_order ) {
	...
	foreach ( $discounts as $code => $coupon ) {
		...
		if ( 'ReduceProductPriceByPercentage' === $coupon['discountType'] && ! empty( $coupon['productGroupId'] ) && ! empty( $scheduled_item['product']['productGroupIds'] ) && in_array( $coupon['productGroupId'], $scheduled_item['product']['productGroupIds'], true ) ) {

			// Calculate the new sale price.
			$price = $price - ( $price * ( $coupon['amount'] / 100 ) );

		} elseif ( 'ReduceProductPriceByPercentage' === $coupon['discountType'] && isset( $coupon['product'] ) && ( $coupon['product']['id'] === $scheduled_item['productId'] ) ) {

			// Calculate the new sale price.
			$price = $price - ( $price * ( $coupon['amount'] / 100 ) );

		} elseif ( 'ReduceProductPriceByAmount' === $coupon['discountType'] && ! empty( $coupon['productGroupId'] ) && ! empty( $scheduled_item['product']['productGroupIds'] ) && in_array( $coupon['productGroupId'], $scheduled_item['product']['productGroupIds'], true ) ) {

			// Calculate the new sale price.
			$discount_price = $price - $coupon['amount'];
			$price          = $discount_price >= 0 ? $discount_price : 0;

		} elseif ( 'ReduceProductPriceByAmount' === $coupon['discountType'] && isset( $coupon['product'] ) && ( $coupon['product']['id'] === $scheduled_item['productId'] ) ) {

			// Calculate the new sale price.
			$discount_price = $price - $coupon['amount'];
			$price          = $discount_price >= 0 ? $discount_price : 0;

		} elseif ( 'ReduceSubtotalByPercentage' === $coupon['discountType'] && $coupon['subtotalDiscountAppliesToRegularPrice'] ) {
	...
	if ( autoship_enable_wc_tax_settings_for_prices_display() ) {
		...
		foreach ( $scheduled_items as $item ) {
			...
			foreach ( $item['product']['productGroupIds'] as $groud_id ) {
				...
				isset( $group_ids[ $groud_id ] ) ? $group_ids[ $groud_id ][] = $item['product_id'] : $group_ids[ $groud_id ] = array( $item['product_id'] );
			}
		}
		...
		foreach ( $discounts as $code => $coupon ) {
			...
			if ( 'ReduceSubtotalByPercentage' === $coupon['discountType'] && $coupon['subtotalDiscountAppliesToRegularPrice'] ) {
			...
			} elseif ( 'ReduceSubtotalByPercentage' === $coupon['discountType'] ) {

				// Add the new discount amount to the total discounts.
				$totals['discount'] += $totals['subtotal'] * ( $coupon['amount'] / 100 );

			} elseif ( 'ReduceSubtotalByAmount' === $coupon['discountType'] ) {

				// Add the new discount amount to the total discounts.
				$totals['discount'] += $coupon['amount'];

			} elseif ( 'ReduceProductPriceByPercentage' === $coupon['discountType'] && ! empty( $coupon['productGroupId'] ) && isset( $group_ids[ $coupon['productGroupId'] ] ) ) {
			...
			} elseif ( 'ReduceProductPriceByPercentage' === $coupon['discountType'] && isset( $coupon['product'] ) ) {
				// Get the Product ID.
				...
				// Add the new discount amount to the total discounts.
				$totals['discount'] += $price * $product_discounts[ $product_id ]['qty'] * ( $coupon['amount'] / 100 );
			} elseif ( 'ReduceProductPriceByAmount' === $coupon['discountType'] && ! empty( $coupon['productGroupId'] ) && isset( $group_ids[ $coupon['productGroupId'] ] ) ) {
			...
			} elseif ( 'ReduceProductPriceByAmount' === $coupon['discountType'] && isset( $coupon['product'] ) ) {
...
function autoship_get_scheduled_order_form_items_data( $order_id, $scheduled_items, $action = 'form_item', $filter_ids = array() ) {
	...
	foreach ( $scheduled_items as $scheduled_item_key => $scheduled_item ) {
		...
		if ( ! empty( $filter_ids ) && ! in_array( $scheduled_item['id'], $filter_ids, true ) ) {
			continue;
		}
...
function autoship_get_scheduled_order_form_item_data( $order_id, $scheduled_item, $action = 'form_item' ) {
	...
	$item['imageurl'] = isset( $scheduled_item['product']['metadata']['imageUrl'] ) ? $scheduled_item['product']['metadata']['imageUrl'] : '';
	...
	$item['availability'] = false !== $item['product']['processScheduledOrder'] ? 'available-to-process' : 'unavailable-to-process';
 ...
 *
 * @param array $args Optional. The arguments to generate the url.
 *
...
function autoship_get_scheduled_order_create_url( $args = array() ) {
	...
	if ( isset( $args['freqtype'] ) ) {
		$params['freqtype'] = rawurlencode( sanitize_text_field( $args['freqtype'] ) );
	}

	// Attach the next occurrence.
	if ( isset( $args['nextoccurrence'] ) ) {
		$params['nextoccurrence'] = rawurlencode( sanitize_text_field( $args['nextoccurrence'] ) );
	}
...
function autoship_get_scheduled_order_delete_url( $order_id, $confirm = 'confirm' ) {
	...
	return apply_filters( 'autoship_scheduled_order_delete_url', $autoship_page_url ? wp_nonce_url( add_query_arg( $keys, $autoship_page_url ), 'autoship-delete-scheduled-order', 'autoship-delete-scheduled-order-nonce' ) : '' );
}
...
function autoship_get_scheduled_order_status_change_url( $order_id, $status ) {
	...
	return apply_filters( 'autoship_scheduled_order_status_change_url', $autoship_page_url ? wp_nonce_url( add_query_arg( $keys, $autoship_page_url ), 'autoship-update-scheduled-order', 'autoship-update-scheduled-order-nonce' ) : '' );
}
...
function autoship_get_scheduled_order_item_remove_url( $autoship_order_item_id, $autoship_order_id ) {
	...
	return apply_filters( 'autoship_scheduled_order_item_remove_url', $autoship_page_url ? wp_nonce_url( add_query_arg( $keys, $autoship_page_url ), 'autoship-remove-scheduled-order-item', 'autoship-remove-scheduled-order-item-nonce' ) : '' );
}
...
function autoship_get_scheduled_order_item_add_url( $args ) {
	...
	if ( isset( $args['coupon'] ) ) {
		$params['coupon'] = rawurlencode( sanitize_text_field( $args['coupon'] ) );
	}

	if ( isset( $args['redirect'] ) ) {
		$params['redirect'] = rawurlencode( sanitize_text_field( $args['redirect'] ) );
	}
 ...
 * Returns the Remove Scheduled Order Item url.
 *
 * @param int $autoship_order_coupon_code The Autoship order coupon code.
 * @param int $autoship_order_id The Autoship order id.
 *
 * @return string Output of the url.
 */
function autoship_get_scheduled_order_coupon_remove_url( $autoship_order_coupon_code, $autoship_order_id ) {
	...
	$keys = array(
		'remove-scheduled-order-coupon' => rawurlencode( $autoship_order_coupon_code ),
		'scheduled-order'               => $autoship_order_id,
	);

	$autoship_page_url = autoship_get_scheduled_order_url( $autoship_order_id );

	return apply_filters( 'autoship_scheduled_order_coupon_remove_url', $autoship_page_url ? wp_nonce_url( add_query_arg( $keys, $autoship_page_url ), 'autoship-remove-scheduled-order-coupon', 'autoship-remove-scheduled-order-coupon-nonce' ) : '' );
}
...
function autoship_get_view_edit_scheduled_order_url( $scheduled_order_id, $status ) {
	...
	if ( in_array( $status, apply_filters( 'autoship_editable_order_statuses', array( 'Active', 'Paused' ) ), true ) ) {
...
function autoship_refresh_scheduled_order_url( $scheduled_order_id ) {
	...
	return apply_filters( 'autoship_refresh_scheduled_order_url', $autoship_page_url ? wp_nonce_url( add_query_arg( $keys, $autoship_page_url ), 'autoship-refresh-scheduled-order-data', 'autoship-refresh-scheduled-order-data-nonce' ) : '' );
}
...
if ( ! function_exists( 'autoship_get_order_cache_data' ) ) {
	 ...
	 * @return mixed The cached data or an empty string if not set.
	 */
...
function autoship_maybe_retrieve_order_from_cache( $order_id, $key = null ) {
	...
	// If no order in cache, pull it and add it to cache.
	if ( empty( $order ) ) {
...
/**
 * Get the Session data for the supplied key
 *
 * @param string $key The session key.
 * @param int    $order_id Optional. The order id to verify against.
 * @param bool   $decoded Optional. True convert any objects to array.
 * @param bool   $clean_reset Optional. Clear Session on Data Pull.
 *
...
function autoship_check_schedulable_products_session_freshness( $key ) {
	...
	return apply_filters( 'autoship_schedulable_products_session_freshness', $date_diff > $threshold, $date_diff, $threshold, $key, $now );
}
...
function autoship_pull_schedulable_products_session( $product_id = 0, $decoded = false, $force_reset_on_pull = false ) {
	...
	// Get the key and check the freshness of the session if stale clear it and return empty.
...
function autoship_remove_scheduled_order_item_ajax_wrapper() {
	...
	$nonce             = $_REQUEST['autoship-remove-scheduled-order-item-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	$autoship_order_id = autoship_update_scheduled_order_post_valid_nonce_id( $nonce, 'autoship-remove-scheduled-order-item' );
	$item_id           = absint( $_POST['remove-scheduled-order-item'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotValidated

	if ( empty( $item_id ) ) {

		// translators: %s is the Autoship Scheduled Order Item.
		...
		$success = autoship_update_scheduled_order_handler(
			...
			array(
				'autoship_scheduled_order_item_id' => $item_id,
...
function autoship_update_scheduled_order_post_valid_nonce_id( $nonce, $action ) {
	...
	if ( ! isset( $_POST['autoship_scheduled_order_id'] ) || empty( $_POST['autoship_scheduled_order_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

		// translators: %s is the Autoship Scheduled Order.
		wc_add_notice( sprintf( __( 'Invalid %s ID.', 'autoship' ), autoship_translate_text( 'Scheduled Order' ) ), 'error' );

		return false;
	}

	return apply_filters( 'autoship_update_scheduled_order_order_id_verification', absint( $_POST['autoship_scheduled_order_id'] ), $action ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
}
...
function autoship_delete_scheduled_order_endpoint_wrapper() {
	...
	if ( isset( $wp->query_vars['delete-scheduled-order-confirm'] ) && isset( $wp->query_vars['scheduled-order'] ) ) {
		...
		$nonce = $_REQUEST['autoship-delete-scheduled-order-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( ! apply_filters( 'autoship_delete_scheduled_order_action_nonce_verification', wp_verify_nonce( $nonce, 'autoship-delete-scheduled-order' ) ) ) {
		...
		} elseif ( 'confirm' === $wp->query_vars['delete-scheduled-order-confirm'] ) {
			...
			$notice = apply_filters(
				'autoship_confirm_delete_schedule_link_label',
				sprintf(
					// translators: %1$s is the Autoship Scheduled Order label, %2$d is the Scheduled Order ID, %3$s is the Confirm URL, %4$s is the Cancel URL.
					__( '<span class="action-request">Are you sure you want to Delete %1$s #%2$d?</span> <a class="confirm-action" href="%3$s">Confirm</a> or <a class="cancel-action" href="%4$s">Cancel</a>', 'autoship' ),
					$scheduled_order_label,
				...
				$cancel,
				$wp->query_vars['scheduled-order'],
				$wp->query_vars['delete-scheduled-order-confirm']
			);

			wc_add_notice( $notice, 'notice' );

		} elseif ( 'confirmed' === $wp->query_vars['delete-scheduled-order-confirm'] ) {

			// Pre-flight & get Scheduled Order from cache otherwise pull it for processing.
			$original_autoship_order = autoship_maybe_retrieve_order_from_cache( $wp->query_vars['scheduled-order'] );

			$success = autoship_update_scheduled_order_handler( $wp->query_vars['scheduled-order'], 'autoship_delete_order', array( 'original_autoship_order' => $original_autoship_order ) );
		}

		wp_redirect( autoship_get_scheduled_orders_url() ); // phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
		exit();
	}
}
...
function autoship_update_scheduled_order_status_endpoint_wrapper() {
	...
	if ( isset( $wp->query_vars['update-scheduled-order-status'] ) && isset( $wp->query_vars['scheduled-order'] ) ) {
		...
		$nonce = $_REQUEST['autoship-update-scheduled-order-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( ! apply_filters( 'autoship_update_scheduled_order_action_nonce_verification', wp_verify_nonce( $nonce, 'autoship-update-scheduled-order' ) ) ) {
			...
			$success = autoship_update_scheduled_order_handler(
				...
				array(
					'status'                  => $wp->query_vars['update-scheduled-order-status'],
					'original_autoship_order' => $original_autoship_order,
				)
			);
		}
...
function autoship_remove_scheduled_order_item_endpoint_wrapper() {
	...
	if ( isset( $wp->query_vars['remove-scheduled-order-item'] ) && isset( $wp->query_vars['scheduled-order'] ) ) {
		...
		// Nonce and hook for custom verification.
		// Add custom user security checks here if custom rights are added.
		if ( ! apply_filters( 'autoship_remove_scheduled_order_item_action_nonce_verification', wp_verify_nonce( $_REQUEST['autoship-remove-scheduled-order-item-nonce'], 'autoship-remove-scheduled-order-item' ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			$success = autoship_update_scheduled_order_handler(
				...
				'autoship_remove_order_item',
				array(
					'autoship_scheduled_order_item_id' => $wp->query_vars['remove-scheduled-order-item'],
					'original_autoship_order'          => $original_autoship_order,
				)
			);

			wp_redirect( autoship_get_scheduled_order_url( $wp->query_vars['scheduled-order'] ) ); // phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
			exit();
		}
...
function autoship_add_to_scheduled_order_endpoint_wrapper( $action ) {
	...
	if ( apply_filters( 'autoship_add_to_scheduled_order_endpoint', 'add-to-scheduled-order' === $action, $action ) ) {
		...
		if ( ! isset( $link['customer_id'] ) ) {
			$notice = array(
				// translators: %s is the Autoship Scheduled Order.
				'notice'      => __( sprintf( 'A problem was encountered while attempting to add this item to the %s.  Please try again.', autoship_translate_text( 'scheduled order' ) ), 'autoship' ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
				'notice_type' => 'error',
			);
		}
		...
		if ( empty( $notice['notice'] ) ) {
			...
			if ( false !== $result && ! is_wp_error( $result ) ) {
				...
				$qty_string = $link['max'] > 1 ? __( sprintf( '%d item(s) were successfully added to your next %d %s.', $link['qty'], $link['max'], $scheduled_orders_label ), 'autoship' ) : __( sprintf( '%d item(s) were successfully added to your next %s.', $link['qty'], $scheduled_order_label ), 'autoship' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
		...
		$redirect = isset( $link['redirect'] ) ? $link['redirect'] : '';
		$redirect = apply_filters( 'autoship_add_to_next_order_notice_url', $redirect, $notice['notice_type'], $link, $customer_id );
...
function autoship_create_scheduled_order_endpoint_wrapper( $action ) {
	...
	if ( apply_filters( 'autoship_create_scheduled_order_endpoint', 'create-scheduled-order' === $action, $action ) ) {
		...
		if ( ! isset( $link['customer_id'] ) ) {
			$notice = array(
				// translators: %s is the Autoship Scheduled Order.
				'notice'      => __( sprintf( 'A problem was encountered while attempting to add this item to the %s.  Please try again.', $scheduled_order_label ), 'autoship' ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
				'notice_type' => 'error',
			);
		}
		...
		if ( empty( $notice ) ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				$notice = array(
					// translators: %1$s is the Autoship Scheduled Order, %2$d is the order ID.
					'notice'      => sprintf( __( '%1$s #%2$d was successfully created.', 'autoship' ), $scheduled_order_label, $result->id ),
					'notice_type' => 'success',
				);
...
function autoship_remove_scheduled_order_coupon_endpoint_wrapper() {
	...
	if ( isset( $wp->query_vars['remove-scheduled-order-coupon'] ) && isset( $wp->query_vars['scheduled-order'] ) ) {
		...
		$nonce = $_REQUEST['autoship-remove-scheduled-order-coupon-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
function autoship_refresh_scheduled_order_data_endpoint_wrapper() {
	...
	if ( isset( $wp->query_vars['refresh-scheduled-order'] ) && isset( $wp->query_vars['scheduled-order'] ) ) {
		...
		$nonce = $_REQUEST['autoship-refresh-scheduled-order-data-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
function autoship_get_scheduled_order_link_params( $action ) {
	...
	if ( isset( $_GET['qty'] ) && ! empty( $_GET['qty'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$qty         = absint( $_GET['qty'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$link['qty'] = $qty;
	}

	// Allow for item or items as a param.
	if ( ( isset( $_GET['items'] ) && ! empty( $_GET['items'] ) ) || ( isset( $_GET['item'] ) && ! empty( $_GET['item'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended

		$key              = isset( $_GET['items'] ) && ! empty( $_GET['items'] ) ? 'items' : 'item'; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$link['products'] = ! is_array( $_GET[ $key ] ) ? array( absint( $_GET[ $key ] ) => $qty ) : array_filter( $_GET[ $key ], 'absint' ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	}

	if ( isset( $_GET['min'] ) && ! empty( $_GET['min'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$link['min'] = ! is_array( $_GET['min'] ) ? absint( $_GET['min'] ) : array_filter( $_GET['min'], 'absint' ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	}

	if ( isset( $_GET['max'] ) && ! empty( $_GET['max'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$link['max'] = ! is_array( $_GET['max'] ) ? absint( $_GET['max'] ) : array_filter( $_GET['max'], 'absint' ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	}

	$raw_next_occurrence = isset( $_GET['nextoccurrence'] ) && ! empty( $_GET['nextoccurrence'] ) ? sanitize_text_field( $_GET['nextoccurrence'] ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
	if ( isset( $raw_next_occurrence ) ) {
		...
		foreach ( $formats_to_check as $format ) {
			...
			$next_occurrence = autoship_get_api_formatted_date( $raw_next_occurrence, $format, true );
	...
	$link['frequency']      = isset( $_GET['freq'] ) && ! empty( $_GET['freq'] ) ? absint( $_GET['freq'] ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
	$link['frequency_type'] = isset( $_GET['freqtype'] ) && ! empty( $_GET['freqtype'] ) ? sanitize_text_field( $_GET['freqtype'] ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash

	$link['order'] = isset( $_GET['order'] ) && ! empty( $_GET['order'] ) ? absint( $_GET['order'] ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Recommended

	$link['coupon'] = isset( $_GET['coupon'] ) && ! empty( $_GET['coupon'] ) ? sanitize_text_field( $_GET['coupon'] ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash

	$link['redirect'] = isset( $_GET['redirect'] ) && ! empty( $_GET['redirect'] ) ? sanitize_text_field( $_GET['redirect'] ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
	...
	$valid = ( autoship_rights_checker( 'autoship_filter_allow_customer_var_on_add_to_order', array( 'administrator' ) ) && isset( $_GET['customer'] ) ) || ( isset( $_GET['customer'] ) && hash_equals( wp_hash( $user ), $_GET['customer'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash

	if ( $valid ) {
		$link['customer_id'] = hash_equals( wp_hash( $user ), $_GET['customer'] ) ? $user : $_GET['customer']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	}
...
function autoship_validate_and_get_autoship_coupon_discount( $coupon_code, $product_id, $price, $qty, $min, $max, $user_id ) {
	...
	// Check expiration.
	if ( isset( $valid_coupon->expirationDate ) && ! empty( $valid_coupon->expirationDate ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase

		// Get the current DateTime object and Expiration DateTime object.
		$now  = autoship_get_datetime();
		$next = autoship_get_datetime( $valid_coupon->expirationDate ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
	...
	// Check minCycles.
	if ( isset( $valid_coupon->minCycles ) && ! empty( $valid_coupon->minCycles ) && ( $min < $valid_coupon->minCycles ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		return new WP_Error( 'invalid_coupon_mincycles', __( 'Invalid Minimum Cycles for this coupon.', 'autoship' ) );
	}

	// Check maxCycles.
	if ( isset( $valid_coupon->maxCycles ) && ! empty( $valid_coupon->maxCycles ) && ( $max > $valid_coupon->maxCycles ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		return new WP_Error( 'invalid_coupon_maxcycles', __( 'Invalid Maxium Cycles for this coupon.', 'autoship' ) );
	}

	// Check minSubtotal.
	if ( isset( $valid_coupon->minSubtotal ) && ! empty( $valid_coupon->minSubtotal ) && ( ( $price * $qty ) < $valid_coupon->minSubtotal ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
	...
	if ( isset( $valid_coupon->country ) && ! empty( $valid_coupon->country ) ) {
		...
		if ( strtolower( $valid_coupon->country ) !== strtolower( $customer->shippingCountry ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
	...
	if ( isset( $valid_coupon->postcode ) && ! empty( $valid_coupon->postcode ) ) {
		...
		if ( strtolower( $valid_coupon->postcode ) !== strtolower( $customer->shippingPostcode ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
	...
	if ( isset( $valid_coupon->state ) && ! empty( $valid_coupon->state ) ) {
		...
		if ( strtolower( $valid_coupon->state ) !== strtolower( $customer->shippingState ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
	...
	if ( isset( $valid_coupon->city ) && ! empty( $valid_coupon->city ) ) {
		...
		if ( strtolower( $valid_coupon->city ) !== strtolower( $customer->shippingCity ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			return new WP_Error( 'invalid_city', __( 'The supplied coupon is not valid for your City.', 'autoship' ) );
		}
	}

	$amount = $valid_coupon->amount;
	$type   = $valid_coupon->discountType; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase

	if ( 'ReduceSubtotalByPercentage' === $type ) {
		$discount = max( round( $price - ( $price * ( $amount / 100 ) ), 2 ), 0 );
	} elseif ( 'ReduceSubtotalByAmount' === $type ) {
...
function autoship_validate_and_get_woocommerce_coupon_discount( $coupon_code, $product_id, $price, $qty ) {
	...
	} elseif ( 'fixed_product' === $coupon['type'] ) {
	...
	if ( ( $coupon['usage_user_limit'] > 0 ) && $wc_coupon->get_data_store() ) {
		...
		if ( $usage_count >= $coupon['usage_user_limit'] ) {
			// translators: %d is the usage limit.
	...
	if ( isset( $coupon['expired_date'] ) ) {
		...
		$coupon['current_timestamp'] = current_time( 'timestamp', true ); // phpcs:ignore WordPress.DateTime.CurrentTimeTimestamp.RequestedUTC
	...
	if ( ! empty( $coupon['valid_products'] ) && ! in_array( $product_id, $coupon['valid_products'], true ) ) {
	...
	// First check if this product is valid for this coupon.
	if ( ! empty( $coupon['excluded_products'] ) && in_array( $product_id, $coupon['excluded_products'], true ) ) {
		return new WP_Error( 'invalid_coupon_product', __( 'This product is not valid for the supplied coupon.', 'autoship' ) );
	}
	...
	if ( ( ! empty( $coupon['categories'] ) && ( count( array_intersect( $product_cats, $coupon['categories'] ) ) <= 0 ) ) || ( ! empty( $coupon['excluded_categories'] ) && ( count( array_intersect( $product_cats, $coupon['excluded_categories'] ) ) > 0 ) ) ) {
...
function autoship_adjust_create_scheduled_order_url_redirect( $redirect, $notice_type, $link, $customer_id ) {
	...
	return ( 'template' !== autoship_get_scheduled_orders_display_version() ) ? autoship_get_scheduled_orders_url( $customer_id ) : $redirect;
}
 ...
 *
 * @param array $link The link param in an array.
 *
...
function autoship_create_scheduled_order_link_handler( $link ) {
	...
	$general_notice = apply_filters( 'autoship_create_scheduled_order_link_invalid', __( 'The supplied link is no longer valid or has expired.', 'autoship' ), $link );
	...
	if ( isset( $link['products'] ) && ! empty( $link['products'] ) ) {
		...
		$valid_products = apply_filters( 'autoship_create_scheduled_order_endpoint_valid_products_params', array( 'productIds' => array_keys( $link['products'] ) ), $link );
		...
		if ( ! is_wp_error( $available_products ) && ! empty( $available_products ) ) {
			...
			foreach ( $available_products as $key => $value ) {
				...
				$line = apply_filters(
					'autoship_create_scheduled_order_endpoint_item',
					array(
						'productId' => $value->id,
...
function autoship_add_to_scheduled_order_link_handler( $link ) {
	...
	// The General notice is used for any link failures not related to coupons.
	$general_notice = apply_filters( 'autoship_add_to_scheduled_order_link_invalid', __( 'The supplied link is no longer valid or has expired.', 'autoship' ), $link );
	...
	if ( isset( $link['products'] ) && ! empty( $link['products'] ) ) {
		...
		// Setup the Query Params for the QPilot call
		// By default we search for only those ids that are enabled to be added to an order.
		$valid_products = apply_filters( 'autoship_add_to_scheduled_order_endpoint_valid_products_params', array( 'productIds' => $products ), $link );
		...
		if ( ! is_wp_error( $available_products ) && ! empty( $available_products ) ) {
			...
			foreach ( $available_products as $key => $value ) {
				...
				if ( isset( $link['coupon'] ) && ! empty( $link['coupon'] ) ) {
					...
					$coupon_notice = apply_filters( 'autoship_add_to_scheduled_order_link_invalid_coupon', __( 'The supplied discount link is no longer valid or has expired.', 'autoship' ), $link );
	...
	if ( ( isset( $link['order'] ) && ! empty( $link['order'] ) ) || ( isset( $link['frequency'] ) || isset( $link['frequency_type'] ) ) ) {
		...
		if ( isset( $link['order'] ) && ! empty( $link['order'] ) ) {
			...
			if ( is_wp_error( $order ) || ( $order->customerId !== $link['customer_id'] ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				return false;
			}
		} else {
 ...
 *
 * @param WP_Error|bool $validation The current validation status.
 * @param array         $valids The current valid fields being submitted.
 * @param array         $fields The address field list.
 *
 * @return WP_Error|bool The validation status, true if valid, WP_Error if not.
 */
function autoship_validate_scheduled_order_shipping_address_required_fields( $validation, $valids, $fields ) {
	...
	if ( ! is_wp_error( $validation ) ) {
		...
		if ( ! empty( $missing ) ) {
			// translators: %s is a list of missing fields.
...
function autoship_update_scheduled_order_shipping_address_post_wrapper() {
	...
	// Verify action.
	if ( ! isset( $_REQUEST['autoship_update_order_shipping_address'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	...
	// Validate Nonce and get order id.
	$nonce             = $_REQUEST['autoship-update-scheduled-order-shipping-address-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	$country = isset( $_POST['country'] ) ? sanitize_text_field( $_POST['country'] ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	if ( $success ) {
		...
		foreach ( $fields as $key => $data ) {
			...
			if ( isset( $_POST[ $key ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

				// Get Value & clean based on type.
				$value = 'checkbox' === $data['type'] ? (int) isset( $_POST[ $key ] ) : wc_clean( wp_unslash( $_POST[ $key ] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	$url = is_wp_error( $success ) || ! $success ? add_query_arg( 'action', 'edit-shipping-address', autoship_get_scheduled_order_url( $autoship_order_id ) ) : autoship_get_scheduled_order_url( $autoship_order_id );
...
function autoship_update_scheduled_order_schedule_post_wrapper() {
	...
	// Verify action.
	if ( ! isset( $_REQUEST['autoship_update_order_schedule'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	...
	// Validate Nonce and get order id.
	$nonce             = $_REQUEST['autoship-update-scheduled-order-schedule-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	if ( ! isset( $_POST['autoship_order_frequency'] ) || empty( $_POST['autoship_order_frequency'] ) || ! isset( $_POST['autoship_next_occurrence'] ) || empty( $_POST['autoship_next_occurrence'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

		wc_add_notice( sprintf( __( 'Invalid or Missing %s information.', 'autoship' ), autoship_translate_text( 'Scheduled Order' ) ), 'error' ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
		...
		// key is ignored.
		$frequency = apply_filters( 'autoship_update_order_frequency_value_formatted', explode( ':', $_POST['autoship_order_frequency'] ), $_POST['autoship_order_frequency'], $autoship_order_id ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$next_occurrence = apply_filters( 'autoship_update_order_next_occurrence_value_formatted', autoship_get_api_formatted_date( $_POST['autoship_next_occurrence'], 'Y-m-d', true ), $_POST['autoship_next_occurrence'], $autoship_order_id ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
function autoship_update_scheduled_order_items_post_wrapper() {
	...
	// Verify action.
	if ( ! isset( $_REQUEST['autoship_update_schedule_items'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	...
	// Validate Nonce and get order id.
	$nonce             = $_REQUEST['autoship-update-scheduled-order-items-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	$success = autoship_update_scheduled_order_handler(
		...
		array(
			'order_items'             => isset( $_POST['autoship_scheduled_order_items'] ) ? $_POST['autoship_scheduled_order_items'] : array(), // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			'new_order_items'         => isset( $_POST['autoship_scheduled_order_add_items'] ) ? $_POST['autoship_scheduled_order_add_items'] : array(), // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			'original_autoship_order' => $original_autoship_order,
		)
	);
}
...
function autoship_remove_scheduled_order_item_post_wrapper() {
	...
	// Verify action.
	if ( ! isset( $_REQUEST['autoship_remove_order_item'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	...
	if ( empty( $item_id ) ) {
		...
		wc_add_notice( sprintf( __( 'Invalid or Missing %s Item Information.', 'autoship' ), autoship_translate_text( 'Scheduled Order' ) ), 'error' ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
...
function autoship_update_scheduled_order_payment_method_post_wrapper() {
	...
	// Verify action.
	if ( ! isset( $_REQUEST['autoship_update_order_payment_method'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	...
	// Validate Nonce and get order id.
	$nonce             = $_REQUEST['autoship-update-scheduled-order-payment-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	if ( ! isset( $_POST['autoship_order_payment_method'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

		wc_add_notice( sprintf( __( 'Invalid or Missing %s Payment Method information.', 'autoship' ), autoship_translate_text( 'Scheduled Order' ) ), 'error' ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment

		return;

	} elseif ( isset( $_POST['autoship_update_order_payment_method'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$success = autoship_update_scheduled_order_handler(
			...
			array(
				'autoship_order_payment_method' => $_POST['autoship_order_payment_method'], // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				'original_autoship_order'       => $original_autoship_order,
			)
		);
	}
...
function autoship_update_scheduled_order_coupon_post_wrapper() {
	...
	// Verify action.
	if ( ! isset( $_REQUEST['autoship_update_order_coupon'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	...
	$success = autoship_update_scheduled_order_handler(
		...
		array(
			'autoship_coupon_code'    => $_POST['autoship_coupon_code'], // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotValidated
			'original_autoship_order' => $original_autoship_order,
		)
	);
}
...
function autoship_update_scheduled_order_preferred_shipping_rate_post_wrapper() {
	...
	// Verify action.
	if ( ! isset( $_REQUEST['autoship_order_preferred_shipping_rate'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	...
	// Validate Nonce and get order id.
	$nonce             = $_REQUEST['autoship-update-scheduled-order-preferred-shipping-rate-nonce']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	if ( ! isset( $_POST['autoship_order_preferred_shipping_rate'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

		wc_add_notice( sprintf( __( 'Invalid or Missing %s Shipping Rate information.', 'autoship' ), autoship_translate_text( 'Scheduled Order' ) ), 'error' ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment

		return;
	} elseif ( isset( $_POST['autoship_order_preferred_shipping_rate'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		$success = autoship_update_scheduled_order_handler(
			...
			array(
				'autoship_order_preferred_shipping_rate' => $_POST['autoship_order_preferred_shipping_rate'], // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				'original_autoship_order'                => $original_autoship_order,
			)
		);
	}
...
function autoship_update_scheduled_order_handler( $order_id, $action, $data = array() ) {
	...
	$valid_actions = apply_filters(
		'autoship_update_scheduled_order_handler_valid_actions',
		array(
	...
	$customer_id = isset( $data['original_autoship_order'] ) && ! empty( $data['original_autoship_order'] ) && isset( $data['original_autoship_order']['customerId'] ) && ! empty( $data['original_autoship_order']['customerId'] ) ? $data['original_autoship_order']['customerId'] : 0;

	// Rights for use to make the update.
	// Defaults to everyone can modify.
	if ( ! autoship_rights_checker( 'autoship_update_scheduled_order_rights', array(), $customer_id ) || ! autoship_rights_checker( "autoship_update_scheduled_order_{$action}_rights", array(), $customer_id ) ) {
	...
	$default_msgs = apply_filters(
		"default_{$action}_action_messages",
		array(
			// translators: %s is the Scheduled Order label.
			'error'   => sprintf( __( 'A problem was encountered while trying to update the %s.', 'autoship' ), $scheduled_order_label ),
			// translators: %s is the Scheduled Order label and %s is the Scheduled Order ID.
	...
	if ( is_wp_error( $result ) ) {
		...
		wc_add_notice( __( $default_msgs['error'] . '<br/>' . $result->get_error_message(), 'autoship' ), 'error' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	wc_add_notice( __( sprintf( $default_msgs['success'], $order_id ), 'autoship' ), 'success' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
 ...
 * @return int|bool|WP_Error  True if successful or WR_Error|false on failure.
 */
function autoship_delete_order_action_handler( $order_id, $data ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 */
function autoship_delete_order_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
	$scheduled_order_label = autoship_translate_text( 'Scheduled Order' );
...
function autoship_update_order_status_action_handler( $order_id, $data ) {
	...
	if ( ! isset( $valid_statuses[ $data['status'] ] ) ) {
		...
		// translators: %s is the Scheduled Order label.
		wc_add_notice( __( sprintf( 'Invalid %s Status.', autoship_translate_text( 'Scheduled Order' ) ), 'autoship' ), 'error' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_update_order_status_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
function autoship_update_order_shipping_address_action_handler( $order_id, $data ) {
	...
	if ( isset( $data['original_autoship_order'] ) && ! empty( $data['original_autoship_order'] ) ) {
		...
		if ( ! $changed && ( array_merge( $current_address, $data['shipping_address'] ) === count( $current_address ) ) ) {
			...
			// translators: %1$s is the Scheduled Order label and %2$s is the Scheduled Order ID.
			wc_add_notice( sprintf( __( 'No changes updated for %1$s #%2$s.', 'autoship' ), autoship_translate_text( 'Scheduled Order' ), $order_id ), 'notice' );
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_update_order_shipping_address_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
function autoship_update_order_schedule_action_handler( $order_id, $data ) {
	...
	// Verify it needs to be updated if possible.
	$next_occurrence_unchanged = false;
	$frequency_type_unchanged  = false;
	$frequency_unchanged       = false;
	if ( isset( $data['original_autoship_order'] ) && ! empty( $data['original_autoship_order'] ) ) {
		...
		if ( $frequency_unchanged && $frequency_type_unchanged && $next_occurrence_unchanged ) {
			...
			// translators: %1$s is the Scheduled Order label and %2$s is the Scheduled Order ID.
			wc_add_notice( sprintf( __( 'No changes updated for %1$s #%2$s.', 'autoship' ), $scheduled_order_label, $order_id ), 'notice' );
	...
	if ( ! $frequency_unchanged || ! $frequency_type_unchanged ) {
		...
		// translators: %s is the Scheduled Order label.
		$error   = __( sprintf( "A problem was encountered updating the %s's Frequency.", $scheduled_order_label ), 'autoship' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	if ( ! $next_occurrence_unchanged && ! is_wp_error( $result ) ) {
		...
		$error = $updated ?
			// translators: %s is the Scheduled Order label.
			sprintf( __( "The %s's Frequency was updated successfully, however, there was a problem updating the Next Occurrence Date.", 'autoship' ), $scheduled_order_label ) :
			// translators: %s is the Scheduled Order label.
	...
	if ( ! empty( $error ) ) {
		add_filter(
			'default_autoship_update_order_schedule_action_messages',
			function ( $messages ) {
				return array_merge(
					$messages,
					array(
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_update_order_schedule_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
function autoship_update_schedule_items_action_handler( $order_id, $data ) {
	...
	// Verify it needs to be updated if possible.
	$updated_autoship_order_items  = array();
	$original_autoship_order_items = array();
	if ( isset( $data['original_autoship_order'] ) && ! empty( $data['original_autoship_order'] ) ) {
	...
	foreach ( $data['order_items'] as $item_id => $item ) {
		...
		if ( apply_filters( 'autoship_update_schedule_items_changed_made', $change, $original_autoship_order_items[ $item_id ], $item ) ) {
	 ...
	 */
	$updated_autoship_order_items = apply_filters( 'autoship_update_schedule_items_changes', $updated_autoship_order_items, $original_autoship_order_items, $order_id, 'autoship_update_schedule_items', $data );
	...
	if ( ( empty( $updated_autoship_order_items ) || ( ! empty( $updated_autoship_order_items ) && ! is_wp_error( $result ) ) ) && ! empty( $data['new_order_items'] ) ) {
		...
		if ( ! is_wp_error( $data['original_autoship_order'] ) ) {
			...
			foreach ( $data['new_order_items'] as $item_ids => $item_data ) {
				...
				$new_autoship_order_items[] = autoship_create_scheduled_order_item_data( $order_id, $ids[0], $ids[1], $item_data['qty'], $data['original_autoship_order']['frequencyType'], $data['original_autoship_order']['frequency'] );
			}

			/**
			 * Allow devs to modify item changes on the fly.
			 */
			$new_autoship_order_items = apply_filters( 'autoship_update_schedule_new_items_changes', $new_autoship_order_items, $order_id, 'autoship_update_schedule_items', $data );
	...
	} elseif ( ! empty( $new_autoship_order_items ) ) {
		$new_messages = array(
			// translators: %s is the Scheduled Order label.
	...
	if ( ! empty( $new_messages ) ) {
		add_filter(
			'default_autoship_update_schedule_items_action_messages',
			function ( $messages ) {
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_update_schedule_items_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
function autoship_update_order_payment_method_action_handler( $order_id, $data ) {
	...
	if ( isset( $data['original_autoship_order'] ) && ! empty( $data['original_autoship_order'] ) ) {
		...
		if ( $data['autoship_order_payment_method'] === $data['original_autoship_order']['paymentMethodId'] ) {
			...
			// translators: %1$s is the Scheduled Order label and %2$s is the Scheduled Order ID.
			wc_add_notice( sprintf( __( 'No changes needed for the Payment Method for %1$s #%2$s.', 'autoship' ), autoship_translate_text( 'Scheduled Order' ), $order_id ), 'notice' );
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_update_order_payment_method_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
function autoship_update_order_preferred_shipping_rate_action_handler( $order_id, $data ) {
	...
	if ( ! autoship_is_valid_shipping_rate( $data['autoship_order_preferred_shipping_rate'], $autoship_order ) ) {
		// translators: %1$s is the Scheduled Order label and %2$s is the Scheduled Order ID.
		wc_add_notice( __( sprintf( 'The Selected Rate is no longer available for %1$s #%2$s.', autoship_translate_text( 'Scheduled Order' ), $order_id ), 'autoship' ), 'error' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	// Run the Set method
	// Method returns WP_Error on failure.
	$result = autoship_set_scheduled_order_preferred_shipping_rate( $autoship_order, $data['autoship_order_preferred_shipping_rate'] );
	if ( is_wp_error( $result ) ) {
		return $result;
	}
	...
	if ( ! $result ) {
		// translators: %1$s is the Scheduled Order label and %2$s is the Scheduled Order ID.
		wc_add_notice( __( sprintf( 'The Selected Rate is no longer available for %1$s #%2$s. The default shipping rate has been selected instead.', autoship_translate_text( 'Scheduled Order' ), $order_id ), 'autoship' ), 'error' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_update_order_preferred_shipping_rate_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_remove_order_item_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_add_order_coupon_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_remove_order_coupon_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
 ...
 * @return int|bool|WP_Error  True if successful or WR_Error|false on failure.
 */
function autoship_refresh_scheduled_order_action_handler( $order_id, $data ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @return array The filtered error and success messages for the action.
 */
function autoship_refresh_scheduled_order_action_handler_messages( $messages ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
/**
 * Handler for the autoship_scheduled_locked_order_api_action_excemption_handler filter
 *
 * This handler is used to catch the 400 error code when trying to change a locked scheduled order.
 * It provides a user-friendly message indicating that the order is locked and cannot be changed.
 *
 * @param WP_Error  $notice The current notice object.
 * @param Exception $e The exception thrown by the API.
 * @param int       $order_id The ID of the order being processed.
 * @return WP_Error The modified notice object with a user-friendly message.
 */
function autoship_scheduled_locked_order_api_action_excemption_handler( $notice, $e, $order_id ) {
	...
	if ( ( '400' === $e->getCode() ) && ( strpos( $e->getMessage(), 'Cannot change a locked scheduled order' ) !== false ) ) {
		...
		// translators: %s is the Scheduled Order label and %d is the Scheduled Order ID.
		$notice = new WP_Error( 'User Message', __( sprintf( 'Your %s #%d has started processing. No changes can be made at this time.', autoship_translate_text( 'scheduled order' ), $order_id ), 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_get_site_order_settings() {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$result = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Site Settings Retrieval Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	$settings = array(
		...
		'lockDurationDays'           => $result->lockDurationDays, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		'lockNotificationOffsetDays' => $result->lockNotificationOffsetDays, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		'isShowingNotes'             => $result->isShowingNotes, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		'isShowCoupons'              => $result->isShowCoupons, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		'isProcessingPaused'         => $result->isProcessingPaused, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		'hasActiveInvalidProducts'   => $result->hasActiveInvalidProducts, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		'orderProcessingStartTime'   => $result->orderProcessingStartTime, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		'orderProcessingEndTime'     => $result->orderProcessingEndTime, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		'supportsProcessingWindow'   => $result->supportsProcessingWindow, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		'orderProcessingOffset'      => $result->orderProcessingOffset, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
	);
 ...
 * @param string $from_date The date from which the next occurrence should be calculated. Date should be in Y-m-d H:i:s format ( 2019-07-19 15:21:39 ).
 *
...
function autoship_calculate_scheduled_order_next_occurrence( $frequency_type, $frequency, $from_date ) {
	...
	try {
		...
		$next_occurrence = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Next Occurrence Calculation Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_get_scheduled_order_items( $order_id, $item_ids = array() ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_expand_http_code( $e->getCode() );
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Scheduled Item retrieval Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	// Iterate through the scheduled order items and unset any
	// not requested.
	foreach ( $order->scheduledOrderItems as $key => $item ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase

		if ( empty( $item_ids ) || isset( $item_ids[ $item->id ] ) ) {
			$return_items[ $item->id ] = $order->scheduledOrderItems[ $key ]; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		}
	}
...
function autoship_get_available_scheduled_order_payment_methods( $customer_id ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Retrieve Payment Methods Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_get_next_scheduled_order( $customer_id, $frequency = null, $frequency_type = null, $status = null ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Get Next Scheduled Order Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
 ...
 * @param bool $filter Optional. If true, the returned object will be converted to an array.
 *
...
function autoship_get_scheduled_order( $order_id, $filter = false ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			// translators: %d is the Scheduled Order ID.
			...
			$order = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Retrieval Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_get_scheduled_orders( $order_ids = array(), $customer_id = 0 ) {
	...
	if ( ! empty( $order_ids ) ) {
		$all_orders = is_array( $order_ids ) ? autoship_get_scheduled_orders_by_ids( $order_ids ) : autoship_get_scheduled_order( $order_ids );
	} else {
...
function autoship_get_all_customer_scheduled_orders( $customer_id ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$orders = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Retrieval Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
 ...
 *
 * @param int   $customer_id Optional. An Autoship customer id.
 * @param int   $index Optional. The page index to retrieve.
 * @param array $params Optional. An array of search parameters.
 * @param bool  $recursive Optional. True recursively call self starting with supplied page until all results are retrieved. False only retrieve requested page.
 *
...
function autoship_search_all_scheduled_orders( $customer_id = null, $index = 1, $params = array(), $recursive = true ) {
	...
	if ( $recursive && ( $orders->totalPages > $index ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
...
function autoship_get_schedulable_products_from_qpilot() {
	...
	$all_products = empty( $all_products ) ?
		autoship_search_available_products(
			array(
				'availability'        => 1,
...
function autoship_retrieve_scheduled_order_customer_object( $customer_id ) {
	...
	// Check if this customer exists already in QPilot & If not lets assume they need to be added.
 ...
 *
 * @param int   $customer_id The WC Customer ID.
 * @param array $defaults  Optional. An array of default data to use instead of retrieved data.
 *
...
function autoship_create_scheduled_order_assign_customer_info( $customer_id, $defaults = array() ) {
	...
	return wp_parse_args( apply_filters( 'autoship_create_scheduled_order_assign_customer_info', $customer_data, $defaults, $customer_id ), $defaults );
}
...
function autoship_create_scheduled_order_assign_payment_info( $customer_id, $defaults = array() ) {
	...
	if ( ! isset( $defaults['payment_object'] ) || empty( $defaults['payment_object'] ) ) {
		...
		$defaults['payment_object'] = apply_filters( 'autoship_create_scheduled_order_assign_payment_method_default', empty( $payment_methods ) ? null : $payment_methods[0], $payment_methods, $customer_id, $defaults );
	}
	...
	// let the defaults be used if supplied as overrides.
	return wp_parse_args( apply_filters( 'autoship_create_scheduled_order_assign_payment_info', $payment_data, $defaults, $customer_id ), $defaults );
}
...
function autoship_create_scheduled_order_assign_scheduled_items( $customer_id, $items ) {
	...
	$available_products = autoship_get_products_by_ids(
		array_keys( $items ),
		5,
		array(
	...
	foreach ( $products as $key => $value ) {
		$scheduled_order_items[] = array(
			'productId' => $value->id,
			'price'     => $value->price,
			'salePrice' => $value->salePrice, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
 ...
 * @param array $order_data  Optional. The array of default order data to use for creating the scheduled order. Default empty array.
 *
...
function autoship_create_scheduled_order( $customer_id, $order_data = array() ) {
	...
	$items_data = isset( $order_data['scheduledOrderItems'] ) && ! empty( $order_data['scheduledOrderItems'] ) ? $order_data['scheduledOrderItems'] : null;
	...
	$order_data['scheduledOrderItems'] = ( ! is_wp_error( $items_data ) && isset( $items_data ) && ! empty( $items_data ) ) ? $items_data : null;
	...
	// Assign the status.
	$order_data['status'] = isset( $order_data['status'] ) ? $order_data['status'] : apply_filters( 'autoship_create_scheduled_orders_default_status', 'Active', $order_data, $customer_id );
...
function autoship_upsert_scheduled_order( $customer_id, $order ) {
	...
	try {
		...
		$error = autoship_is_user_http_message( $e->getCode() ) ?
			new WP_Error( $notice['msg'], $notice['desc'] ) :
			// translators: %1$d is the error code, %2$s is the order id, %3$d is the customer id, %4$s is the notice description.
			new WP_Error( 'Creating/Updating Scheduled Orders Failed', sprintf( __( '%1$d Creating/Updating Scheduled Order #%2$s for Customer #%3$d Failed.  Additional Details: %4$s' ), $e->getCode(), $order['originalExternalId'], $customer_id, $notice['desc'] ) );
 ...
 *
 * @param int   $order_id A Scheduled Order id to update.
 * @param array $scheduled_order_items Optional. An array of item arrays.
 *
...
function autoship_create_scheduled_order_items( $order_id, $scheduled_order_items ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Update Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_update_scheduled_order( $order_id, $order ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			// translators: %d is the error code, %s is the order id, %s is the notice description.
			$error = autoship_is_user_http_message( $e->getCode() ) ?
			new WP_Error( $notice['msg'], $notice['desc'] ) :
			// translators: %1$d is the error code, %2$d is the order id, %3$s is the notice description.
			new WP_Error( 'Updating Scheduled Orders Failed', sprintf( __( 'Updating Scheduled Order #%1$d Failed.  Additional Details: %2$s' ), $order_id, $notice['desc'] ) );
...
function autoship_set_scheduled_order_preferred_shipping_rate( $autoship_order, $shipping_rate ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Shipping Rate Update Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_set_scheduled_order_payment_method( $order_id, $payment_method_id ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Payment Method Update Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_set_scheduled_order_frequency( $order_id, $frequency_type, $frequency ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Frequency Update Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_set_scheduled_order_shipping_address( $autoship_order, $address = array() ) {
	...
	$autoship_order = array_merge(
		$autoship_order,
		apply_filters(
			'autoship_generated_scheduled_order_updated_shipping_data',
			array(
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Shipping Address Update Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function update_scheduled_order_next_occurrence( $order_id, $next_occurrence ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Next Occurrence Update Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
			autoship_log_entry( __( 'Autoship Orders', 'autoship' ), sprintf( '%d Order #%d Next Occurrence Update to %s Failed. Additional Details: %s', $e->getCode(), $order_id, $next_occurrence, $e->getMessage() ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
		}
 ...
 * @param float $price Optional. The Price. If not supplied uses the WC Product Price. Default NULL.
 * @param float $sale_price Optional. The Sale Price to use. If not supplied uses the WC Product Autoship Recurring Price or sale price. Default NULL.
 * @param int   $min_cycles The first time this item should be processed. Default NULL.
 * @param int   $max_cycles The number of times this item should be processed. Default NULL.
 *
...
function autoship_update_next_scheduled_order_with_item( $customer_id, $product_id, $qty = 1, $price = null, $sale_price = null, $min_cycles = null, $max_cycles = null ) {
	...
	$result = autoship_update_next_scheduled_order_with_items(
		$customer_id,
		array(
			array(
 ...
 *
 * @param int   $customer_id The wc customer id.
 * @param array $scheduled_order_items The items.
 *
 * @type int $productId The WC Product ID.
 * @type float $price The item price. Default if not supplied pulled from Product
 * @type float $salePrice The item sale price. Default if not supplied pulled from Product
...
function autoship_update_next_scheduled_order_with_items( $customer_id, $scheduled_order_items ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Add Item(s) to Next Scheduled Order Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_set_scheduled_order_status( $order_id, $status ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Status Update Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_delete_scheduled_order( $order_id ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_expand_http_code( $e->getCode() );
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Order Delete Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_remove_scheduled_order_item( $order_id, $scheduled_order_item_id ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Remove Order item Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
 ...
 * @param string $template The template file to use. Default 'scheduled-orders'.
 *
...
function autoship_scheduled_orders_app_display( $customer_id, $autoship_customer_id, $template = 'scheduled-orders' ) {
	...
	// Load the app.
	$app_params = array(
		'settings_url'     => autoship_get_relative_uri( admin_url( '/admin-ajax.php?action=autoship_get_scheduled_orders_settings' ) ),
		'autoship_version' => Autoship_Version,
	);

	$app_url = apply_filters( 'autoship_scheduled_orders_app_url', $app_path . '?' . http_build_query( $app_params ), $app_path, $app_params );
...
function autoship_scheduled_orders_hosted_display( $customer_id, $autoship_customer_id, $template = 'scheduled-orders' ) {
	...
	if ( empty( $autoship_orders ) || is_wp_error( $autoship_orders ) ) {
		...
		try {
			$access_token = $client->generate_customer_access_token( $autoship_customer_id, autoship_get_client_secret() );

		} catch ( Exception $e ) {
			if ( 401 === $e->getCode() ) {
				try {
					autoship_refresh_token_auth();
					$url = $_SERVER['REQUEST_URI']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

					return sprintf( '<script>window.location = %s;</script>', wp_json_encode( $url ) );
				} catch ( Exception $e ) {
		...
		$app_params = apply_filters(
			'autoship_scheduled_orders_hosted_url_params',
			array(
				'tokenBearerAuth' => $access_token->tokenBearerAuth, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			),
			$customer_id,
			$autoship_customer_id
		);

		$app_url = sprintf( '%s/widgets/scheduled-orders/%d/%d?%s', autoship_get_merchants_url(), $client->get_site_id(), $autoship_customer_id, http_build_query( $app_params ) );
	...
	return autoship_render_template( apply_filters( 'autoship_scheduled_orders_hosted_display_template', $template, $customer_id, $autoship_customer_id ), $args );
}
 ...
 * @param int    $autoship_customer_id The Autoship customer id.
 * @param string $template Optional. The Autoship Scheduled Orders page template.
 * @param int    $page Optional. The page number to display. Default 1.
 * @param bool   $paginate Optional. Whether to paginate the orders. Default false.
 *
 * @return string Output of the template file.
 *
 * Includes the following filters to adjust settings
...
function autoship_scheduled_orders_template_display( $customer_id, $autoship_customer_id, $template = 'scheduled-orders/orders-template', $page = 1, $paginate = false ) {
	...
	if ( ! autoship_rights_checker( 'autoship_scheduled_orders_template_display', array(), $customer_id ) ) {
		...
		$template = apply_filters( 'autoship_scheduled_orders_template_display_no_rights_redirect_template', 'scheduled-orders/order-error-template', $customer_id );

		$autoship_orders = new WP_Error(
			// translators: %s is the name of the Scheduled Orders.
			sprintf( __( 'Invalid %s Display', 'autoship' ), autoship_translate_text( 'Scheduled Orders' ) ),
			// translators: %s is the name of the Scheduled Orders.
			sprintf( __( 'A problem was encountered while trying to display the %s. Please refresh the page and try again.', 'autoship' ), autoship_translate_text( 'Scheduled Orders' ) )
		);
	...
	return autoship_render_template(
		...
		array(
			...
			'customer_id'          => $customer_id,
			'autoship_customer_id' => $autoship_customer_id,
			'autoship_order'       => ! empty( $autoship_orders ) && ! is_wp_error( $autoship_orders ) ? reset( $autoship_orders ) : $autoship_orders, // Only used for the order error template display.
		)
	);
}
...
function autoship_scheduled_order_template_display( $customer_id, $autoship_customer_id, $autoship_order_id, $template = 'scheduled-orders/order-template' ) {
	...
	if ( autoship_rights_checker( 'autoship_scheduled_order_template_display', array( 'administrator' ), $customer_id ) || ( $autoship_order['customerId'] === $autoship_customer_id ) ) {
		...
		$autoship_order = new WP_Error(
			// translators: %s is the name of the Scheduled Order.
			sprintf( __( '404 %s Not Found', 'autoship' ), autoship_translate_text( 'Scheduled Order' ) ),
			// translators: %1$s is the name of the Scheduled Order, %2$s is the order number, %3$s is the name of the Scheduled Order.
			sprintf( __( '%1$s #%2$s could not be found or is invalid. Please check the %3$s number and try again.', 'autoship' ), autoship_translate_text( 'Scheduled Order' ), '<mark class="order-number">' . $autoship_order_id . '</mark>', autoship_translate_text( 'Scheduled Order' ) )
		);
	}
...
function autoship_view_scheduled_order_template_display( $customer_id, $autoship_customer_id, $autoship_order_id, $template = 'scheduled-orders/order-view-template' ) {
	...
	if ( ! autoship_rights_checker( 'autoship_scheduled_order_template_display', array(), $customer_id ) ) {
		...
		$autoship_orders = new WP_Error(
			// translators: %s is the name of the Scheduled Orders.
			sprintf( __( '404 %s Not Found', 'autoship' ), autoship_translate_text( 'Scheduled Order' ) ),
			// translators: %1s is the name of the Scheduled Order, %2$s is the order number, %3$s is the name of the Scheduled Order.
			sprintf( __( '%1$s #%2$s could not be found or is invalid. Please check the %3$s number and try again.', 'autoship' ), autoship_translate_text( 'Scheduled Order' ), '<mark class="order-number">' . $autoship_order_id . '</mark>', autoship_translate_text( 'Scheduled Order' ) )
		);
...
/**
 * Display Scheduled Orders V2 Portal.
 *
 * @param int    $customer_id The wc_customer id.
 * @param int    $autoship_customer_id The Autoship customer id.
 * @param string $template Optional. The Autoship Scheduled Orders page template.
 ...
 * @return string Output of the template file.
 */
function autoship_scheduled_orders_v2_portal_display( $customer_id, $autoship_customer_id, $template = 'scheduled-orders' ) {
	// Get the template to use if not supplied.
	$template = empty( $template ) ? 'scheduled-orders-v2-portal' : $template;

	$autoship_orders = autoship_get_customer_scheduled_orders( $customer_id );

	$should_display_without = autoship_get_display_portal_without_scheduled_orders();

	$display_without = empty( $autoship_orders ) && ( 'yes' === $should_display_without );

	if ( ! is_wp_error( $autoship_orders ) && ( ! empty( $autoship_orders ) || $display_without ) ) {
		...
		$args = apply_filters(
			'autoship_scheduled_orders_v2_portal_display_template_args',
			array(
				'customer_id'          => $customer_id,
				'autoship_customer_id' => $autoship_customer_id,
				'site_id'              => $client->get_site_id(),
				'token'                => $access_token->tokenBearerAuth, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			)
		);
	} else {
		$template = 'scheduled-orders/orders-error-template';
	...
	return autoship_render_template(
		apply_filters(
			'autoship_scheduled_orders_v2_portal_display_template',
			$template,
			$customer_id,
			$autoship_customer_id
		),
 ...
 * @return void
 */
function autoship_scheduled_orders_header_wp_notices_display( $customer_id, $autoship_customer_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	wc_print_notices();
	wc_clear_notices();
}
...
function autoship_scheduled_orders_custom_html_header_display() {
	...
	<?php
		...
		echo apply_filters( 'the_content', autoship_get_scheduled_orders_html() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	?>
	</div>
	<?php
}
 ...
 * @param int   $customer_id The woocommerce customer id.
 * @param int   $autoship_customer_id The autoship customer id.
 * @param array $autoship_orders The autoship orders array.
 *
...
function autoship_scheduled_orders_header_actions( $customer_id, $autoship_customer_id, $autoship_orders ) {
	...
	$actions = apply_filters(
		'autoship_scheduled_orders_header_actions',
		array(
			'autoship-create-new-order' => sprintf(
				'<a href="%s" class="button autoship-action-btn">%s</a>',
				autoship_get_scheduled_order_create_url(
					array(
						'customer' => $customer_id,
					)
				),
				// translators: %s is the name of the Scheduled Order.
				sprintf( __( 'Create New %s', 'autoship' ), $label )
			),
		),
		$customer_id,
		$autoship_customer_id,
...
function autoship_non_hosted_scheduled_order_error_display( $customer_id, $autoship_customer_id ) {
	$autoship_orders        = autoship_get_customer_scheduled_orders( $customer_id );
	$display_setting        = autoship_get_scheduled_orders_display_version();
	$should_display_without = autoship_get_display_portal_without_scheduled_orders();

	$display_without = empty( $autoship_orders ) && ( 'yes' === $should_display_without );

	// If its the v2 portal or app and no orders are found dont show anything.
	if ( ( 'v2_portal' === $display_setting || 'app' === $display_setting ) && $display_without ) {
		return;
	} elseif ( empty( $autoship_orders ) && ! is_wp_error( $autoship_orders ) ) {
 ...
 * @return string The html output
 */
function autoship_scheduled_order_error_template_custom_html_display( $autoship_orders, $customer_id, $autoship_customer_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	<?php
		// Get the Text from Admin Settings.
		echo apply_filters( 'the_content', autoship_get_scheduled_orders_body_html() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	?>
	</div>
	<?php
}
...
function autoship_scheduled_order_error_template_notice_display( $customer_id, $autoship_customer_id, $autoship_orders ) {
	...
	$notice = apply_filters( 'autoship_scheduled_orders_error_notice', $autoship_orders->get_error_code(), $autoship_orders, $customer_id, $autoship_customer_id );

	$notice_details = apply_filters( 'autoship_scheduled_orders_error_notice_details', $details = $autoship_orders->get_error_message(), $autoship_orders, $customer_id, $autoship_customer_id );

	if ( empty( $notice ) && empty( $notice_details ) ) {
		return;
	}
	?>
	<h3 class="autoship-error-notice"><span><?php echo wp_kses_post( $notice ); ?></span></h3>
	<p class="autoship-error-notice-details"><?php echo wp_kses_post( $notice_details ); ?></p>
	<?php
}
 ...
 * @return void
 */
function autoship_scheduled_order_header_locked_notice_display( $autoship_order, $customer_id, $autoship_customer_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	if ( $autoship_order['lock_data']['locked'] ) {
		echo apply_filters( 'autoship_scheduled_order_header_locked_notice_display', '<p class="autoship_order_notice woocommerce-message woocommerce-notice woocommerce-info" role="alert">' . $autoship_order['lock_data']['notice'] . '</p>', $autoship_order, $customer_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
/**
 * Displays the Scheduled Order Schedule Upsell Carousel
 *
 * @param array $autoship_order The autoship order array.
 * @param int   $customer_id The woocommerce customer id.
 * @param int   $autoship_customer_id The autoship customer id.
 ...
 * @return string The html output
 */
function autoship_scheduled_order_upsell_products_template_display( $autoship_order, $customer_id, $autoship_customer_id ) {

	$state_found = in_array(
		strtolower( $autoship_order['status'] ),
		array(
			'processing',
			'locked',
		),
	...
	if ( $state_found || 'no' === autoship_get_scheduled_order_upsell_carousel() ) {
		return;
	}

	$upsell_ids          = array();
	$order_items         = array();
	$order_by            = wc_review_ratings_enabled() ? 'rating' : 'rand';
	$disable_carousel_js = autoship_get_scheduled_order_upsell_disable_carousel_js();
	$args                = apply_filters(
		'autoship_upsell_display_args',
		array(
			'posts_per_page' => - 1,
			'orderby'        => $order_by,
			'order'          => 'desc',
	...
	// Get upsell products from the SO items metadata.
	if ( isset( $autoship_order['scheduledOrderItems'] ) ) {
		foreach ( $autoship_order['scheduledOrderItems'] as $key => $item ) {
			$order_items[] = $item['productId'];
			$upsells       = get_post_meta( $item['productId'], '_upsell_ids', true );
			if ( is_array( $upsells ) ) {
	...
	// Uses array_diff to remove products that are part of SO.
	$upsell_ids = apply_filters( 'autoship_scheduled_order_upsell_ids', array_diff( $upsell_ids, $order_items ) );

	return autoship_render_template(
		apply_filters(
			'autoship_scheduled_order_display_template',
			'scheduled-orders/order-upsell-products',
			$customer_id,
			$autoship_customer_id
		),
		array(
			...
			'disable_carousel_js'  => $disable_carousel_js,
		)
	);
}
 ...
 * @return string The html output
 */
function autoship_scheduled_order_item_qty_modify_for_cycle_item( $product_quantity, $item, $scheduled_item ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 *
 * @param array $item The scheduled order item.
 * @param array $scheduled_item The scheduled order raw item data.
 ...
 * @return void
 */
function autoship_scheduled_order_item_cycle_notice( $item, $scheduled_item, $autoship_order, $customer_id, $autoship_customer_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed

	if ( $item['cycle_item'] && $item['valid_for_cycle'] && isset( $item['max_cycles'] ) ) {

		$single_notice = apply_filters( 'autoship_scheduled_order_item_single_cycle_notice', __( '1 time only.', 'autoship' ), $item );

		$multiple_notice = apply_filters(
			'autoship_scheduled_order_item_multiple_cycle_notice',
			// translators: %d is the number of cycles remaining.
		...
		$notice = ( $item['max_cycles'] - $item['cycles'] ) > 1 ? $multiple_notice : $single_notice;

		if ( ( $item['max_cycles'] !== $item['cycles'] ) && ( $item['max_cycles'] > $item['cycles'] ) && ( $item['max_cycles'] >= $item['min_cycles'] ) ) {
			echo apply_filters( 'autoship_scheduled_order_item_cycle_notice', sprintf( '%s %s %s', '<span class="autoship-item-notice"><i class="dashicons dashicons-calendar-alt"></i>', $notice, '</span>' ), $notice, $item ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
}
...
function autoship_scheduled_order_item_stock_notice_display( $item, $scheduled_item, $autoship_order, $customer_id, $autoship_customer_id ) {
	if ( 'instock' !== $item['stock_status'] ) {
		echo apply_filters( 'autoship_scheduled_order_item_stock_notice', sprintf( '<span class="autoship-item-notice"><i class="dashicons dashicons-warning"></i>%s</span>', __( 'Out of Stock', 'autoship' ) ), $item, $scheduled_item, $autoship_order, $customer_id, $autoship_customer_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
 ...
 * @param int   $customer_id The woocommerce customer id.
 * @param int   $autoship_customer_id The autoship customer id.
 * @param array $autoship_order_items The autoship order items array.
 *
 * @return string The html output
 */
...
function autoship_scheduled_order_shipping_rate_form_display( $autoship_order, $customer_id, $autoship_customer_id ) {
	...
	<?php
		echo autoship_render_template( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			apply_filters(
				'autoship_scheduled_order_display_template',
...
function autoship_scheduled_order_payment_form_add_method_link( $autoship_order, $customer_id, $autoship_customer_id ) {
	...
	echo apply_filters( 'autoship_scheduled_order_payment_form_add_method_link', sprintf( '<div class="add-payment-method-link"><a href="%1$s">%2$s</a></div>', $url, __( 'Add New Payment Method', 'autoship' ) ), $autoship_order, $customer_id, $autoship_customer_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
...
function autoship_edit_scheduled_order_payment_form_display_action() {
	...
	?>
	<div class="action-info"><a href="#" class="<?php echo esc_attr( $skin['action_btn'] ); ?> activate-action" data-toggle-text="<?php esc_html_e( 'Cancel Edit Payment Method', 'autoship' ); ?>" data-target=".autoship-edit-scheduled-order-payment-form"><?php esc_html_e( 'Edit Payment Method', 'autoship' ); ?></a></div>
	<?php
}
...
function autoship_edit_scheduled_order_schedule_form_display_action( $autoship_order, $customer_id, $autoship_customer_id ) {
	...
	<div class="action-info main-actions">
		...
		<a href="#" class="<?php echo esc_attr( $skin['action_btn'] ); ?> activate-action"
			data-toggle-text="<?php esc_html_e( 'Cancel Edit Schedule', 'autoship' ); ?>"
			data-target=".autoship-edit-scheduled-order-schedule-form"><span
					class="<?php echo esc_attr( $skin['action_span'] ); ?>"><?php esc_html_e( 'Edit Schedule', 'autoship' ); ?></span></a>
		...
		if ( ! empty( $actions ) ) {
			foreach ( $actions as $key => $action ) {
				...
				<a title="<?php echo esc_html( $action['title'] ); ?>" href="<?php echo esc_url( $action['url'] ); ?>"
					class="<?php echo esc_attr( $skin['action_btn'] ); ?> <?php echo sanitize_html_class( strtolower( $key ) ); ?>"
					data-autoship-order="<?php echo esc_attr( $autoship_order['id'] ); ?>" data-autoship-action="<?php echo esc_attr( $key ); ?>"
					data-autoship-view="order">
					<span class="<?php echo esc_attr( $skin['action_span'] ); ?>">
						<?php
						esc_html_e( $action['name'] ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
						?>
					</span>
				</a>
				<?php
 ...
 * @return void
 */
function autoship_shipping_rate_form_close_modal_display_action( $autoship_order, $customer_id, $autoship_customer_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	?>

	<button class="button autoship-action-btn cancel">
		<?php
		echo apply_filters( 'autoship_scheduled_order_edit_preferred_shipping_rate_form_action_cancel_label', __( 'Cancel', 'autoship' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		?>
	</button>
 ...
 * @return void
 */
function autoship_shipping_rate_form_refresh_rates_action( $autoship_order, $customer_id, $autoship_customer_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	?>
	<div class="refresh-rates-link">
		<a href="<?php echo esc_url( autoship_refresh_scheduled_order_url( $autoship_order['id'] ) ); ?>">
			<?php echo esc_html( __( 'Refresh Rates', 'autoship' ) ); ?>
		</a>
	</div>
	<?php
}
...
function autoship_filter_notices_for_all_orders( $notice, $autoship_order ) {
	...
	return apply_filters( 'autoship_filter_notices_for_all_orders', ( 'Active' === $status || 'Pending' === $status ) ? '' : $notice, $status, $notice );
}
...
function autoship_scheduled_order_address_edit_link_adjustment_display( $link, $autoship_order, $context = 'view' ) {
	return 'edit' === $context ?
		// translators: %1$s is the text to toggle the link, %2$s is the text to display.
		sprintf( __( '<a href="#" class="activate-action slim-autoship-action" data-toggle-text="%1$s" data-target=".autoship-edit-scheduled-order-shipping-address-form">%2$s</a>', 'autoship' ), esc_html__( 'Cancel Edit Shipping Address', 'autoship' ), esc_html__( 'Update your shipping address.', 'autoship' ) ) :
		$link;
}
...
function autoship_filter_schedulable_products_use_wc_data() {
	$use_wc_data = 'yes' === autoship_get_legacy_support_qpilot_products_data() ? false : true;
 ...
 *
 * @param string $reply_type The type of reply to return.
 *
 * @return string The html output
 */
function autoship_get_schedulable_products_script_data( $reply_type = 'output_return' ) {
	...
	if ( is_wp_error( $all_products ) ) {
		...
		wc_add_notice( __( 'An issue was encountered while trying to retrieve the available products. <br/>Additional Details: ', 'autoship' ) . $all_products->get_error_message(), 'error' );
		$all_products = '';
	}
	...
	if ( 'output_return' === $reply_type ) {
		autoship_print_scripts_data( array( 'autoship_available_products_data' => $all_products ) );

		return $all_products;
	} elseif ( 'output' === $reply_type ) {
		autoship_print_scripts_data( array( 'autoship_available_products_data' => $all_products ) );
	} elseif ( 'json' === $reply_type ) {
		return wp_json_encode( array( 'autoship_available_products_data' => $all_products ) );
	}
 ...
 *
 * @param array $available_products The currently available products.
...
function autoship_get_schedulable_products_display_labels( $available_products, $products ) {
	...
	foreach ( $products as $product ) {
		...
		$available_products[ $wc_product->get_id() ]['item_price'] = $wc_product->get_sale_price() ? autoship_get_adjusted_product_item_price( $wc_product->get_sale_price(), $wc_product->get_id() ) : $available_products[ $wc_product->get_id() ]['item_original_price'];

		$overrides = autoship_get_product_custom_data_values( $wc_product );

		$available_products[ $wc_product->get_id() ]['label'] = apply_filters(
			'autoship_scheduled_order_form_item_add_display_name',
			$overrides['title'] . ' ' . autoship_get_formatted_price(
				$available_products[ $wc_product->get_id() ]['item_price'],
				array(
...
if ( ! function_exists( 'autoship_filter_schedulable_products_data' ) ) {
	 ...
	 *
	 * @param array $products An Array of stdClass objects or WC_Product objects.
	 *
	 * @return array The filtered array
	 */
...
function autoship_get_add_item_html() {
	...
	$product    = ! isset( $_POST['product'] ) || empty( $_POST['product'] ) ? null : $_POST['product']; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	$product_id = ! isset( $_POST['product_id'] ) || empty( $_POST['product_id'] ) ? null : absint( $_POST['product_id'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	if ( ! isset( $product ) && ! isset( $product_id ) ) {
		...
		autoship_ajax_result(
			...
			array(
				...
				'notice_target' => apply_filters( 'autoship_add_item_via_ajax_msg_html_target', '.scheduled-order-add-ons-msg', 'error' ),
			)
		);
	}
	...
	if ( ! isset( $product ) ) {
		...
		if ( is_wp_error( $product ) ) {
			...
			$notice_content = apply_filters( 'autoship_add_item_via_ajax_failure_msg', __( 'Error Adding Product Due to Invalid Call. <br/>Additional Details: ' . $product->get_error_message(), 'autoship' ), 404, $product ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	autoship_render_template(
		apply_filters( 'autoship_scheduled_order_display_template', 'scheduled-orders/order-ajax-add-item-form', $product['id'], $product ),
		array(
	...
	// translators: %s is the Scheduled Order label.
	$notice_content = apply_filters( 'autoship_add_item_via_ajax_success_msg', sprintf( __( 'Click Update Items to Save your %s and see your new total amount.', 'autoship' ), $label ), 200, $product );
...
function autoship_confirm_delete_schedule_notice_html() {
	...
	if ( ( ! isset( $_POST['order_id'] ) || empty( $_POST['order_id'] ) ) && ( ! isset( $_POST['order_view'] ) || empty( $_POST['order_view'] ) ) && ( ! isset( $_POST['confirm'] ) || empty( $_POST['confirm'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

		// translators: %s is the Scheduled Order label.
		$notice_content = apply_filters( 'autoship_delete_scheduled_order_via_ajax_failure_msg', __( sprintf( 'Error %s Due to Invalid Call.', autoship_translate_text( 'Scheduled Order' ) ), 'autoship' ), 400, '' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText

		autoship_ajax_result(
			400,
			array(
				'success'       => 'error',
				'notice'        => apply_filters( 'autoship_delete_scheduled_order_via_ajax_msg_html', sprintf( '<div class="autoship-msg woocommerce-%1s">%2s</div>', 'error', $notice_content ), 'error', $notice_content ),
				'notice_target' => apply_filters( 'autoship_delete_scheduled_order_via_ajax_msg_html_target', '.scheduled-order-add-ons-msg', 'error' ),
			)
		);
	}

	$scheduled_order = $_POST['order_id']; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	$view            = $_POST['order_view']; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

	$url    = autoship_get_scheduled_order_delete_url( $scheduled_order, 'confirmed' );
	$cancel = autoship_get_scheduled_orders_url();

	// Figure out where the notice will be added in UI.
	$notice_target = apply_filters( 'autoship_confirm_delete_schedule_notice_html_target', 'orders' === $view ? '#row-' . $scheduled_order : '.schedule-summary', $view, $scheduled_order );

	// Figure out where the notice will be added in UI.
	$notice_wrapper_target = apply_filters( 'autoship_confirm_delete_schedule_notice_html_wrapper_target', 'orders' === $view ? 'tr.autoship-notice-row' : '.autoship-notice-block', $view, $scheduled_order );
	...
	$notice = apply_filters(
		'autoship_confirm_delete_schedule_link_label',
		sprintf(
			// translators: %1$s is the action request, %2$s is the Scheduled Order label, %3$d is the Scheduled Order id, %4$s is the notice wrapper target, %5$s is the delete url, %6$s is the notice wrapper target, %7$s is the cancel url.
			__( '<span class="action-request">Are you sure you want to Delete %1$s #%2$d?</span> <a class="confirm-action" data-notice-wrapper="%3$s" href="%4$s">Confirm</a> or <a class="cancel-action" data-notice-wrapper="%5$s" href="%6$s">Cancel</a>', 'autoship' ),
			$label,
			$scheduled_order,
			$notice_wrapper_target,
			$url,
			$notice_wrapper_target,
			$cancel
		),
	...
	autoship_ajax_result(
		...
		array(
			'success'       => 'success',
			'notice'        => apply_filters( 'autoship_confirm_delete_via_ajax_msg_html', $notice_content, 'alert', $notice, $view, $scheduled_order ),
			'notice_target' => apply_filters( 'autoship_confirm_delete_via_ajax_msg_html_target', $notice_target, $notice, $view, $scheduled_order ),
		)
	);
	die();
}
 ...
 * @return string The html output
 */
function autoship_confirm_delete_schedule_notice_orders_html_wrapper( $notice, $scheduled_order ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed

	$columns = autoship_get_my_account_scheduled_orders_columns();
	ob_start();
	?>
	<tr class="autoship-notice-row woocommerce-orders-table__row">
		<td class="autoship-order-notice action-notice" data-title="<?php echo esc_attr( __( 'Notice', 'autoship' ) ); ?>" colspan="<?php echo esc_attr( count( $columns ) - 1 ); ?>">
			<p role="alert">
 ...
 * @return string The html output
 */
function autoship_confirm_delete_schedule_notice_order_html_wrapper( $notice, $scheduled_order ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	ob_start();
	?>
	<div class="autoship-notice-block">

		<div class="autoship-order-notice action-notice" data-title="<?php echo esc_attr( __( 'Notice', 'autoship' ) ); ?>">
			<p role="alert">
				<?php echo wp_kses_post( $notice ); ?>
			</p>
		</div>
...
/**
 * View Scheduled Order Items Template
 *
 ...
 */
add_action( 'autoship_before_scheduled_order_edit', 'autoship_scheduled_order_header_template_display', 8, 3 );
add_action( 'autoship_before_scheduled_order_edit', 'autoship_scheduled_order_upsell_products_template_display', 9, 3 );
...
/**
 * View Scheduled Order Item Form
 *
...
/**
 * View Schedule order Ajax Add Item Form (order-ajax-add-item-form.php)
 *
...
/**
 * View Scheduled Order Payment Form (scheduled-order-payment-form.php)
 *
...
/**
 * View Scheduled Order Form (order-schedule-form.php)
 *
...
/**
 * View Scheduled Order Shipping Rate Form (order-shipping-rate-form.php)
 *
...
/**
 * View Scheduled Order Shipping Rate Form Modal (order-shipping-rate-form.php)
 *
