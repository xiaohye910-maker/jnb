<?php
/**
 * Contains the functions regarding the utilities of the plugin.
 *
 * @package Autoship
 * @since 1.0.0
 */
 ...
 */
function autoship_supported_wp_version( $version, $comparison = '>=' ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 *
 * @param stdClass $item a stdClass object.
 *
 * @return array   The converted version.
 */
function autoship_convert_object_to_array( $item ) {
	if ( is_object( $item ) ) {
 ...
 * @param array  $items     An array to insert in to.
 * @param string $new_key   The key to insert.
 * @param mixed  $new_value A value to insert.
 *
 * @return array|bool The new array if the key exists, FALSE otherwise.
 */
function autoship_array_insert_before( $key, array &$items, $new_key, $new_value ) {
 ...
 * @return array|bool The new array if the key exists, FALSE otherwise.
 */
function autoship_array_insert_after( $key, array &$items, $new_key, $new_value ) {
	if ( array_key_exists( $key, $items ) ) {
		$new = array();
		foreach ( $items as $k => $value ) {
			$new[ $k ] = $value;
...
/**
 * Recursively merges two arrays - mimics wp_parse_args for multi-dimensional arrays.
 *
 * @param array $a The key to insert after.
 * @param array $b An array to insert in to.
 *
 * @return array|bool The new array if the key exists, FALSE otherwise.
 */
 ...
 *
 * @param string|DateTime $date The date string to convert or DateTime object to use.
 * @param string          $format The format of the local date.
 *
...
function autoship_get_formatted_local_date( $date, $format = '' ) {
	...
	// Change to international date to comply with current language.
	$timestamp      = $date->getTimestamp(); // Convert to timestamp.
 ...
 *
 * @param string       $date The date string to convert. If empty current date time returned.
 * @param string       $from_format The from format.
...
function autoship_get_utc_datetime( $date = null, $from_format = null, $to_format = '' ) {
	...
	if ( ! $date || empty( $date ) ) {
		...
		// Get the Date Time object if not passed through.
		if ( ! ( $date instanceof DateTime ) ) {
			$date = isset( $from_format ) ? DateTime::createFromFormat( $from_format, $date, autoship_get_local_timezone() ) : DateTime::createFromFormat( 'U', strtotime( $date ), autoship_get_local_timezone() );
		}
...
function autoship_force_datetime_into_processing_window( $date, $format = null ) {
	...
	$window = autoship_get_api_processing_window();
 ...
 * @param bool   $force_ptime When set to True it validates and adjusts the time component to be in processing window.
 *
...
function autoship_calculate_relative_next_occurrence_date( $value, $option, $basedate = null, $format = '' ) {
	...
	switch ( strtolower( $option ) ) {
		...
		case 'dayoftheweek':
			...
			if ( strtolower( $basedate->format( 'l' ) ) === $dayofweek ) {
...
function autoship_get_frequency_display_name( $frequency_type, $frequency ) {
	...
	switch ( $frequency_type ) {
		case 'Days':
			// translators: %d is the frequency in days.
			$display_name = sprintf( __( 'Every %d days', 'autoship' ), $frequency );
			break;
		case 'Weeks':
			// translators: %d is the frequency in weeks.
			$display_name = sprintf( __( 'Every %d weeks', 'autoship' ), $frequency );
			break;
		case 'Months':
			// translators: %d is the frequency in months.
			$display_name = sprintf( __( 'Every %d months', 'autoship' ), $frequency );
			break;
		case 'DayOfTheWeek': // If the frequency is less than 8 then it is a day of the week.
			if ( $frequency < 8 ) {
				$days = array(
					__( 'Sunday', 'autoship' ),
					__( 'Monday', 'autoship' ),
			...
			// Fall through to DayOfTheMonth if frequency is greater than 7.
		case 'DayOfTheMonth':
			if ( ( ( $frequency % 100 ) >= 11 ) && ( ( $frequency % 100 ) <= 13 ) ) {
				...
				// translators: %d is the frequency in days.
				$display_name = sprintf( __( 'The %1$d%2$s of every month', 'autoship' ), $frequency, $suffixes[ $frequency % 10 ] );
				break;
			}
		default:
			// translators: %s is the frequency type and %d is the frequency.
			$display_name = sprintf( __( '%1$s:%2$d', 'autoship' ), $frequency_type, $frequency );
	}
...
function autoship_include_template( $template, $vars = array() ) {
	...
	// Find theme template.
	$theme_template = apply_filters( 'autoship_theme_template', get_stylesheet_directory() . '/' . Autoship_Plugin_Folder_Name . '/templates/' . $template . '.php', $template, $vars );
	if ( file_exists( $theme_template ) ) {
		// Include theme template.
		include $theme_template;
	} else {
		// Include plugin template.
		$plugin_template = apply_filters( 'autoship_plugin_template', Autoship_Plugin_Dir . '/templates/' . $template . '.php', $template, $vars );
...
/**
 * Retrieves the current export file directory or url.
 *
 ...
 * @uses wp_upload_dir()
 */
function autoship_get_export_directory( $type = 'basedir' ) {
	$paths = wp_upload_dir( null, false );

	return apply_filters( 'autoship_export_directory_path', $paths[ $type ] . '/autoship-cloud-exports/' );
}
 ...
 */
function autoship_maybe_setup_export_directory( $filename ) {
	...
	if ( ! is_dir( $dirname ) ) {
		...
		if ( $success && ! file_exists( trailingslashit( $path ) . '.htaccess' ) ) {
			$file_handle = fopen( trailingslashit( $path ) . '.htaccess', 'w' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
			if ( $file_handle ) {
				fwrite( $file_handle, 'deny from all' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
		...
		if ( $success && ! file_exists( trailingslashit( $path ) . 'index.html' ) ) {
			$file_handle = fopen( trailingslashit( $path ) . 'index.html', 'w' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
			if ( $file_handle ) {
		...
		if ( ! $success ) {
			autoship_log_entry( __( 'Autoship Cloud Export', 'autoship' ), __( 'Autoship Cloud Export: Failed to create or open the export directory.', 'autoship' ) );
 ...
 * @param array  $fields File headers.
 *
 * @return string Name of the file
 * @throws Exception Thrown on error.
 */
function autoship_bulk_export_create_file( $filename, $fields ) {
	...
	try {
		...
		$created = autoship_maybe_setup_export_directory( $filename );
		...
		$file = fopen( $path . $filename, 'w+' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
		...
		if ( 'directory-creation-failed' === $e->getMessage() ) {
			// translators: %s is the path to the directory.
			return new WP_Error( 'directory-creation-failed', sprintf( esc_html( __( 'The Directory at %s can\'t be created! Please check permissions.', 'autoship' ) ), $path ) );
		}

		if ( 'column-header-creation-failed' === $e->getMessage() ) {
			return new WP_Error( 'column-header-creation-failed', __( 'Writing Column Headers to CSV File failed.', 'autoship' ) );
		}

		if ( 'file-creation-failed' === $e->getMessage() ) {
			// translators: %1$s is the filename and %2$s is the path.
			return new WP_Error( 'file-creation-failed', sprintf( esc_html( __( 'CSV File %1$s can\'t be opened and/or created! Please check rights for %2$s.', 'autoship' ) ), $filename, $path ) );
		}

		return new WP_Error( 'general-file-creation-failed', __( 'General Creating / Reading Export File Error.', 'autoship' ) );

	}
	// Hook here to retreive file.
	do_action( 'autoship_set_latest_export_file', $filename );
