<?php
/**
 * QuickLink Scanner Detected Template (Shopify-aligned).
 *
 * H-2: Displays human verification message when an email scanner is detected.
 * This template can be overridden by copying it to:
 * yourtheme/autoship/quicklinks/scanner-detected2.php
 *
 * @package Autoship
 * @since   3.2.0
 *
 * @var string      $site_name             The site name.
 * @var string|null $custom_logo_url       Optional custom logo URL from QPilot.
 * @var string|null $custom_stylesheet_url Optional custom stylesheet URL from QPilot.
 */
...
<head>
    ...
    <title><?php esc_html_e( 'Human Verification Required', 'autoship' ); ?> - <?php echo esc_html( $site_name ); ?></title>
...
<script>
    // If this is a real browser with JavaScript enabled, set a flag
    // and reload after 2 seconds to bypass scanner detection
    (function() {
        // Check if we've already tried reloading
        var hasReloaded = sessionStorage.getItem('scanner_reload_attempted');

        if (!hasReloaded && navigator.userAgent && !navigator.webdriver) {
            // Set flag to prevent reload loop
            sessionStorage.setItem('scanner_reload_attempted', 'true');

            // Show message to user
            console.log('Real browser detected. Reloading in 2 seconds to bypass scanner detection...');
