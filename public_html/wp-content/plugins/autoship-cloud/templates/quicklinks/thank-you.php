<?php
/**
 * QuickLink Thank You Template (v2 - Shopify-aligned styling).
 *
 * Displays success message after QuickLink action execution.
 * This template matches the Shopify QuickLinks styling.
 *
 * This template can be overridden by copying it to:
 * yourtheme/autoship/quicklinks/thank-you2.php
 *
 * @package Autoship
 * @since   3.2.0
 *
 * @var string      $message         The success message to display.
 * @var int|null    $action_type     The action type constant (optional).
 * @var string|null $action_name     The action name - Resume, Pause, etc. (optional).
 * @var int|null    $order_id        The scheduled order ID (optional).
 * @var string|null $status          The subscription status after action (optional).
 * @var string|null $next_occurrence The next occurrence date in UTC (optional).
 */
...
$action_name     = isset( $action_name ) ? $action_name : null;
$order_id        = isset( $order_id ) ? $order_id : null;
$status          = isset( $status ) ? $status : __( 'Updated', 'autoship' );
$next_occurrence = isset( $next_occurrence ) ? $next_occurrence : null;

// Format next occurrence date if available and not in the past.
$formatted_next_date = null;
if ( $next_occurrence ) {
    try {
        $date_obj = new DateTime( $next_occurrence, new DateTimeZone( 'UTC' ) );
        $now      = new DateTime( 'now', new DateTimeZone( 'UTC' ) );

        // Only display if the date is in the future.
        if ( $date_obj > $now ) {
            $formatted_next_date = $date_obj->format( 'F j, Y' );
        }
    } catch ( Exception $e ) {
        $formatted_next_date = null;
    }
...
// Action-specific footer messages.
$footer_messages = array(
        'Resume'     => __( 'Your subscription will resume with the next scheduled delivery.', 'autoship' ),
        'Pause'      => __( 'Your subscription is now paused. No charges will occur until you resume.', 'autoship' ),
        'ProcessNow' => __( 'We are processing your order now. Check your email for order confirmation and tracking details.', 'autoship' ),
        'Reactivate' => __( 'Welcome back! Your subscription is now active again.', 'autoship' ),
);
$footer_message = isset( $footer_messages[ $action_name ] ) ? $footer_messages[ $action_name ] : __( 'You can manage your subscription anytime through your account dashboard.', 'autoship' );
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?php echo esc_html( sprintf( __( 'Action Successful - %s', 'autoship' ), $site_name ) ); ?></title>
...
<div class="success-card">
    ...
    <div class="card-content">
        ...
        <div class="merchant-logo-section">
            ...
            <?php else : ?>
                <?php
                // Fallback: Check for WordPress theme custom logo.
                ...
                if ( $theme_logo_id ) :
                    ...
                    <div class="merchant-name"><?php echo esc_html( $site_name ); ?></div>
                <?php else : ?>
                    <!-- No logo: show store name in larger font -->
        ...
        <div class="details-box">
            <div class="details-header">
                <?php esc_html_e( 'Subscription Details', 'autoship' ); ?>
            </div>

            <div class="details-row">
                <span class="details-label"><?php esc_html_e( 'Status:', 'autoship' ); ?></span>
                <span class="details-value"><?php echo esc_html( $status ); ?></span>
            </div>

            <?php if ( $formatted_next_date ) : ?>
                <div class="details-row">
                    <span class="details-label"><?php esc_html_e( 'Next Occurrence Date:', 'autoship' ); ?></span>
                    <span class="details-value"><?php echo esc_html( $formatted_next_date ); ?></span>
                </div>
            <?php endif; ?>

            <?php if ( $order_id ) : ?>
                <div class="details-row">
                    <span class="details-label"><?php esc_html_e( 'Order ID:', 'autoship' ); ?></span>
                    <span class="details-value">#<?php echo esc_html( $order_id ); ?></span>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="action-button">
            <?php if ( is_user_logged_in() && function_exists( 'wc_get_page_permalink' ) ) : ?>
                <a href="<?php echo esc_url( trailingslashit( wc_get_page_permalink( 'myaccount' ) ) . 'scheduled-orders' ); ?>" class="btn btn-primary">
                    <?php esc_html_e( 'View My Subscriptions', 'autoship' ); ?>
                </a>
            <?php else : ?>
                <a href="<?php echo esc_url( home_url() ); ?>" class="btn btn-primary">
                    <?php esc_html_e( 'Continue Shopping', 'autoship' ); ?>
                </a>
            <?php endif; ?>
        </div>
        ...
        <div class="contact-section">
            <?php
            $support_email = apply_filters( 'autoship_quicklinks_support_email', 'support@qpilot.cloud' );
            printf(
            /* translators: %s: support email link */
                    esc_html__( 'Questions? Contact us at %s', 'autoship' ),
                    '<a href="mailto:' . esc_attr( $support_email ) . '" class="contact-link">' . esc_html( $support_email ) . '</a>'
            );
            ?>
        </div>
    </div>
