...
class AccountManager {
	 ...
	 */
	public static function login( string $user_email, string $user_password ) {
		$environment = new Environment();
		$installer   = new Installer();

		// Get the installation keys to send it to the registration endpoint.
		$install_id    = $installer->get_installation_id();
		$install_key   = $installer->get_installation_key();
		$site_name     = $environment->get_site_name();
		$site_url      = $environment->get_site_url();
		$site_redirect = $environment->get_oauth_redirect_url();

		// Attempt to create the user in the QPilot endpoint.
		$endpoint = $environment->get_api_url() . '/Quicklaunch/LoginUser';
		...
		$options = self::decode_credentials( $body );
		$stored  = self::store_options( $options );
	 ...
	 * @throws Exception When an error occurs while logging in.
	 */
	public static function register( string $user_email, string $user_password, string $user_phone ): void {
		$environment = new Environment();
		$installer   = new Installer();
		...
		// Attempt to create the user in the QPilot endpoint.
		$endpoint = $environment->get_api_url() . '/Quicklaunch/RegisterUser';
		$response = wp_remote_post(
			$endpoint,
			array(
				'headers' => array(
	 ...
	 */
	private static function decode_credentials( array $body ): array {
	 ...
	 * @return bool
	 */
	private static function store_options( array $options ): bool {
		if ( empty( $options ) ) {
			return false;
		}
		...
		foreach ( $options as $option_name => $option_value ) {
			...
			if ( ! $result && get_option( $option_name ) != $option_value ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseNotEqual
				$success = false;
			}
		}
