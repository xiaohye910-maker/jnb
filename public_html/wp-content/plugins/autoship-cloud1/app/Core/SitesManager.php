...
class SitesManager {
	 ...
	 */
	public static function get_user_sites( string $user_email, string $user_password ): array {

		$environment = new Environment();
		$response    = wp_remote_post(
			$environment->get_api_url() . '/AccessTokens/Login',
			array(
				'headers' => array(
		...
		$environment = new Environment();
		$store_url   = strtolower( wp_parse_url( $environment->get_site_url(), PHP_URL_HOST ) );
	 ...
	 */
	public static function connect_site(): int {

		// Verifies the presence of tokens and client id.
		...
		// Connect using the Autoship connection service.
		autoship_oauth2_connect_site();

		$site_id = intval( get_option( 'autoship_site_id', 0 ) );
		if ( $site_id <= 0 ) {
