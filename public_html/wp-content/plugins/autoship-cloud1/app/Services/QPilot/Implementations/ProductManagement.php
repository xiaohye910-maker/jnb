...
class ProductManagement implements ProductManagementInterface {
	...
	public function get_products_summary( ProductSummaryRequest $request ): ProductSummaryResponse {
		...
		$params = array(
			...
			'pageSize'  => $request->get_page_size(),
			'orderBy'   => $request->get_order_by(),
			'order'     => $request->get_order(),
			'search'    => $request->get_search(),
			'startDate' => $request->get_start_date(),
			'endDate'   => $request->get_end_date(),
			'active'    => $request->get_active(),
			'valid'     => $request->get_valid(),
		);

		$data = array(
			'externalIds' => $request->get_product_ids(),
		);
