...
function autoship_init_integration_test( bool $force = false ) {
	...
	try {
		 ...
		 */
		$result = $client->check_integration_status( $force );
...
function autoship_integration_statuses_check( $result = null, $log = true, $e_code = '520', $e_message = 'An Unknown Error was encountered.' ) {
	...
	$qpilot_errors = array();
	if ( isset( $result ) && ! $result->isSuccess && ! empty( $result->errors ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		foreach ( $result->errors as $error ) {
			$qpilot_errors[ $error->type ] = array(
				'code'        => $error->statusCode, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				'code_notice' => autoship_expand_http_code( $error->statusCode, 'desc' ), // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
	...
	if ( $log ) {
		...
		foreach ( $notices as $values ) {
			...
			if ( apply_filters( 'autoship_log_api_health_error_details', true, $values ) ) {
				...
				autoship_log_entry( __( 'Autoship Health Check', 'autoship' ), sprintf( __( '%1$s Additional API Error Details: %2$s', 'autoship' ), $values['msg'], $values['api'] ) );
			}
		}
	}
...
function autoship_admin_health_status_menu_bubble() {
	...
	// Check if the connection is healthy and if the current stamps are fresh else run the test integration.
	if ( ! autoship_check_integration_status_freshness() || ( 'healthy' !== $healthy ) ) {

		if ( ! FeatureManager::is_enabled( 'development_mode' ) ) {
			// New: enqueue async background job instead of immediate test.
			autoship_queue_integration_check();
		}
	}
...
function autoship_filter_qpilot_client_response_usermessages( $messages, $args ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	foreach ( $messages as $message ) {
		$filtered_notices[] = __( autoship_search_for_translate_text( $message ), 'autoship' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	}
...
function autoship_log_qpilot_client_response_errors( $args ) {
	autoship_log_entry( sprintf( '%d %s Request Error: %s', $args['code'], $args['callArgs']['method'], $args['response_error'] ), sprintf( 'Error Details: %s // Endpoint Details: %s', implode( ' ', $args['errors'] ), $args['callArgs']['endpoint'] ) );
}
...
if ( ! function_exists( 'autoship_queue_integration_check' ) ) {
	...
	function autoship_queue_integration_check( $force = false ) {
		...
		if ( ! function_exists( 'as_enqueue_async_action' ) ) {
			// Action Scheduler missing; fallback to immediate run to preserve behavior.
			autoship_init_integration_test();
			return;
		}
		...
		// Enqueue async job (runs as soon as possible).
		as_enqueue_async_action( $hook, array( 'forced' => (bool) $force ), 'autoship' );
	}
}

add_action(
	'autoship_run_integration_check',
	function ( $args ) { // phpcs:ignore
		// Skip on new installations.
		if ( function_exists( 'autoship_is_new' ) && autoship_is_new() ) {
			return;
		}

		// Executes the same logic as before (fires hooks & updates timestamps and health status).
		autoship_init_integration_test();

		// Track last run (optional for UI).
		update_option( '_autoship_last_integration_check_utc', time(), false );
	},
	10,
	1
);

// Admin AJAX to force a background integration check.
add_action(
	'wp_ajax_autoship_force_integration_check',
	function () { // phpcs:ignore
		...
		autoship_queue_integration_check( true );
