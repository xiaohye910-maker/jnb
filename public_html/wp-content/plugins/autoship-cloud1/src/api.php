...
/**
 * The functional code that handles the API integration.
 *
 * @package Autoship
 * @since 1.0.0
 */
...
if ( ! function_exists( 'qpilot_remote_post' ) ) {
	...
	function qpilot_remote_request( $endpoint, $args ) {
		...
		if ( Logger::is_logging_enabled() ) {
			$input_data = var_export( $args, true ); // phpcs:ignore

			Logger::trace( 'Autoship Request Tracer', "Requesting $endpoint with the following input data: $input_data" );
		}

		$response = ( ! isset( $args['method'] ) || 'GET' === $args['method'] ) ? wp_remote_get( $endpoint, $args ) : wp_remote_post( $endpoint, $args );
		...
		if ( is_wp_error( $response ) ) {
			...
			Logger::trace( 'Autoship Request Tracer', "Elapsed: $elapsed milliseconds Code: $error_code Endpoint: $endpoint. Error: $error_message" );
		...
		if ( Logger::is_tracing_enabled() ) {
			...
			$output_data = var_export( $response_data, true ); // phpcs:ignore

			Logger::trace( 'Autoship Request Tracer', "Requested $endpoint with the following response: $output_data" );
			Logger::trace( 'Autoship Request Tracer', "Elapsed: $elapsed milliseconds. Endpoint: $endpoint. " );
		}
...
function autoship_get_default_client( $scope = '' ) {
	...
	if ( ( $token_created_at + $token_expires_in - $one_week ) < time() ) {
		try {
			...
			autoship_log_entry(
				...
				// translators: %d is the error code, %s is the error message.
				sprintf( '%1$d Refresh token failed. Additional Details: %2$s', $e->getCode(), $e->getMessage() )
			);
		}
	}
	...
	if ( ! $admin && ( 'merchant' !== $scope ) && $customer_id ) {
		...
		try {
			...
			$client->set_token_auth( $token->tokenBearerAuth ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		} catch ( Exception $e ) {
...
function autoship_sync_invalid_product_codes() {
	...
	$error_codes         = array(
		...
		2 => array(
			...
			// translators: %s is the URL to the Products Report.
 ...
 * @param string $key Optional. The key to retrieve from the error code array.
 *
...
function autoship_get_merchants_url() {
	return apply_filters( 'autoship-merchants-url', 'https://merchants.qpilot.cloud' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
...
function autoship_get_api_url() {
	return apply_filters( 'autoship-api-url', 'https://api.qpilot.cloud' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
...
function autoship_qpilot_get_sitemeta( $sitemeta = array() ) {
	...
	$language_customizations = array_filter(
	...
	return apply_filters( 'autoship_qpilot_sitemeta_upsert', array_merge( $sitemeta, $defaults ), $sitemeta, $defaults );
}
...
function autoship_qpilot_orders_routes() {
	/**
	 * Registers a new REST API endpoint for QPilot
	 *
	...
	if ( class_exists( 'WC_REST_Orders_Controller' ) ) {
		...
		if ( ! empty( $scheduled_order_processing_id ) ) {
			...
			if ( ! empty( $orders ) ) {
				...
				return new WP_Error(
					'duplicate_order',
					__( 'Order has already been created from scheduled order processing.', 'autoship' ),
					array(
						'status' => 400,
						'order'  => wp_json_encode( array_values( $orders )[0]->get_data() ),
					)
				);
			}
		}
...
function autoship_qpilot_statuscheck_put_update( $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
	...
	if ( false === $result ) {
		...
		$response = new WP_REST_Response(
			array(
				'code'    => 'autoship_put_failure',
				'message' => 'Autoship PUT Update Failed. WordPress Update WP Options Record Failed.',
			)
		);
		...
		$response = new WP_REST_Response(
			array(
				'code'    => 'autoship_put_success',
				'message' => 'Autoship PUT Update Success',
			)
		);
...
function autoship_qpilot_statuscheck_post_update( $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
	...
	if ( false === $result ) {
		...
		$response = new WP_REST_Response(
			array(
				'code'    => 'autoship_post_failure',
				'message' => 'Autoship POST Update Failed. WordPress Update WP Options Record Failed.',
			)
		);
		...
		$response = new WP_REST_Response(
			array(
				'code'    => 'autoship_post_success',
				'message' => 'Autoship POST Update Success',
			)
		);
...
function autoship_qpilot_statuscheck_get_update( $request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
	...
	if ( false === $result ) {
		...
		$response = new WP_REST_Response(
			array(
				'code'    => 'autoship_get_failure',
				'message' => 'Autoship GET Update Failed. WordPress Update WP Options Record Failed.',
			)
		);
		...
		$response = new WP_REST_Response(
			array(
				'code'    => 'autoship_get_success',
				'message' => 'Autoship GET Update Success',
			)
		);
 ...
 *
 * @param array $data The data to update the product availability.
 *
...
function autoship_qpilot_availability_update( $data ) {
	if ( isset( $data['id'] ) && ! empty( $data['id'] ) && isset( $data['availability'] ) ) {
		...
		if ( ! $wc_product ) {
			...
			$response = new WP_REST_Response(
				array(
					'code'    => 'autoship_availability_update_failure',
					'message' => 'Autoship Update Availability Failed. The requested product could not be found or is no longer valid.',
				)
			);
			$response->set_status( 500 );
		...
		if ( 'simple' === $product_type || 'variation' === $product_type || 'variable' === $product_type ) {
			...
			$response = new WP_REST_Response(
				array(
					'code'    => 'autoship_availability_update_success',
					'message' => 'The supplied product was successfully updated.',
				)
			);
			...
			$response = new WP_REST_Response(
				array(
					'code'    => 'autoship_availability_update_failed',
					'message' => 'Autoship Update Availability Failed. The requested product was not a valid product type.',
				)
			);
			$response->set_status( 500 );
		}
