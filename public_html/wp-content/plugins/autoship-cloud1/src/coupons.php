<?php
/**
 * The functional code that handles the coupons integration.
 *
 * @package Autoship
 * @since 1.0.0
 */
...
/**
 * Adds the Autoship Coupon Usage Restriction Panels to the WooCommerce Coupons.
 */
...
/**
 * Saves the Autoship Coupon Usage Restriction Panels to the WooCommerce Coupons.
 *
...
function autoship_save_custom_coupon_fields( $post_id ) {
	...
	foreach ( $autoship_field_names as $name ) {
		$value = isset( $_POST[ $name ] ) ? sanitize_text_field( $_POST[ $name ] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslashed,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
 ...
 *
 * @param array $autoship_order The Autoship Order data array.
 *
...
/**
 * Checks if the supplied coupon is a valid format. Currently can't contain comma's and be longer than 20 characters.
 *
 * @param string $autoship_coupon_code The coupon code to validate.
 *
 * @return bool True if valid else false.
 */
...
function autoship_validate_and_remove_scheduled_order_coupon( $order_id, $coupon_code, $order = null ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = new WP_Error( 'Coupon Removal Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_search_scheduled_order_coupons( $page = 1, $search = null, $all = true ) {
	...
	try {
		...
		$notice = autoship_expand_http_code( $e->getCode() );
		$notice = new WP_Error( 'Coupon Search Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	if ( $all && ( $coupons->totalPages > $page ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
...
function autoship_validate_and_apply_scheduled_order_coupon( $order_id, $coupon_code, $order = null ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_expand_http_code( $e->getCode() );
			$notice = new WP_Error( 'Coupon Validation Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
 ...
 * @param stdClass|array $order The Autoship Order object or array.
 *
...
function autoship_apply_scheduled_order_coupon( $order_id, $coupon_code, $order = null ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_expand_http_code( $e->getCode() );
			$notice = new WP_Error( 'Coupon Update Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_get_coupon_type_by_code( $type, $val = false ) {
	...
	$codes = apply_filters(
		...
		array(
			...
			'ReduceProductPriceByPercentage' => 'percent',
 ...
 * @param array  $coupon The coupon data to use for creating the coupon.
 *
 ...
 */
function autoship_create_coupon( $code, $coupon = array() ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_expand_http_code( $e->getCode() );
			$notice = new WP_Error( 'Coupon Creation Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_delete_coupon( $coupon_id ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Coupon Delete Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
...
function autoship_get_coupon_by_code( $code ) {
	...
	try {
		...
		if ( 404 === $e->getCode() ) {
			...
			$notice = autoship_expand_http_code( $e->getCode() );
			$notice = new WP_Error( 'Coupon Retrieval Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
			autoship_log_entry( __( 'Autoship Scheduled Order Coupons', 'autoship' ), sprintf( 'Retrieving Coupon by Code Failed. Additional Details: Error Code %s - %s', $e->getCode(), $e->getMessage() ) );
		}
 ...
 */
function autoship_qpilot_orders_update_via_rest_convert_fees_to_coupons( $request, $original_request ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed,Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	foreach ( $coupons as $key => $coupon ) {
		...
		if ( ! empty( $type ) ) {
			...
			$coupon_data = array(
				...
				// translators: %d is the coupon ID.
				...
				foreach ( $request['line_items'] as $item ) {
					if ( isset( $item['meta_data'] ) && is_array( $item['meta_data'] ) ) {
						foreach ( $item['meta_data'] as $meta ) {
							if ( ( isset( $meta['key'] ) && '_qpilot_scheduled_order_item' === $meta['key'] ) && isset( $meta['value']['salePrice'] ) ) {
			...
			if ( isset( $coupon['product'] ) && isset( $coupon['product']['externalId'] ) && ! in_array( $coupon['product']['externalId'], $excluded_ids, true ) ) {
				...
				if ( ! is_wp_error( $product_group ) ) {
					if ( isset( $product_group->products ) && is_array( $product_group->products ) ) {
						foreach ( $product_group->products as $prod ) {
							if ( ! in_array( $prod->id, $excluded_ids, true ) ) {
	...
	if ( empty( $fee_lines ) ) {
		...
		$request->set_param( 'fee_lines', $fee_lines );
	}
