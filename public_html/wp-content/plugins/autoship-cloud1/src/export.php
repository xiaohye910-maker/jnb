<?php
/**
 * The functional code that handles the exports.
 *
 * @package Autoship
 * @since 1.0.0
 */
...
function autoship_legacy_export_query_ids() {
	...
	$wc_autoship_schedules = $wpdb->get_results( $query, ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.NoCaching
 ...
 * Exports the WC Autoship Scheduled Orders to a CSV file.
 *
 * @param bool $ajax True if the call is an ajax call else false. Default True.
 * @param bool $email True if the email notification should be sent after export. Default True.
 *
...
function autoship_export_ajax_scheduled_orders( $ajax = true, $email = true ) {
	...
	try {
		...
		if ( ! is_dir( $dirname ) ) {
			$dir = mkdir( $dirname, 0755, true ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_mkdir
		...
		$log_file = fopen( $path . $logfilename, 'a' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
		...
		fwrite( $log_file, "\n" . $current_server_time . ' :: ' . $message ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
		...
		foreach ( $wc_autoship_schedules as $key => $single_schedule ) {
			...
			if ( ! isset( $customers[ $wc_customer_id ] ) ) {
				...
				if ( false === $user_exists ) {
					...
					// translators: %1$d is the customer ID, %2$d is the schedule ID.
					$message = sprintf( __( 'Customer %1$d could not be found or is invalid. This scheduled order #%2$d will not be included.', 'autoship' ), $wc_customer_id, $single_schedule['id'] );
					fwrite( $log_file, "\n" . $current_server_time . ' :: ' . $message ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite
					continue;
				}
			...
			if ( isset( $single_schedule['payment_token_id'] ) ) {
				...
				// translators: %d is the payment token ID.
				$description = $payment_method ? $payment_method->get_display_name() : sprintf( __( 'Payment Method with Token ID %d could not be found or is invalid.', 'autoship' ), $single_schedule['payment_token_id'] );
			}
			...
			if ( ! isset( $external_id ) ) {
				...
				if ( ! $product ) {
					++$invalids;
					// translators: %d is the external product ID.
			...
			$product_image_url   = ! $product ? '' : autoship_get_wc_product_image_url( $external_id );
			...
			$data['ScheduledOrder_Status']            = '1' === $single_schedule['autoship_status'] ? 'Active' : 'Paused';
		...
		fclose( $log_file ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose
		...
		if ( 'directory-creation-failed' === $e->getMessage() ) {
			$result = array(
				...
				'notice'  => sprintf( __( 'The Directory at %s can\'t be created! Please check permissions.', 'autoship' ), $path ),
			);
		}
		...
		if ( 'column-header-creation-failed' === $e->getMessage() ) {
			$result = array(
				'code'    => 500,
				'success' => false,
				'notice'  => __( 'Writing Column Headers to CSV File failed.', 'autoship' ),
			);
		}
		...
		if ( 'file-creation-failed' === $e->getMessage() ) {
			$result = array(
				'code'    => 500,
				'success' => false,
				// translators: %1$s is the filename, %2$s is the path to the file.
				'notice'  => sprintf( __( 'CSV File %1$s can\'t be opened and/or created! Please check rights for %2$s.', 'autoship' ), $filename, $path ),
			);
		}
		...
		if ( 'no-orders-to-export' === $e->getMessage() ) {
			$result = array(
				'code'    => 200,
				'success' => true,
				'notice'  => __( 'No Scheduled Orders to Export', 'autoship' ),
			);
		}
	...
	$result = array(
		'success'  => true,
		'code'     => 200,
		// translators: %1$d is the number of Scheduled Orders exported, %2$s is the filename.
		'notice'   => sprintf( __( '%1$d Scheduled Orders successfully exported to %2$s.', 'autoship' ), count( $wc_autoship_schedules ), $filename ),
		'filename' => $filename,
		'path'     => $path,
	);

	if ( count( $wc_autoship_schedules ) === $invalid_orders ) {
		// translators: %1$d is the number of Scheduled Orders, %2$s is the path to the log file.
		$result['notice'] = sprintf( __( 'None of the %1$d Scheduled Orders could be exported. See the log for additional details in %2$s', 'autoship' ), count( $wc_autoship_schedules ), $log_path );
	} elseif ( $invalid_orders ) {
		// translators: %1$d is the number of Scheduled Orders, %2$s is the filename, %3$d is the number of invalid Scheduled Orders.
		$result['notice'] = sprintf( __( '%1$d Scheduled Orders successfully exported to %2$s.  %3$d Scheduled Orders were missing or invalid and could not be exported.', 'autoship' ), count( $wc_autoship_schedules ), $filename, count( $invalid_orders ) );
	}
...
function autoship_download_csv() {
	...
	if ( ! file_exists( $path . $file ) ) {
		update_option( 'autoship_last_export', '' );
	} elseif ( current_user_can( 'export' ) && isset( $_GET['autoship_download_csv'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
