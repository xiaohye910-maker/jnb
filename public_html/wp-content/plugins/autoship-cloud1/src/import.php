<?php
/**
 * The functional code that handles the import integration.
 *
 * @package Autoship
 * @since 1.0.0
 */
 ...
 * @param bool $with_payment_method Optional. Whether to include the payment method data.
 *
...
function autoship_query_legacy_orders( $id = null, $batch_size = null, $with_payment_method = true ) {
	...
	$params = array();
	$query  = "SELECT schedules.*, tokens.gateway_id FROM {$wp}wc_autoship_schedules as schedules ";

	if ( $with_payment_method ) {
		$query .= "JOIN {$wp}woocommerce_payment_tokens as tokens ON schedules.payment_token_id = tokens.token_id ";
	}
	...
	$orders = $wpdb->get_results( empty( $params ) ? $query : $wpdb->prepare( $query, $params ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.NotPrepared
...
function autoship_query_legacy_order_items( $ids ) {
	...
	$items = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wp}wc_autoship_schedule_items WHERE schedule_id IN ( %s )", $range ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
 ...
 *
 * @param int  $last_id Optional. The last processed schedule id.
 * @param bool $ajax Optional. Whether this is an Ajax call.
 */
function autoship_ajax_import_schedule_data( $last_id = 0, $ajax = true ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	$last_id = isset( $_POST['last_id'] ) ? $_POST['last_id'] : $last_id; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	foreach ( $wc_autoship_schedules as $schedule_id => $schedule ) {
		...
		if ( is_wp_error( $result ) ) {
			array_push( $failed, array( $schedule['id'], $result->get_error_message(), wp_json_encode( $schedule ) ) );
		}
	}
	...
	$stats = autoship_query_legacy_upsert_stats(
		$wc_autoship_schedules,
		$last_id,
		array(
			'failed' => $failed,
			'result' => 1,
	...
	$status = ( 100 === $stats['complete_percent'] ) ? 1 : 0;

	$value_array = array(
		'schedule_type'    => $type,
		'response_array'   => wp_json_encode( $result ),
...
function autoship_ajax_import_product_settings() {
	...
	// Batch size.
	$all_products_count = $wpdb->get_var( "SELECT COUNT(*) FROM {$wp}posts WHERE post_type = 'product' AND post_status = 'publish'" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared

	// Get last exported product id.
	$last_id = isset( $_POST['last_id'] ) ? $_POST['last_id'] : 0; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

	// Get the products.
	$product_ids = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM {$wp}posts WHERE ID > %d AND post_type = 'product' AND post_status = 'publish' ORDER BY ID ASC", $last_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	$previous_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wp}posts WHERE ID <= %d AND post_type = 'product' AND post_status = 'publish'", $last_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared
 ...
 * Migrates Scheduled Orders from the Legacy tables to QPilot
 *
 * @param array $schedule The legacy WC Autoship schedule data.
 *
...
function autoship_import_create_scheduled_order( $schedule ) {
	...
	if ( isset( $schedule['payment_method'] ) ) {
		...
		if ( null !== $schedule['payment_method'] ) {
			...
			$payment_method = autoship_get_general_payment_method_customer_data(
	...
	$scheduled_order_data = autoship_generate_customer_shipping_data(
		$wc_customer,
		array(
			...
			'status'              => '1' === $schedule['autoship_status'] ? 'Active' : 'Paused',
	...
	return ! is_wp_error( $upsert ) && ! empty( $invalids ) ? new WP_Error( 'Create Scheduled Order Item Error', sprintf( 'Product(s) (%1$s) associated with this schedule no longer exist: Scheduled Order for ( %2$s ) was created without these products.', implode( ',', $invalids ), $wc_customer->get_email() ) ) : $upsert;
}
...
/**
 * Imports the payment integrations from the legacy WC Autoship to QPilot.
 *
 * @return void
 */
function autoship_import_payment_integrations() {
	...
	if ( empty( $site_id_str ) ) {
		...
		wp_redirect( admin_url( 'admin.php?page=migrations' ) ); // phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
		die();
	}
	...
	foreach ( $payment_integrations as $integration ) {
		$integration_types[ $integration->paymentMethodType ] = true; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
	}
	...
	if ( empty( $integration_types['Stripe'] ) ) {
		...
		if ( ! empty( $stripe_settings ) ) {
			$stripe_test_mode        = ! empty( $stripe_settings['authorize_only'] ) && 'yes' === $stripe_settings['authorize_only'];
	...
	if ( empty( $integration_types['PayPal'] ) ) {
		...
		if ( ! empty( $paypal_settings ) ) {
			$paypal_sandbox = ! empty( $paypal_settings['sandbox_mode'] ) && 'yes' === $paypal_settings['sandbox_mode'];
			$paypal_data    = array(
	...
	if ( empty( $integration_types['Braintree'] ) ) {
		...
		if ( ! empty( $bt_settings ) ) {
			$bt_sandbox = ! empty( $bt_settings['sandbox_mode'] ) && 'yes' === $bt_settings['sandbox_mode'];
			$bt_data    = array(
	...
	if ( empty( $integration_types['AuthorizeNet'] ) ) {
		...
		if ( ! empty( $anet_settings ) ) {
			$anet_sandbox = ! empty( $anet_settings['sandbox_mode'] ) && 'yes' === $anet_settings['sandbox_mode'];
			$anet_data    = array(
 ...
 *
 * @param int $payment_token_id The Payment Token ID to retrieve.
 */
function autoship_import_get_payment_row( $payment_token_id ) {
	...
	return $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
		$wpdb->prepare(
			"SELECT token.*, last_four.meta_value last_four, card_type.meta_value card_type, exp_month.meta_value exp_month, exp_year.meta_value exp_year FROM {$wp}woocommerce_payment_tokens as token  LEFT JOIN {$wp}woocommerce_payment_tokenmeta as last_four ON last_four.payment_token_id = token.token_id AND last_four.meta_key = 'last4' LEFT JOIN {$wp}woocommerce_payment_tokenmeta as card_type ON card_type.payment_token_id = token.token_id AND card_type.meta_key = 'card_type' LEFT JOIN {$wp}woocommerce_payment_tokenmeta as exp_month ON exp_month.payment_token_id = token.token_id AND exp_month.meta_key = 'expiry_month' LEFT JOIN {$wp}woocommerce_payment_tokenmeta as exp_year ON exp_year.payment_token_id = token.token_id AND exp_year.meta_key = 'expiry_year' WHERE token_id = %s", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$payment_token_id
		),
		ARRAY_A
	);
...
/**
 * Retrieves the Payment Method Data for the given Payment Token ID.
 *
 * @param int $payment_token_id The Payment Token ID to retrieve.
 *
...
/**
 * Retrieves the Authorize.Net Payment Method Data for the given Payment Token ID.
 *
 * @param int $payment_token_id The Payment Token ID to retrieve.
 *
 * @return array The Authorize.Net Payment Method Data.
 */
...
/**
 * Retrieves the Braintree Payment Method Data for the given Payment Token ID.
 *
 * @param int $payment_token_id The Payment Token ID to retrieve.
 *
 * @return array The Braintree Payment Method Data.
 */
...
/**
 * Retrieves the PayPal Payment Method Data for the given Payment Token ID.
 *
 * @param int $payment_token_id The Payment Token ID to retrieve.
 *
 * @return array The PayPal Payment Method Data.
 */
...
/**
 * Retrieves the CyberSource Payment Method Data for the given Payment Token ID.
 *
 * @param int $payment_token_id The Payment Token ID to retrieve.
 */
...
/**
 * Updates the import schedules in the database.
 *
 * @param array $result The result of the import.
 * @param array $value_array The value array containing schedule type, response array, complete percent, and status.
 *
...
function wc_autoship_update_db_import_schedules( $result, $value_array ) {
	...
	try {
		$delete_type_record = $wpdb->query( 'delete from ' . $table . " where schedule_type='" . $value_array['schedule_type'] . "'" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
		$update_table       = $wpdb->query( 'insert into ' . $table . "(schedule_type,response_array,complete_percent,status) values('" . $value_array['schedule_type'] . "','" . $value_array['response_array'] . "','" . $value_array['complete_percent'] . "','1')" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
	} catch ( Exception $e ) {
...
/**
 * Updates the test connection in the database.
 *
 * @param array $result The result of the import.
 * @param array $value_array The value array containing schedule type, response array, complete percent, and status.
 *
...
/**
 * Retrieves the import values from the database.
 *
 * @param string $schedule_type The schedule type to retrieve.
 *
 * @return array The import values.
 */
function get_import_values( $schedule_type ) {
	global $wpdb;
	$table = $wpdb->prefix . 'wc_autoship_import';
	try {
		$get_data = $wpdb->get_results( 'select * from ' . $table . " where schedule_type='" . $schedule_type . "' order by ID desc LIMIT 0,1" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
	} catch ( Exception $e ) {
		echo esc_html( $e->getMessage() );
	}
...
function format_date( $date ) {
	$new_date = date( 'F d, Y', strtotime( $date ) ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
...
function format_time( $date ) {
	$new_time = date( 'H:i', strtotime( $date ) ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date

	return $new_time;
}

/**
 * Checks if the import table exists in the database.
 *
 * @return int 1 if the table exists, 0 if it does not.
 */
function check_table_exist() {
	global $wpdb;
	$table = $wpdb->prefix . 'wc_autoship_import';
	if ( $wpdb->get_var( "SHOW TABLES LIKE '" . $table . "'" ) !== $table ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
		return 0;
	} else {
		return 1;
	}
