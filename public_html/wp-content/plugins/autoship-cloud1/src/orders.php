<?php
/**
 * The orders functions for the Autoship plugin.
 *
 * @package Autoship
 * @since 1.0.0
 */
...
function autoship_order_customer_dashboard_icon_column( $columns ) {
	...
	} elseif ( isset( $columns['order-date'] ) ) {
		$location = 'order-date';
	} elseif ( isset( $columns['order-number'] ) ) {
		$location = 'order-number';
	}

	$location = apply_filters( 'autoship_order_customer_dashboard_column_location', $location );

	$inserted = autoship_array_insert_after( $location, $columns, 'order-autoship-icon', '' );

	if ( ! empty( $location ) && ( ! empty( $inserted ) ) ) {
		$columns = $inserted;
	} else {
		$columns['order-autoship-icon'] = '';
	}
...
function autoship_order_customer_dashboard_icon_column_content( $order ) {
	...
	if ( ! empty( $external_data['origin_id'] ) ) {
		...
		// translators: %1$s: Scheduled Order, %2$d: Origin ID.
		$external_data['notices'][] = wc_sanitize_tooltip( sprintf( __( '%1$s %2$d', 'autoship' ), $scheduled_order_label, $external_data['origin_id'] ) );
		// translators: %s: Schedule.
	...
	} elseif ( ! empty( $external_data['scheduled_ids'] ) ) {

		$external_data['source'] = 'source';
		$external_data['class']  = 'scheduled';
		// translators: %1$s: Scheduled Order, %2$s: Scheduled Orders.
		$external_data['notices'][] = wc_sanitize_tooltip( sprintf( __( 'Created %1$s(s) %2$s', 'autoship' ), $scheduled_order_label, implode( ', ', $external_data['scheduled_ids'] ) ) );

		// Otherwise do we know if this is even a source or result?
	} elseif ( ! empty( $external_data['source'] ) ) {
	...
	printf( '<a href="%s"><mark class="order-origin-icon %s autoship-tips" data-tip="%s"><span></span></mark></a>', $scheduled_orders_url, $external_data['class'], apply_filters( 'autoship_populate_order_customer_dashboard_icon_tooltip', wp_kses_post( implode( '<br/>', $external_data['notices'] ) ), $external_data, $order ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
...
function autoship_customer_view_order_header_attribution( $order_id ) {
	...
	if ( ! empty( $external_data['origin_id'] ) ) {
		...
		$external_data['source'] = 'result';
		$external_data['class']  = 'created';
		// translators: %1$s: Scheduled Order, %2$d: Origin ID, %3$d: Scheduled Orders URL.
		$external_data['notice'] = sprintf( __( 'Created by %1$s <a href="%2$s">%3$d</a>', 'autoship' ), $scheduled_order_label, $scheduled_orders_url, $external_data['origin_id'] );
		...
		$external_data['source'] = 'source';
		$external_data['class']  = 'scheduled';
		// translators: %1$s: Scheduled Order, %2$s: Scheduled Orders URL, %3$s: Scheduled Order IDs.
		$external_data['notice'] = sprintf( __( 'Created %1$s(s) <a href="%2$s">%3$s</a>', 'autoship' ), $scheduled_order_label, $scheduled_orders_url, implode( ', ', $external_data['scheduled_ids'] ) );

		// Otherwise do we know if this is even a source or result?
	} elseif ( ! empty( $external_data['source'] ) ) {
		$external_data['class']  = 'source' === $external_data['source'] ? 'maybe-scheduled' : 'maybe-created';
		$external_data['notice'] = 'source' === $external_data['source'] ?
			// translators: %1$s: Scheduled Orders.
			sprintf( __( 'This order contains Items with associated Schedules and may have produced %1$s.', 'autoship' ), $scheduled_orders_label ) :
			// translators: %1$s: Scheduled Order.
			sprintf( __( 'This order might have been generated by a %1$s.', 'autoship' ), $scheduled_order_label );
	...
	echo apply_filters( 'autoship_populate_customer_view_order_header_attribution_html', sprintf( '<h3 class="autoship-order-attribution %s">%s</h3>', $external_data['class'], $external_data['notice'] ), $external_data, $order ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
...
function autoship_register_edit_order_metaboxes() {
	...
	add_meta_box( 'autoship_assign_scheduled_orders', 'Autoship Link Scheduled Orders', 'autoship_add_scheduled_order_ids_metabox', $screen );
}
...
function autoship_add_scheduled_order_ids_metabox( $post ) {
	$theorder = ( $post instanceof WP_Post ) ? wc_get_order( $post->ID ) : $post;
	...
	$assigned_notice = empty( $val ) ?
		__( '<strong>No Scheduled Order IDs assigned.</strong>' ) : // phpcs:ignore WordPress.WP.I18n.NoHtmlWrappedStrings
		// translators: %s: Assigned Scheduled Order IDs.
		sprintf( __( 'Scheduled Order IDs <strong>%s</strong> assigned.' ), $val );
	...
	<div id="autoship-scheduled-order-ids-options-main">
		...
		<div id="scheduled-order-ids-options">
			...
			<div class="scheduled-order-ids-options-form-action">
				...
				<p><?php echo esc_html( __( 'The following Autoship Scheduled Order IDs are assigned to this WooCommerce Order. Click on the "Update Scheduled Order ID(s)" button to change the assigned ids.', 'autoship' ) ); ?></p>

				<p><?php echo wp_kses_post( $assigned_notice ); ?></p>

				<div class="action-button-bar">
					<div class="action-buttons">
						<a class="autoship_hide_show_toggler autoship-modal-trigger button button-primary"
							data-target=".scheduled-order-ids-options-form, .scheduled-order-ids-options-form-action"
							data-modal-toggle="#autoship_order_assigned_ids_warning">Update Scheduled Order ID(s)</a>
					</div>
				</div>
			...
			<div class="scheduled-order-ids-options-form" style="display:none">
				...
				<p><?php echo esc_html( __( 'Use the Autoship Scheduled Order IDs field below to assign one or more Scheduled Order IDs from QPilot to this Order. Enter more than one id by adding ids separated by a comma.', 'autoship' ) ); ?></p>
				...
				<div class="auto-flex-row">
					...
					<div class="form-field form-field-wide autoship-scheduled-order-ids auto-flex-col">
						<label for="autoship_assigned_order_ids">
							<?php echo esc_html( __( 'Scheduled Order IDs', 'autoship' ) ); ?>
						</label>
				...
				<div class="action-button-bar">
					<div class="action-buttons">
						<button type="submit" id="apply-autoship-scheduled-orders-btn" class="button button-primary">
							Assign Scheduled Order IDs
						</button>
						
						<?php if ( ! empty( $val ) ) : ?>

							<button class="button button-secondary autoship-order-actions-btn"
									data-target-action="autoship_clear_existing_order_keys">Remove Assigned IDs
							</button>
	...
	<?php
	autoship_generate_modal( 'autoship_order_assigned_ids_warning', __( '<h3>Autoship Cloud Warning!</h3><hr/><p><strong><span style=\"color:#11a0d2;\">Important!</span></strong> Changing the Scheduled Order IDs assigned to a WooCommerce Order can affect reporting.</p>', 'autoship' ), 'warning-modal' );
}
...
function autoship_order_dashboard_columns() {
	...
	$columns = array(
		'order_autoship_origin' => array(
			...
			'meta_key' => '_qpilot_scheduled_order_id', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
			'orderby'  => 'meta_value',
		...
		'order_autoship_icon'   => array(
			'location' => 'order_status',
			'label'    => '',
			'meta_key' => '_autoship_scheduled_order_type', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
			'orderby'  => 'meta_value',
	...
	if ( autoship_display_next_occurrence_offset_enabled() ) {
		...
		$columns['order_autoship_next_occurrence_offset'] = array(
			...
			'meta_key' => '_qpilot_next_occurrence_offset', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
			'orderby'  => 'meta_value',
 ...
 * @param bool $evaluate True to return the value else echo.
 *
 * @return string The Ship BY Date
 */
function autoship_populate_next_occurrence_offset( $id, $evaluate = false ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	if ( ! $date_created ) {
		echo apply_filters( 'autoship_populate_next_occurrence_offset_empty_created_date', '&ndash;', $offset, $order ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

		return;
	} elseif ( empty( $offset ) ) {
		echo apply_filters( 'autoship_populate_next_occurrence_offset_empty', '&ndash;', $order ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	$display = sprintf( '<time datetime="%1$s" title="%2$s">%3$s</time>', esc_attr( $order_date->format( 'c' ) ), esc_html( $order_date->format( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ) ) ), esc_html( $show_date ) );

	echo apply_filters( 'autoship_populate_next_occurrence_offset_column_val', $display, $offset, $order ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
...
function autoship_populate_order_dashboard_origin( $id ) {
	...
	echo apply_filters( 'autoship_populate_order_dashboard_origin', $external_id, $order ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
...
function autoship_populate_order_dashboard_icon( $id ) {
	...
	if ( ! empty( $external_data['origin_id'] ) ) {
		...
		// translators: %d: Scheduled Order ID.
		...
		// translators: %s: Scheduled Order IDs.
		$tooltip[] = wc_sanitize_tooltip( sprintf( __( 'Scheduled Order(s): %s', 'autoship' ), implode( ', ', $external_data['scheduled_ids'] ) ) );

		// Otherwise do we know if this is even a source or result?
	} elseif ( ! empty( $external_data['source'] ) ) {
		$external_data['class'] = 'source' === $external_data['source'] ? 'maybe-scheduled' : 'maybe-created';
		$tooltip[]              = wc_sanitize_tooltip( 'source' === $external_data['source'] ? __( 'This order contains Items with associated Schedules and may have produced scheduled orders.', 'autoship' ) : __( 'This order might have been generated by a Scheduled Order.', 'autoship' ) );
	}
	...
	printf( '<mark class="order-origin-icon %s tips" data-tip="%s"><span></span></mark>', esc_attr( $external_data['class'] ), apply_filters( 'autoship_populate_order_dashboard_icon_tooltip', wp_kses_post( implode( "\n", $tooltip ) ), $tooltip, $external_data, $order ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
...
function autoship_order_dashboard_custom_column_sort( $query ) {
	...
	if ( isset( $columns[ $_GET['orderby'] ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
function autoship_order_populate_dashboard_columns( $column, $post_id ) {
	...
	if ( isset( $autoship_columns[ $column ] ) ) {
		...
		echo function_exists( $callback ) ? $callback( $post_id ) : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
function autoship_group_order_items( $order_items, $order = null ) {
	...
	foreach ( $order_items as $item_key => $item ) {
		...
		if ( ! isset( $frequencies_hash[ $item_hash_key ] ) ) {
			$frequencies_hash[ $item_hash_key ] = $item_hash_values + array( 'items' => array() );
		}
...
function autoship_order_total_scheduled_items( $order ) {
	...
	foreach ( $order_items as $item ) {
		if ( ( isset( $item['autoship_frequency_type'] ) && ! empty( $item['autoship_frequency_type'] ) ) && ( isset( $item['autoship_frequency'] ) && ! empty( $item['autoship_frequency'] ) ) ) {
...
function autoship_get_scheduled_order_payment_data( $order ) {
	Logger::trace( 'autoship_get_scheduled_order_payment_data', 'Getting Scheduled Order Payment Data' );

	// Get order.
	if ( is_numeric( $order ) ) {
		Logger::trace( 'autoship_get_scheduled_order_payment_data', 'The woo commerce order is numeric' );

		$order = wc_get_order( $order );
	}

	if ( ! $order ) {
		Logger::trace( 'autoship_get_scheduled_order_payment_data', 'The woo commerce order is not valid' );
	...
	return apply_filters(
		'autoship_get_scheduled_order_payment_data',
		isset( $payment_data ) ?
			autoship_generate_customer_billing_data(
				$order->get_user_id(),
				array(
 ...
 */
function autoship_adjust_sepa_order_payment_method_data( $billing_data, $order, $payment_data ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed

	if ( isset( $billing_data['description'] ) && 'SEPA IBAN' === $billing_data['description'] ) {
 ...
 *
 * @param WC_Order|int $order A WC Order object or order id.
 * @param string       $extension Optional. An extension to add to the id. Default empty string.
 *
...
function autoship_get_scheduled_order_external_id( $order, $extension = '' ) {
	...
	return apply_filters( 'autoship_create_scheduled_order_external_id', ! $order ? $extension : 'wc-' . $order->get_id() . $extension, $order, $extension );
}

/**
 * Creates and Gathers the Scheduled order Data. Uses the following functions.
 *
 * @param WC_Order|int $order A WC Order object or order id.
 * @param string       $creation_date Optional. The date the order should be
...
function autoship_get_scheduled_order_data( $order, $creation_date = null ) {
	...
	$grouped_items = autoship_group_order_items( $order_items, $order );
	...
	$scheduled_order_data['origin'] = in_array( $order->get_created_via(), $orders_created_via, true ) ? 'CustomerCheckout' : 'CustomerApi';
	...
	foreach ( $grouped_items as $frequency_group ) {
		...
		$next_occurrence = ! empty( $frequency_group['next_occurrence'] ) ? autoship_format_next_occurrence_for_save( $frequency_group['next_occurrence'] ) : null;
		...
		$new_sheduled_order['originalExternalId'] = autoship_get_scheduled_order_external_id( $order, ! empty( $next_occurrence ) ? "-{$frequency_key}-{$next_occurrence}" : "-{$frequency_key}" );
...
function autoship_get_wc_order_source_id( $order ) {
	...
	return ! empty( $origin_id ) ? autoship_get_scheduled_order_wc_source_id( $origin_id ) : false;
}
...
function autoship_get_scheduled_order_wc_source_id( $scheduled_order_id ) {
	...
	if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
		$wc_order_id = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
			$wpdb->prepare(
				"SELECT order_id FROM {$wpdb->prefix}wc_orders_meta WHERE meta_key = %s AND meta_value LIKE '%-%d-%'", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.LikeWildcardsInQuery
		...
		$wc_order_id = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
			$wpdb->prepare(
				"SELECT post_id FROM $wpdb->postmeta WHERE meta_key = %s AND meta_value LIKE '%-%d-%'", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.LikeWildcardsInQuery
				'_autoship_created_scheduled_orders_key',
				$scheduled_order_id
			)
		);
...
function autoship_get_shipping_tax_rates_from_order( $tax_class, $location, $order ) {
	...
	extract( $location ); // phpcs:ignore WordPress.PHP.DontExtract.extract_extract
...
function autoship_qpilot_orders_update_via_rest_adjust_prices_for_taxes( $and_taxes, $order ) {
	...
	foreach ( $order->get_items( 'shipping' ) as $item_id => $item ) {
		...
		$shipping_tax_rates = autoship_get_shipping_tax_rates_from_order(
			$shipping_tax_class,
			array(
				'country'  => $country,
				'state'    => $state,
 ...
 *
 * @param bool     $updated True if the order has been updated.
 * @param bool     $adjusted True if the order has been adjusted.
 * @param WC_Order $order The WC Order object.
 *
 ...
 */
function autoship_adjust_line_item_tax_after_rest_save( $order, $request, $creating ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @param array|int $vals The Scheduled Order ids to set.
 *
...
function autoship_maybe_save_new_scheduled_order_ids( $order ) {
	...
	// Prevent loop when $order->save() is called later in metadata update.
	remove_action( 'woocommerce_after_order_object_save', 'autoship_maybe_save_new_scheduled_order_ids', 10, 1 );

	// Only save the ids if not empty in the form else we re-assign legacy ids.
	if ( isset( $_POST['autoship_assigned_order_ids'] ) && ! empty( $_POST['autoship_assigned_order_ids'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
		autoship_save_assigned_scheduled_order_ids_metabox( $order, $_POST['autoship_assigned_order_ids'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	} else {
...
function autoship_save_assigned_scheduled_order_ids_metabox( $order, $val ) {
	...
	foreach ( $ids as $key => $value ) {
		...
		if ( ! in_array( $value, $existing_ids, true ) ) {
			$existing_ids[] = $value;
		}
	}
	...
	autoship_add_message(
		empty( $ids ) ?
			__( 'Assigned Autoship Scheduled Order IDs Cleared.', 'autoship' ) :
			// translators: %s is the list of assigned scheduled order ids.
			sprintf( __( 'Autoship Scheduled Order ID(s) <strong>%s</strong> assigned Successfully.', 'autoship' ), implode( ', ', $ids ) ),
		'updated',
 ...
 * @param float  $item_recurring_price Optional. The Sale Price to use. If not supplied uses the WC Product Autoship Recurring Price. or sale price. Default NULL.
 *
...
function autoship_create_scheduled_order_item_data( $order_id, $product_id, $external_id, $qty, $frequency_type, $frequency, $price = null, $item_recurring_price = null ) {
	...
	$prices = wp_parse_args(
		...
		array(
			'regular_price'            => $price,
			'autoship_recurring_price' => $item_recurring_price,
		)
	);

	// Item price & recurring sale price.
	$item_price = apply_filters( 'autoship_create_scheduled_order_item_price', floatval( $prices['regular_price'] ), $frequency_type, $frequency, $product_id, $external_id );

	$item_recurring_price = apply_filters( 'autoship_create_scheduled_order_item_sale_price', isset( $prices['autoship_recurring_price'] ) ? floatval( $prices['autoship_recurring_price'] ) : null, $frequency_type, $frequency, $product_id, $external_id );

	// Get the Product specific line item data.
	return apply_filters(
		'autoship_create_scheduled_order_item_data',
		array(
			'quantity'         => $qty,
 ...
 * @param int    $product_id The Autoship Product id.
 * @param int    $external_id The wc product id.
 * @param string $frequency_type Optional. The frequency Type being used. Supplied for filters.
 * @param string $frequency Optional. The frequency being used. Supplied for filters.
 *
...
function autoship_create_scheduled_order_item_product_data( $order_id, $product_id, $external_id, $frequency_type = '', $frequency = '' ) {
	...
	$item_price = apply_filters( 'autoship_create_scheduled_order_item_product_price', floatval( $product->get_regular_price() ), $frequency_type, $frequency, $product );

	// Get the Autoship re-order Sale Price.
	$autoship_recurring_price = autoship_get_product_recurring_price( $external_id, $frequency_type, $frequency );

	// The product level sale price is either the recurring price or the wc sale price.
	$item_sale_price = apply_filters( 'autoship_create_scheduled_order_item_product_sale_price', $autoship_recurring_price, $frequency_type, $frequency, $product );

	// Get the Product specific line item data.
	$scheduled_order_item_product_data = array(
		'product' => autoship_generate_product_upsert_data(
			$product,
			array(
				'price'     => $item_price,
	...
	return apply_filters( 'autoship_create_scheduled_order_item_product_data', $scheduled_order_item_product_data, $frequency_type, $frequency, $product_id, $external_id );
}
...
function autoship_create_scheduled_orders( $order, $creation_date = null ) {
	...
	Logger::trace( 'Autoship Orders Creator', "Prepare Scheduled Orders Data for WooCommerce Order: $order_id" );
	$creation_start_time = floor( microtime( true ) * 1000 );

	// Create the scheduled order data to upsert. Group the data for the order by frequency, frequency type, next occurrence, and preferred shipping rate.
	$group_scheduled_order_data = autoship_get_scheduled_order_data( $order, $creation_date );

	$creation_end_time = floor( microtime( true ) * 1000 );
	$creation_elapsed  = $creation_end_time - $creation_start_time;
	Logger::trace( 'Autoship Orders Creator', "Prepared Scheduled Orders for WooCommerce Order: $order_id Elapsed: $creation_elapsed milliseconds" );
	...
	foreach ( $group_scheduled_order_data as $scheduled_order_data ) {
		...
		Logger::trace( 'Autoship Orders Creator', "Invoking Scheduled Orders Data Filter for WooCommerce Order: $order_id and Scheduled Order Data Index: $index" );
		$filter_start_time = floor( microtime( true ) * 1000 );

		$scheduled_order_data = apply_filters( 'autoship_create_scheduled_order_data', $scheduled_order_data, $order_id );

		$filter_end_time = floor( microtime( true ) * 1000 );
		$filter_elapsed  = $filter_end_time - $filter_start_time;
		Logger::trace( 'Autoship Orders Creator', "Invoked Scheduled Orders Data Filter for WooCommerce Order: $order_id and Scheduled Order Data Index $index Elapsed: $filter_elapsed milliseconds" );

		Logger::trace( 'Autoship Orders Creator', "Sending Scheduled Order Creation to QPilot for WooCommerce Order: $order_id and Scheduled Order Data Index: $index" );
		$qpilot_start_time = floor( microtime( true ) * 1000 );
		...
		$qpilot_end_time = floor( microtime( true ) * 1000 );
		$qpilot_elapsed  = $qpilot_end_time - $qpilot_start_time;
		Logger::trace( 'Autoship Orders Creator', "Received Scheduled Order Creation Response from QPilot for WooCommerce Order: $order_id and Scheduled Order Data Index $index Elapsed: $qpilot_elapsed milliseconds" );

		if ( is_wp_error( $scheduled_order ) ) {
			$failure_start_time = floor( microtime( true ) * 1000 );

			// Attach the order keys, save the error and add an error note.
			// translators: %1$d is the order id, %2$s is the error message.
			$note  = sprintf( __( 'Creating Scheduled Orders from Order #%1$d Failed.  Additional Details: %2$s' ), $order_id, $scheduled_order->get_error_message() );
			...
			$failure_end_time = floor( microtime( true ) * 1000 );
			$failure_elapsed  = $failure_end_time - $failure_start_time;

			Logger::trace( 'Autoship Orders Creator', "Failed Creation of Scheduled Order in QPilot for WooCommerce Order: $order_id and Scheduled Order Data Index: $index Elapsed: $failure_elapsed milliseconds" );

		} else {
			$success_start_time = floor( microtime( true ) * 1000 );
			...
			$success_end_time = floor( microtime( true ) * 1000 );
			$success_elapsed  = $success_end_time - $success_start_time;

			Logger::trace( 'Autoship Orders Creator', "Successful Creation of Scheduled Order in QPilot for WooCommerce Order: $order_id and Scheduled Order Data Index: $index. Scheduled Order Id: $scheduled_order->id Elapsed: $success_elapsed milliseconds" );
		}

		++$index;
	}

	$storing_start_time = floor( microtime( true ) * 1000 );
	...
	if ( ! empty( $group_scheduled_order_data ) ) {
		...
		if ( empty( $error ) ) {
			// translators: %s is the list of scheduled order ids.
	...
	$storing_end_time = floor( microtime( true ) * 1000 );
	$storing_elapsed  = $storing_end_time - $storing_start_time;

	Logger::trace( 'Autoship Orders Creator', "Finishing Persisting Scheduled Order Data in WooCommerce Order: $order_id Elapsed: $storing_elapsed milliseconds" );

	$end_time = floor( microtime( true ) * 1000 );
	$elapsed  = $end_time - $start_time;

	if ( Logger::is_logging_enabled() ) {
		$output_data = var_export( $scheduled_order_ids, true ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export

		Logger::trace( 'Autoship Orders Creator', "Finished Creating Scheduled Orders for WooCommerce Order: $order_id Elapsed time: $elapsed milliseconds. Scheduled Order Ids: $output_data" );
	}
 ...
 *
 * @param array $scheduled_order_ids Array of Scheduled order IDs.
 * @param int   $order_id The WC Order ID.
...
function autoship_verify_payment_method_on_generated_scheduled_orders( $scheduled_order_ids, $order_id, $scheduled_orders ) {
	...
	foreach ( $scheduled_orders as $scheduled_order ) {
		...
		if ( ! isset( $scheduled_order->paymentMethod ) || empty( $scheduled_order->paymentMethod ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
...
/**
 * Creates / Upserts a Scheduled Order for any WC Order generated at checkout but put on Hold.
 *
 ...
 */
add_action( 'add_meta_boxes', 'autoship_register_edit_order_metaboxes', 9 );
