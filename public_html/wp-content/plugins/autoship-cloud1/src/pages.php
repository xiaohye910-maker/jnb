<?php
/**
 * The pages functions for the Autoship plugin.
 *
 * @package Autoship
 * @since 1.0.0
 */
 ...
 *
 * @param ?string  $nav_menu Nav menu output to short-circuit with.
 * @param stdClass $args An object containing wp_nav_menu() arguments.
 *
 * @return string|null Nav menu output
 * @see wp_nav_menu()
 */
function autoship_scheduled_orders_remove_title_filter_nav_menu( $nav_menu, $args ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @see wp_nav_menu()
 */
function autoship_scheduled_orders_add_title_filter_non_menu( $items, $args ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
 ...
 * @see wp_nav_menu()
 */
function autoship_scheduled_order_remove_title_filter_nav_menu( $nav_menu, $args ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	// we are working with menu, so remove the title filter.
 ...
 * @see wp_nav_menu()
 */
function autoship_scheduled_order_add_title_filter_non_menu( $items, $args ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	// we are done working with menu, so add the title filter back.
 ...
 *
 * @param int $value The customer ID to show scheduled orders for.
 */
 ...
 *
 * @param int $value The scheduled order ID to show.
 */
 ...
 */
function autoship_add_to_scheduled_order_endpoint_redirect( $redirect = null, $user = null ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed

	global $wp_query;

	// Check for woocommerce endpoint, user logged in and addtoscheduled order var.
	if ( ! apply_filters( 'autoship_add_to_scheduled_order_endpoint_redirect_validation', isset( $wp_query->query_vars['scheduled-orders'] ) && is_account_page() && isset( $_GET['action'] ) && ! empty( $_GET['action'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		return $redirect;
		// If the user isn't logged in let the existing controls deal with it.
	} elseif ( ! is_user_logged_in() ) {
		return $redirect;
		// Ahhh it's ours - fire away.
	} elseif ( apply_filters( 'autoship_is_add_to_scheduled_order_endpoint_redirect_action', true ) ) {
		do_action( 'autoship_pre_add_to_scheduled_order_link_handler' );

		$success = autoship_add_to_scheduled_order_endpoint_wrapper( $_GET['action'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
 ...
 */
function autoship_add_to_scheduled_order_endpoint_redirect_is_valid_action( $redirect ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
	return isset( $_GET['action'] ) && ! ( 'create-scheduled-order' === $_GET['action'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
}
 ...
 * @return string|NULL The redirect string or null.
 */
function autoship_create_scheduled_order_endpoint_redirect( $redirect = null, $user = null ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed

	global $wp_query;

	// Check for woocommerce endpoint, user logged in and createscheduledorder order var.
	if ( ! apply_filters( 'autoship_create_scheduled_order_endpoint_redirect_validation', isset( $wp_query->query_vars['scheduled-orders'] ) && is_account_page() && isset( $_GET['action'] ) && ! empty( $_GET['action'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		// Ahhh it's ours - fire away.
	} elseif ( apply_filters( 'autoship_is_create_scheduled_order_endpoint_redirect_action', true ) ) {

		do_action( 'autoship_pre_create_scheduled_order_link_handler' );

		$success = autoship_create_scheduled_order_endpoint_wrapper( $_GET['action'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
function autoship_account_add_endpoints() {
	...
	foreach ( $endpoints as $key => $endpoint ) {
		add_rewrite_endpoint( $endpoint, apply_filters( 'autoship_account_add_endpoint_masks', EP_ROOT | EP_PAGES, $key, $endpoint ) );
	}
...
function autoship_non_endpoint_query_vars( $vars ) {
	...
	$autoship_vars = apply_filters(
		'autoship_non_endpoint_query_vars',
		array(
			'autoship_method_id',
			'autoship_method_type',
		)
	);
