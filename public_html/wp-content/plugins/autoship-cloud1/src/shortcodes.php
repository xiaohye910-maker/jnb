<?php
/**
 * Contains the functions regarding the shortcodes of the plugin.
 *
 * @package Autoship
 * @since 1.0.0
 */
...
function autoship_shortcode_create_scheduled_order_action( $attributes = array() ) {
	...
	$attributes = shortcode_atts(
		array(
			...
			'products'    => '',   // A list of product ids and associated qty that should be added to the order by default  Ids and Qty should be separated by a : (i.e. 244:2 = Product ID 244 Qty 2 ).  Multiple Products should be separated by a comma.
			...
			'mincycle'    => '',   // A single min cycle or array of product ids and min cycles to use  Ids and Min Cycles should be separated by a : (i.e. 244:2 = Product ID 244 Min Cyle 2 ).  Multiple Products should be separated by a comma.
			'maxcycle'    => '',   // A single max cycle or array of product ids and max cycles to use  Ids and Max Cycles should be separated by a : (i.e. 244:2 = Product ID 244 Max Cyle 2 ).  Multiple Products should be separated by a comma.
			'atts'        => '',   // Any additional data attributes to attach to the link.
			// translators: %s is the Scheduled Order.
			'label'       => sprintf( __( 'Create %s', 'autoship' ), autoship_translate_text( 'Scheduled Order' ) ),  // The Button/Link Label.
		),
		$attributes,
		'autoship-create-order-action'
	);
	...
	if ( $attributes['customer_id'] && ( ( get_current_user_id() == $attributes['customer_id'] ) || autoship_rights_checker( 'autoship_filter_allow_customer_var_on_create_order', array( 'administrator' ) ) ) ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
	...
	$link = apply_filters( 'autoship_shortcode_create_scheduled_order_action_link', sprintf( '%s<a href="%s" class="button autoship-action-btn %s" %s >%s</a>%s', wp_kses_post( $attributes['wrap_start'] ), esc_url( autoship_get_scheduled_order_create_url( $params ) ), esc_attr( $attributes['classes'] ), wp_kses_post( $attributes['atts'] ), esc_html( $attributes['label'] ), wp_kses_post( $attributes['wrap_end'] ) ), $attributes, $params );
...
function autoship_shortcode_scheduled_orders( $attributes = array() ) {
	...
	$attributes = shortcode_atts(
		...
		$attributes,
		'autoship_shortcode_scheduled_orders'
	);

	// If a customer id is supplied be sure the current user is belongs to the id or has rights.
	if ( $attributes['customer_id'] && ( get_current_user_id() != $attributes['customer_id'] ) ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseNotEqual

		// If they don't have rights nada!  By default only admins can see other users' accounts.
		if ( ! autoship_rights_checker( 'autoship_customer_accounts_view_scheduled_orders_rights', array( 'administrator' ) ) ) {
			// translators: %s is the number of scheduled orders.
	...
	// translators: %s is the number of scheduled orders.
	return function_exists( $display_function ) ? $display_function( $attributes['customer_id'], $attributes['autoship_customer_id'], $attributes['template'], $attributes['page'], $attributes['paginate'] ) : sprintf( __( 'No %s found.', 'autoship' ), autoship_translate_text( 'scheduled orders' ) );
}
...
function autoship_shortcode_scheduled_order( $attributes = array() ) {
	$attributes = shortcode_atts(
		...
		$attributes,
		'autoship_shortcode_scheduled_order'
	);
...
function autoship_shortcode_view_scheduled_order( $attributes = array() ) {
	$attributes = shortcode_atts(
		...
		$attributes,
		'autoship_shortcode_view_scheduled_order'
	);
		...
		try {
			...
			return autoship_render_template(
				...
				array(
					...
					'token_auth'                => $access_token->TokenAuth, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				)
			);
		} catch ( Exception $e ) {
