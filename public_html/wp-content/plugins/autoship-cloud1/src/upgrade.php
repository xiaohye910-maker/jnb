<?php
/**
 * Contains the functions regarding the upgrade functions of the plugin.
 *
 * @package Autoship
 * @since 1.0.0
 */
 ...
 *
 * @param string $version The version to which the site should be converted.
 */
function autoship_convert_site_processing( $version = 'v3' ) {
	...
	if ( 'v3' === $version ) {
		...
		try {
			...
			$migrate = autoship_is_user_http_message( $e->getCode() ) ? new WP_Error( $notice['msg'], $notice['desc'] ) : new WP_Error( 'Site Processing Version Migration Failed', __( $notice['desc'], 'autoship' ) ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
 ...
 */
function autoship_maybe_convert_site_processing( $site ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
function autoship_has_legacy_origin() {
	...
	$legacy     = $wpdb->get_var( "SHOW TABLES LIKE '$table_name'" ) === $table_name; // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared
 ...
 * @param string $version The version to which was just upgraded to.
 */
function autoship_new_install_trigger_handler( $version ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
 ...
 * @param string $version The version to which was just upgraded to.
 */
function autoship_new_install_set_processing_version( $version ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
...
function autoship_legacy_version_notice_trigger() {
	...
	if ( ( false === $notice ) || isset( $_GET['autoship_activate_sync_upgrade'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	?>
...
function autoship_legacy_processing_version_notice_trigger() {
	...
	if ( ( 'vX' === $processing_version ) || ( 'v3' === $processing_version ) || isset( $_GET['autoship_activate_legacy_processing_upgrade'] ) || autoship_is_new() ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
 ...
 */
function autoship_ppc_gateway_upgrade_warning_notice( $plugin_file, $plugin_data, $status ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed

	// Only display notice if the version of Papal Checkout displays the Payments notice.
	if ( ! isset( $plugin_data['Version'] ) || version_compare( $plugin_data['Version'], '2.1.2', '<' ) || ! isset( $plugin_data['plugin'] ) || ! is_plugin_active( $plugin_data['plugin'] ) ) {
		return;
	}
		...
		<td colspan="4" class="plugin-update colspanchange">
			<div class="update-message notice inline notice-error notice-alt">
				<div class='autoship-ppec-migrate-notice-title autoship-ppec-migrate-notice-section'>
					<p><?php echo wp_kses_post( __( '<strong>IMPORTANT!</strong> The new PayPal Payments plugin is not currently supported by Autoship Cloud.  Please do not attempt to replace WooCommerce PayPal Checkout Gateway with WooCommerce PayPal Payments.', 'autoship' ) ); ?></p>
				</div>
			</div>
		</td>
	</tr>
...
function autoship_upgrade_site_processing_version_endpoint_wrapper() {
	...
	$nonce_value = wc_get_var( $_POST['autoship-activate-legacy-processing-upgrade-nonce'], wc_get_var( $_POST['_wpnonce'], '' ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	if ( ! wp_verify_nonce( $nonce_value, 'autoship-activate-legacy-processing-upgrade' ) ) {

		autoship_add_message( wp_kses_post( __( '<strong>Invalid or Expired Call.</strong>', 'autoship' ) ), 'notice-error', 'autoship_upgrade' ); // phpcs:ignore WordPress.WP.I18n.NoHtmlWrappedStrings
		$success = false;

		// Validate that the user confirmation check box is checked.
	} elseif ( ! isset( $_POST['autoship_upgrade_site_processing_version'] ) || 'confirm' !== $_POST['autoship_upgrade_site_processing_version'] ) {

		autoship_add_message( wp_kses_post( __( '<strong>Invalid Upgrade Submission. Please confirm you would like to upgrade processing to v3 and submit your request again.</strong>', 'autoship' ) ), 'notice-error', 'autoship_upgrade' ); // phpcs:ignore WordPress.WP.I18n.NoHtmlWrappedStrings
		$success = false;
		...
		if ( is_wp_error( $success ) ) {
			...
			autoship_add_message( wp_kses_post( sprintf( __( '<strong>Error Upgrading Processing Version:</strong> Error Code %1$s - %2$s', 'autoship' ), $success->get_error_code(), $success->get_error_message() ) ), 'notice-error', 'autoship_upgrade' );
			$success = false;

		} else {

			autoship_add_message( wp_kses_post( __( '<strong>Autoship Cloud: Scheduled Orders Upgraded Successfully!</strong> You have completed the upgrade to the latest version of Autoship Cloud.', 'autoship' ) ), 'notice-success', 'autoship_upgrade' );
...
function autoship_downgrade_site_processing_version_endpoint_wrapper() {
	...
	if ( ! isset( $_GET['autoship-action'] ) || ! autoship_rights_checker( 'autoship_admin_downgrade_processing_version', array( 'administrator' ) ) || ! isset( $_GET['autoship_downgrade_site_processing_version'] ) || 'confirm' !== $_GET['autoship_downgrade_site_processing_version'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	...
	if ( is_wp_error( $success ) ) {
		...
		// translators: %1$s is the error code, %2$s is the error message.
		autoship_add_message( wp_kses_post( sprintf( __( '<strong>Error Downgrading Processing Version:</strong> Error Code %1$s - %2$s', 'autoship' ), $success->get_error_code(), $success->get_error_message() ) ), 'notice-error', 'autoship_downgrade' );
	} else {

		autoship_add_message( wp_kses_post( __( '<strong>Autoship Cloud: Plugin Processing Version Downgraded Successfully!</strong>.', 'autoship' ) ), 'notice-success', 'autoship_downgrade' );
	}
