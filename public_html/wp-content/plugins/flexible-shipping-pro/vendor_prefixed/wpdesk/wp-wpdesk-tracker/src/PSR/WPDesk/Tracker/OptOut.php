    ...
    public function handle_opt_out()
    {
        ...
        if ('plugins' === $screen->id && isset($_GET['wpdesk_tracker_opt_out_' . $this->plugin_slug])) {
            ...
            if (wp_verify_nonce($security, $this->plugin_slug)) {
                ...
                new Notice(sprintf(esc_html__('You successfully opted out of collecting usage data by %1$s. If you change your mind, you can always opt in later in the plugin\'s quick links.', 'flexible-shipping-pro'), esc_html($this->plugin_name)));
            }
        }
    }
}
