 ...
 */
class WOE_HTMLPurifier_Injector_AutoParagraph extends WOE_HTMLPurifier_Injector
{
    /**
     * @type string
     */
    ...
    /**
     * @return WOE_HTMLPurifier_Token_Start
     */
    private function _pStart()
    {
        $par = new WOE_HTMLPurifier_Token_Start('p');
        $par->armor['MakeWellFormed_TagClosedError'] = true;
        return $par;
    }

    /**
     * @param WOE_HTMLPurifier_Token_Text $token
     */
        ...
        if ($this->allowsElement('p')) {
            if (!empty($this->currentNesting)) {
                if ($this->_isInline($token)) {
                    ...
                    if (!$prev instanceof WOE_HTMLPurifier_Token_Start) {
                        // Token wasn't adjacent
                        if ($prev instanceof WOE_HTMLPurifier_Token_Text &&
                            substr($prev->data, -2) === "\n\n"
                        ) {
                ...
                if ($this->backward($i, $prev)) {
                    if (!$prev instanceof WOE_HTMLPurifier_Token_Text) {
                        ...
                        array_unshift($token, new WOE_HTMLPurifier_Token_Text("\n\n"));
                    } else {
                        // State 3.1.2: ...</p>\n\n{p}<b>
                        //                            ---
                        // State 3.2.2: ...</p>\n\n<div>
     ...
     *    into paragraphs
     * @param WOE_HTMLPurifier_Token[] $result Reference to array of tokens that the
    ...
    private function _splitText($data, &$result)
    {
        ...
        if ($c == 1) {
            ...
            // should never happen.
            $result[] = new WOE_HTMLPurifier_Token_Text($data);
            return;
        }
        for ($i = 0; $i < $c; $i++) {
            ...
            if (trim($par) !== '') {
                ...
                if ($i == 0) {
                    ...
                    if (empty($result)) {
                        ...
                        $result[] = new WOE_HTMLPurifier_Token_End('p');
                        $result[] = new WOE_HTMLPurifier_Token_Text("\n\n");
                        ...
                        array_unshift($result, new WOE_HTMLPurifier_Token_Text("\n\n"));
                    }
                } elseif ($i + 1 == $c) {
        ...
        foreach ($paragraphs as $par) {
            $result[] = new WOE_HTMLPurifier_Token_Text($par);
            $result[] = new WOE_HTMLPurifier_Token_End('p');
            $result[] = new WOE_HTMLPurifier_Token_Text("\n\n");
            $result[] = $this->_pStart();
        }
    ...
    {
        if ($this->currentToken instanceof WOE_HTMLPurifier_Token_Start) {
            $nesting = 1;
        } else {
            $nesting = 0;
        }
     ...
     * @param WOE_HTMLPurifier_Token $current
     * @return bool
     */
    private function _checkNeedsP($current)
    {
        if ($current instanceof WOE_HTMLPurifier_Token_Start) {
        ...
        } elseif ($current instanceof WOE_HTMLPurifier_Token_Text) {
