...
class WOE_HTMLPurifier_Injector_RemoveEmpty extends WOE_HTMLPurifier_Injector
{
    /**
    ...
    public function prepare($config, $context)
    {
        ...
        foreach ($this->exclude as $key => $attrs) {
            if (!is_array($attrs)) {
                // HACK, see WOE_HTMLPurifier/Printer/ConfigForm.php
    ...
    public function handleElement(&$token)
    {
        ...
        for ($i = count($this->inputZipper->back) - 1; $i >= 0; $i--, $deleted++) {
            ...
            if ($next instanceof WOE_HTMLPurifier_Token_Text) {
        ...
        if (!$next || ($next instanceof WOE_HTMLPurifier_Token_End && $next->name == $token->name)) {
            ...
            for ($b = 0, $c = count($this->inputZipper->front); $b < $c; $b++) {
                ...
                if ($prev instanceof WOE_HTMLPurifier_Token_Text && $prev->is_whitespace) {
                    continue;
                }
                break;
            }
